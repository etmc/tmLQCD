...
int write_lime_spinor(spinor * const s, spinor * const r, char * filename) {
  ...
  if(g_cart_id == 0) {
    ...
    MB_flag=1; ME_flag=1;
    limeheader = limeCreateHeader(MB_flag, ME_flag, "etmc-spinorfield-binary-data", bytes);
    status = limeWriteRecordHeader( limeheader, limewriter);
    if(status < 0 ) {
      fprintf(stderr, "LIME write header (etmc-spinorfield-binary-data) error %d\n", status);
#ifdef MPI
      MPI_Abort(MPI_COMM_WORLD, 1);
      MPI_Finalize();
#endif
...
int read_lime_spinor(spinor * const s, spinor * const r, char * filename) {
  ...
  while( (status = limeReaderNextRecord(limereader)) != LIME_EOF ) {
    ...
    header_type = limeReaderType(limereader);
    if(!strcmp("etmc-spinorfield-binary-data",header_type)) break;
  }
  if(status == LIME_EOF) {
    if(g_proc_id == 0) {
      fprintf(stderr, "no etmc-spinorfield-binary-data record found in file %s\n",filename);
    }
    limeDestroyReader(limereader);
    fclose(ifs);
    return(-1);
