...
static inline void omp_re_acc_add(omp_re_acc_t* const acc, const double* const values,
...
static inline void omp_cplx_acc_add(omp_cplx_acc_t* const acc, const complex double* const values,
                                    const int num_values) {
  TM_CHECK_INIT_OMP_ACC(acc);
  TM_CHECK_BOUNDS_OMP_ACC(acc, num_values);
  TM_CHECK_NUM_THREADS_OMP_ACC(acc);
