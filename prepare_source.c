  ...
  if (optr->type != DBTMWILSON && optr->type != DBCLOVER) {
    ...
    if (source_type == SRC_TYPE_POINT || source_type == SRC_TYPE_TS) {
      ...
      /* "Read inversion source from file" */
        ...
        if (SourceInfo.splitted) {
          ...
          if (SourceInfo.automaticTS) {
            ...
            if (g_proc_id == 0) {
              for (t = 0; t < g_nproc_t * T; t++) {
                if (T_global > 99)
                  sprintf(source_filename, "%s.%.4d.%.3d.%.2d", SourceInfo.basename, nstore, t, ix);
                else
          ...
          if (T_global > 99)
            sprintf(source_filename, "%s.%.4d.%.3d.%.2d", SourceInfo.basename, nstore, SourceInfo.t,
                    ix);
          else
            sprintf(source_filename, "%s.%.4d.%.2d.%.2d", SourceInfo.basename, nstore, SourceInfo.t,
                    ix);
          if (g_cart_id == 0) {
        ...
        if (rstat) {
          fprintf(stderr,
                  "Error reading file %s in prepare_source.c\nUnable to proceed, aborting....\n",
                  source_filename);
          exit(-1);
        }
      }
      if (PropInfo.splitted) {
        if (T_global > 99)
          sprintf(source_filename, "%s.%.4d.%.3d.%.2d.inverted", PropInfo.basename, nstore,
                  SourceInfo.t, ix);
        else
          sprintf(source_filename, "%s.%.4d.%.2d.%.2d.inverted", PropInfo.basename, nstore,
                  SourceInfo.t, ix);
      } else {
        if (T_global > 99)
          sprintf(source_filename, "%s.%.4d.%.3d.inverted", PropInfo.basename, nstore,
                  SourceInfo.t);
        else
          sprintf(source_filename, "%s.%.4d.%.2d.inverted", PropInfo.basename, nstore,
                  SourceInfo.t);
      }
      ...
      // If a pion timeslice source has already been inverted for the current sample and gauge
      // configuration, we would like to re-use the same timeslice, which we ensure with the loop
      // below. The reason for doing this is that we cannot guarantee that the call to ranlxs below
      // is reproducible. Note: source_generation_pion_only reinitialises the RNG with a
      // systematically chosen seed and thus does not suffer from this problem when called below.
      if (SourceInfo.automaticTS) {
        int found = 0;
        if (g_proc_id == 0 && !PropInfo.splitted) {
          for (t = 0; t < g_nproc_t * T; t++) {
            sprintf(source_filename, "%s.%.4d.%.5d.%.2d.inverted", SourceInfo.basename, nstore,
                    isample, t);
            if ((ifs = fopen(source_filename, "r")) != NULL) {
              fclose(ifs);
              found = 1;
      ...
      source_generation_pion_only(g_spinor_field[0], g_spinor_field[1], SourceInfo.t, isample,
                                  nstore, seed);
      sprintf(source_filename, "%s.%.4d.%.5d.%.2d.inverted", PropInfo.basename, nstore, isample,
              SourceInfo.t);
      ...
      sprintf(source_filename, "%s.%.4d.%.5d.%.2d.inverted", SourceInfo.basename, nstore, isample,
              SourceInfo.t);
      ...
      extended_pion_source(g_spinor_field[0], g_spinor_field[1], g_spinor_field[2],
                           g_spinor_field[3], SourceInfo.t, (g_nproc_t * T) / 2, 0., 0., 0.);
      sprintf(source_filename, "%s.%.4d.%.5d.%.2d.inverted", PropInfo.basename, nstore, isample,
              SourceInfo.t);
      // if the generalised pion propagator is to be written to the same file as the source,
      // splitting must be disabled
    ...
    if (optr->solver != CG && optr->solver != PCG && optr->solver != MIXEDCG &&
      ...
      if (ifs != NULL) {
        ...
        rstat =
            read_spinor(optr->prop0, optr->prop1, source_filename, (PropInfo.splitted ? 0 : ix));
        if (rstat) {
    ...
    if (source_type == SRC_TYPE_POINT || source_type == SRC_TYPE_TS) {
      if (read_source_flag == 0 || read_source_flag == 2) {
        if (source_location == 0) {
          ...
          source_spinor_field_point_from_file(g_spinor_field[2], g_spinor_field[3], is, ic,
                                              source_location);
        }
      } else {
        if (SourceInfo.splitted) {
      ...
      gaussian_volume_source(g_spinor_field[0], g_spinor_field[1], isample, nstore, 1);
      gaussian_volume_source(g_spinor_field[2], g_spinor_field[3], isample, nstore, 2);
    }
    mul_one_pm_itau2(g_spinor_field[4], g_spinor_field[6], g_spinor_field[0], g_spinor_field[2],
                     +1., VOLUME / 2);
    mul_one_pm_itau2(g_spinor_field[5], g_spinor_field[7], g_spinor_field[1], g_spinor_field[3],
                     +1., VOLUME / 2);
