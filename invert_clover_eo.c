...
int invert_clover_eo(spinor* const Even_new, spinor* const Odd_new, spinor* const Even,
                     ...
                     solver_params_t solver_params, su3*** gf, matrix_mult Qsq, matrix_mult Qm,
  ...
  if (even_odd_flag) {
    ...
    if (external_inverter == QUDA_INVERTER) {
      return invert_eo_quda(Even_new, Odd_new, Even, Odd, precision, max_iter, solver_flag,
                            rel_prec, even_odd_flag, solver_params, sloppy, compression);
    }
#endif
    ...
    if (external_inverter == QPHIX_INVERTER) {
      ...
      iter =
          cg_her(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2, Qsq);
      Qm(Odd_new, Odd_new);
      ...
      iter = incr_eigcg(VOLUME / 2, solver_params.eigcg_nrhs, solver_params.eigcg_nrhs1, Odd_new,
                        g_spinor_field[DUM_DERI], solver_params.eigcg_ldh, Qsq,
      ...
      iter = mixed_cg_her(Odd_new, g_spinor_field[DUM_DERI], solver_params, max_iter, precision,
                          rel_prec, VOLUME / 2, &Qsw_pm_psi, &Qsw_pm_psi_32);
      Qm(Odd_new, Odd_new);
      ...
      if (g_proc_id == 0) {
        printf("# This solver is not available for this operator. Exiting!\n");
        fflush(stdout);
      }
      return 0;
    }
