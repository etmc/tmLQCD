...
/* |R>=rnorm^2 Q^2 |S> */
void norm_Q_sqr_psi(spinor *const R, spinor *const S, const double rnorm);
...
void norm_Q_n_psi(spinor *const R, spinor *const S, const int n, const double rnorm);
/* this is Q/sqrt(Q^2) */
void Q_over_sqrt_Q_sqr(spinor *const R, double *const c, const int n, spinor *const S,
...
void calculateOverlapPolynomial() {
  ...
  for (int i = ov_n_cheby - 3; i < ov_n_cheby; i++) printf("%d %e\n", i, ov_cheby_coef[i]);
}
...
void init_Dov_WS() {
  ...
  dov_ws->n_spinors = 7;
  if (g_proc_id == 0)
    printf("Initilizing Dov spinor workspace with %d spinors!!!\n", dov_ws->n_spinors);
  allocate_spinor_field_array(&(dov_ws->dum_spinors), &(dov_ws->dum_spinors_membuf), VOLUMEPLUSRAND,
                              dov_ws->n_spinors);
  dov_ws->lock_map = malloc(sizeof(int) * dov_ws->n_spinors);
  for (i = 0; i < dov_ws->n_spinors; i++) dov_ws->lock_map[i] = 0;
}
...
spinor *lock_Dov_WS_spinor(int num) {
  if (num < dov_ws->n_spinors) {
    ...
    if (g_proc_id == 0)
      fprintf(stderr, "Error number of spinor fields exceeded: adjust it to %d in Dov_psi.c !!!!\n",
              num + 1);
    return NULL;
  }
}
...
void unlock_Dov_WS_spinor(int num) {
  if (num < dov_ws->n_spinors) {
    if (dov_ws->lock_map[num] == 1) {
      ...
      if (g_proc_id == 0)
        fprintf(stderr, "spinor %d was not locked already (double unlock ?? )\n", num + 1);
    }
  } else {
    if (g_proc_id == 0)
      fprintf(stderr,
              "Error number of spinor fields exceeded  (in unlock ?? check your unlock indices "
              "against lock indices !!! ): adjust it to %d in Dov_psi.c !!!!\n",
              num + 1);
  }
}
    ...
    */
    ...
    for (j = 2; j <= n - 1; j++) {
      ...
      /*
        ...
        if(g_proc_id == g_stdio_proc){printf("j= %d\t|c T|^2= %g\t c_j= %g\t|r|^2=
