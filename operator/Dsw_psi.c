...
//The above is common with D_psi.c
...
void Block_Dsw_psi(block * blk, spinor * const rr, spinor * const s) {
  ...
  int ix; //lexiographic index of the block site w.r.t the local mpi process
  ...
  for(i = 0; i < blk->volume; i++) {
     ...
     ix = g_ipt[it+sT][ix+sX][iy+sY][iz+sZ];

     assign_mul_one_sw_pm_imu_site_lexic(ix,&tmpr,t,g_mu);
