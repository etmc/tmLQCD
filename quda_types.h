...
typedef enum tm_quda_ferm_bc_t { TM_QUDA_THETABC = 0, TM_QUDA_APBC, TM_QUDA_PBC } tm_quda_ferm_bc_t;
...
static inline int check_quda_clover_state(const tm_QudaCloverState_t* const quda_clover_state,
  ...
  return (quda_clover_state->loaded &&
          ...
          (quda_clover_state->prec_refinement_sloppy ==
...
static inline int check_quda_mg_setup_state(const tm_QudaMGSetupState_t* const quda_mg_setup_state,
  ...
  if ((quda_mg_setup_state->initialised != 1) ||
    ...
    // in other cases, e.g., when the operator parameters change or if the gauge_id has "moved" only
    // a little, we don't need to redo the setup, we can simply rebuild the coarse operators with
    // the new parameters (within reason). Note that we use 2*DBL_EPSILON to have a little bit more
    // wiggle room in case of badly implemented floating point or something like that...
...
static inline void copy_quda_mg_tunable_params_from_input(
    tm_QudaMGTunableParams_t* const tunable_params, const tm_QudaParams_t* const quda_input) {
