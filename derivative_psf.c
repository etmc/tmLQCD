...
void derivative_psf(const int nr) {
  ...
  if(nr == 0) {
    ...
    if(ITER_MAX_BCG == 0 || (fabs(g_mu) > 0) || (g_nr_of_psf != nr+1)) {
      ...
      /*       gamma5(spionr_field[DUM_DERI+1], spinor_field[first_psf], VOLUME/2); */
      ...
      gamma5(spinor_field[DUM_DERI+1], spinor_field[DUM_DERI+1], VOLUME/2);
  ...
  if(nr == 2) {
    ...
    if(ITER_MAX_BCG == 0 || (fabs(g_mu) > 0) || (g_nr_of_psf != nr+1)) {
      /* If CG is used anyhow */
      /*       gamma5(spinor_field[DUM_DERI+1], spinor_field[DUM_DERI+2]); */
      /* Invert Q_{+} Q_{-} */
      /* X_W -> DUM_DERI+1 */
      chrono_guess(spinor_field[DUM_DERI+1], spinor_field[DUM_DERI+2], g_csg_field[nr], g_csg_index_array[nr],
		   g_csg_N[2*nr], g_csg_N[2*nr+1], VOLUME/2, &Qtm_pm_psi);
