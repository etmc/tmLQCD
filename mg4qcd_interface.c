                   ...
                   const int even_odd_flag, su3 **gf, matrix_mult f)
{
   ...
   if (g_proc_id == 0)
     printf("Total iterations on coarse grids %d\n", mg_status.coarse_iter_count);
   if (g_proc_id == 0 && !mg_status.success) 
     printf("ERROR: the solver did not converge!\n");
                   ...
                   const int even_odd_flag, su3 **gf, matrix_mult f)
{
   ...
   if (mg_update_setup>0) {
	...
	printf("ERROR: updating setup did not run correctly");
   }
  ...
  // for rescaling  convention in MG4QCD: (4+m)*\delta_{x,y} in tmLQCD: 1*\delta_{x,y} -> rescale by 1/4+m
   ...
   else
   {
     ...
     /* Hard test commented, soft test kept
      ...
      Msw_full(eo_field[0],eo_field[1] ,Even_new, Odd_new);

      convert_eo_to_lexic(solver_field[2],eo_field[0],eo_field[1]);
      ...
      M_full(eo_field[0],eo_field[1] ,Even_new, Odd_new);
      mul_gamma5(eo_field[0],VOLUME/2); 
      mul_gamma5(eo_field[1],VOLUME/2); 
      g_mu=-g_mu;
      M_full(eo_field[2],eo_field[3], eo_field[0],eo_field[1] );
      
      
      convert_eo_to_lexic(solver_field[2],eo_field[2],eo_field[3]);
      mul_gamma5(solver_field[2],VOLUME);
      
      Q_pm_psi(solver_field[3] ,solver_field[1]);

      //convert_eo_to_lexic(solver_field[3],eo_field[0],eo_field[1]);
