...
void reduce_su3_ray(void *u_i /* in */, void *u_io /* in/out */, int *len /* in */,
                    MPI_Datatype *dt /* in */) {
  int n;
  su3 *u, *v, tmp;
  u = (su3 *)u_i;
...
void tmlqcd_mpi_init(int argc, char *argv[]) {
  ...
  if (g_proc_id == 0) {
    printf(
        "# Creating the following cartesian grid for a %d dimensional parallelisation:\n# %d x %d "
        "x %d x %d\n",
  ...
  if ((g_nproc_t < 1 || g_nproc_x < 1 || g_nproc_y < 1 || g_nproc_z < 1) ||
      (LX % g_nproc_x != 0 || LY % g_nproc_y != 0 || LZ % g_nproc_z != 0 ||
       T_global % g_nproc_t != 0)) {
    if (g_proc_id == 0) {
  ...
  if (g_debug_level > 1) {
    fprintf(stdout, "# Process %d of %d on %s: cart_id %d, coordinates (%d %d %d %d)\n", g_proc_id,
            g_nproc, processor_name, g_cart_id, g_proc_coords[0], g_proc_coords[1],
            g_proc_coords[2], g_proc_coords[3]);
    fflush(stdout);
  }
  if (g_stdio_proc == -1) {
  ...
  /* even/odd ordered spinor fields */
  MPI_Type_vector(T * LX * LY / 2, 12, 24, MPI_DOUBLE,
                  &field_z_slice_half); /* this is ?!? (Not used) */
