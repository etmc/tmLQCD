...
void X_over_sqrt_X_sqr(spinor *const R, double *const c, const int n, spinor *const S,
...
void h_X_sqr_eta(spinor *const R1, spinor *const R2, spinor *const S, double const mstar) {
  ...
  if (n_cheby != x_n_cheby || rec_coefs) {
    ...
    chebyshev_coefs(
        epsilon, 1., x_cheby_coef, x_n_cheby,
        -0.5);  // coefs for f(x)=x^(-0.5) // represents P(y)=1/sqrt(y) in paper "Chiral symmetry
                // breaking an the Banks-Casher relation in lattice QCD with Wilson quarks" page 12.
    rec_coefs = 0;
    n_cheby = x_n_cheby;
  }
...
void h_X_4_eta(spinor *const R1, spinor *const R2, spinor *const S, double const mstar) {
  ...
  spinor *s_ = calloc(3 * VOLUMEPLUSRAND + 1, sizeof(spinor));
  spinor **s = calloc(3, sizeof(spinor *));
...
void X_over_sqrt_X_sqr(spinor *const R, double *const c, const int n, spinor *const S,
  ...
  spinor *sv_, *sv, *d_, *d, *dd_, *dd, *aux_, *aux, *aux3_,
      *aux3;  // *_ holds the adress of the sse-unaligned memory block
  ...
  /*input S = aux3*/
  if (0)
    assign_sub_lowest_eigenvalues(aux3, S, no_eigenvalues - 1, VOLUME);
  else
    assign(aux3, S, VOLUME);
  ...
  if (1) {
    for (j = n - 1; j >= 1; j--) {
      ...
      if (j == n - 1) {
        ...
        norm_X_sqr_psi(R, aux, mstar);  // WARNING: - maybe we have to pass this point only when
                                        // j=n-2, because R is not manipulated  in the loop body.
      ...
      assign_mul_add_mul_add_mul_add_mul_r(d, R, dd, aux3, fact2, fact1, temp1, temp2,
                                           VOLUME);  // =d_j+1
      /* dd = sv */
    ...
    /* R = d */
    if (0)
      assign_sub_lowest_eigenvalues(R, d, no_eigenvalues - 1, VOLUME);
    else
      assign(R, d, VOLUME);
...
#if (defined SSE3 || defined SSE2 || defined SSE)
  spinor *Sin_ = calloc(VOLUMEPLUSRAND + 1, sizeof(spinor));
  spinor *Sin = (spinor *)(((unsigned long int)(Sin_) + ALIGN_BASE) & ~ALIGN_BASE);
#else
  spinor *Sin = calloc(VOLUMEPLUSRAND, sizeof(spinor));
#endif

  random_spinor_field_lexic(Sin, repro, RN_GAUSS);

  spinor *s_ = calloc(4 * VOLUMEPLUSRAND + 1, sizeof(spinor));
  spinor **s = calloc(4, sizeof(spinor *));
