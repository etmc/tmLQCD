...
void rlxs_init(int level, int seed) {
  ...
  if ((INT_MAX < 2147483647) || (FLT_RADIX != 2) || (FLT_MANT_DIG < 24)) error(0);
...
void rlxs_reset(int state[]) {
  ...
  if ((INT_MAX < 2147483647) || (FLT_RADIX != 2) || (FLT_MANT_DIG < 24)) error(4);

  define_constants();

  if (state[0] != rlxs_size()) error(5);
