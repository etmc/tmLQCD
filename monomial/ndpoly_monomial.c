...
void ndpoly_derivative(const int id, hamiltonian_field_t *const hf) {
  ...
  if (g_epsbar != 0.0 || phmc_exact_poly == 0) {
    ...
    assign(g_chi_up_spinor_field[mnl->MDPolyDegree], g_chi_up_spinor_field[mnl->MDPolyDegree - 2],
           VOLUME / 2);
    assign(g_chi_dn_spinor_field[mnl->MDPolyDegree], g_chi_dn_spinor_field[mnl->MDPolyDegree - 2],
           VOLUME / 2);

    for (j = (mnl->MDPolyDegree - 1); j >= 1; j--) {
      assign(g_chi_up_spinor_field[mnl->MDPolyDegree - 1], g_chi_up_spinor_field[mnl->MDPolyDegree],
             VOLUME / 2);
      assign(g_chi_dn_spinor_field[mnl->MDPolyDegree - 1], g_chi_dn_spinor_field[mnl->MDPolyDegree],
             VOLUME / 2);

      Q_tau1_sub_const_ndpsi(
          g_chi_up_spinor_field[mnl->MDPolyDegree], g_chi_dn_spinor_field[mnl->MDPolyDegree],
          g_chi_up_spinor_field[mnl->MDPolyDegree - 1],
          g_chi_dn_spinor_field[mnl->MDPolyDegree - 1],
          mnl->MDPolyRoots[2 * mnl->MDPolyDegree - 3 - j], phmc_Cpol, phmc_invmaxev);

      /* Get the even parts of the  (j-1)th  chi_spinors */
      H_eo_tm_ndpsi(mnl->w_fields[0], mnl->w_fields[1], g_chi_up_spinor_field[j - 1],
                    g_chi_dn_spinor_field[j - 1], EO);

      /* \delta M_eo sandwitched by  chi[j-1]_e^\dagger  and  chi[2N-j]_o */
      deriv_Sb(EO, mnl->w_fields[0], g_chi_up_spinor_field[mnl->MDPolyDegree], hf,
               mnl->forcefactor); /* UP */
      deriv_Sb(EO, mnl->w_fields[1], g_chi_dn_spinor_field[mnl->MDPolyDegree], hf,
               mnl->forcefactor); /* DN */

      /* Get the even parts of the  (2N-j)-th  chi_spinors */
      H_eo_tm_ndpsi(mnl->w_fields[0], mnl->w_fields[1], g_chi_up_spinor_field[mnl->MDPolyDegree],
                    g_chi_dn_spinor_field[mnl->MDPolyDegree], EO);
    ...
    assign(g_chi_up_spinor_field[0], mnl->pf, VOLUME / 2);
    for (k = 1; k < (mnl->MDPolyDegree - 1); k++) {
      Qtm_pm_sub_const_nrm_psi(g_chi_up_spinor_field[k], g_chi_up_spinor_field[k - 1],
                               mnl->MDPolyRoots[k - 1]);
    }
    assign(g_chi_up_spinor_field[mnl->MDPolyDegree], g_chi_up_spinor_field[mnl->MDPolyDegree - 2],
           VOLUME / 2);
...
void ndpoly_heatbath(const int id, hamiltonian_field_t *const hf) {
  ...
  if (phmc_exact_poly == 0) {
    Qtm_ndpsi(g_chi_up_spinor_field[1], g_chi_dn_spinor_field[1], g_chi_up_spinor_field[0],
              g_chi_dn_spinor_field[0]);
    ...
    Qtm_ndpsi(g_chi_up_spinor_field[1], g_chi_dn_spinor_field[1], g_chi_dn_spinor_field[0],
    ...
    cg_her_nd(g_chi_up_spinor_field[1], g_chi_dn_spinor_field[1], g_chi_up_spinor_field[0],
              g_chi_dn_spinor_field[0], 1000, 1.e-16, 0, VOLUME / 2, Qtau1_P_ndpsi);
    ...
    /* solve (Q+)*(Q-)*P((Q+)*(Q-)) *x=y */
    cg_her(g_chi_up_spinor_field[1], g_chi_up_spinor_field[0], 1000, 1.e-16, 0, VOLUME / 2,
           Qtm_pm_Ptm_pm_psi);
    ...
    for (j = 1; j < 8; j++) { /* To omit corrections just set  j<1 */
      ...
      if (j % 2) { /*  Chi[j] = ( Qdag P  Ptilde ) Chi[j-1]  */
        ...
        Ptilde_ndpsi(g_chi_up_spinor_field[j - 1], g_chi_dn_spinor_field[j - 1], mnl->MDPolyCoefs,
                     mnl->MDPolyDegree, g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j],
                     &Qtm_pm_ndpsi);
...
int init_ndpoly_monomial(const int id) {
  ...
  else if (g_epsbar == 0.0 && phmc_exact_poly == 1)
    mnl->EVMaxInv = 1. / mnl->StildeMax;
  phmc_cheb_evmin = mnl->EVMin;
  phmc_invmaxev = mnl->EVMaxInv;
  phmc_cheb_evmax = 1.0;

  /* Here we prepare the less precise MD polynomial first   */
  degree_of_polynomial_nd(&mnl->MDPolyDegree, &mnl->MDPolyCoefs, mnl->EVMin, mnl->EVMax, Qsq,
                          mnl->rngrepro);
  ...
  if ((g_proc_id == 0) && (g_debug_level > 1)) {
    printf("# monomial %s approximation interval [stilde_min, stilde_max] = [%e, %e]\n", mnl->name,
           mnl->StildeMin, mnl->StildeMax);
    printf("# monomial %s degree for P = %d, epsilont = %e, normalisation = %e", mnl->name,
           mnl->MDPolyDegree - 1, mnl->EVMin, mnl->EVMaxInv);
  }
  ...
  degree_of_Ptilde(&mnl->PtildeDegree, &mnl->PtildeCoefs, mnl->EVMin, mnl->EVMax, mnl->MDPolyDegree,
