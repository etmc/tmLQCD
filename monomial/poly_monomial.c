...
void poly_derivative(const int id, hamiltonian_field_t* const hf) {
  ...
  if (mnl->even_odd_flag) {
    ...
    for (k = 0; k < degreehalf - 1; k++) {
      Qtm_pm_sub_const_nrm_psi(chi_spinor_field[k + 1], chi_spinor_field[k], mnl->MDPolyRoots[k]);
    }

    assign(chi_spinor_field[degreehalf + 1], chi_spinor_field[degreehalf - 1], VOLUME / 2);

    /* loop over monoms */
    for (j = degreehalf; j >= 1; j--) {
      assign(chi_spinor_field[degreehalf], chi_spinor_field[degreehalf + 1], VOLUME / 2);

      Qtm_pm_sub_const_nrm_psi(chi_spinor_field[degreehalf + 1], chi_spinor_field[degreehalf],
    ...
    if (mnl->MDPolyDetRatio == 1) {
       ...
       ******************************************/
      Qtm_pm_sub_const_nrm_psi(chi_spinor_field[degreehalf], chi_spinor_field[degreehalf + 1],
                               mnl->MDPolyRoots[mnl->MDPolyDegree - 1]);

      /* devide by this factor cause its multiplied again in update_fermion_momenta see comment
       * below */
      mul_r(chi_spinor_field[degreehalf], 1. / mnl->MDPolyLocNormConst * mnl->MDPolyLmax,
            chi_spinor_field[degreehalf], VOLUME / 2);
    ...
    if (g_proc_id == 0) {
      fprintf(stderr,
              "Error: PHMC for light quarks not implementeted for non even/odd preconditioning\n");
    }
...
double poly_acc(const int id, hamiltonian_field_t* const hf) {
  ...
  if (mnl->even_odd_flag) {
    ...
    for (j = 0; j < mnl->MDPolyDegree / 2; j++) {
      ...
      Qtm_pm_sub_const_nrm_psi(mnl->w_fields[1], mnl->w_fields[0], mnl->MDPolyRoots[j]);
    }
    ...
    if (compute_evs != 0) {
      no_eigenvalues = 10;
      eigenvalues(&no_eigenvalues, mnl->maxiter, eigenvalue_precision, 0 /* compute minimal evs*/,
                  0 /*dont write evecs*/, nstore, mnl->even_odd_flag);

      no_eigenvalues = 1;
      eigenvalues(&no_eigenvalues, mnl->maxiter, eigenvalue_precision, 1 /* compute maximal evs*/,
                  0 /*dont write evecs*/, nstore, mnl->even_odd_flag);
    }
...
void poly_heatbath(const int id, hamiltonian_field_t* const hf) {
  ...
  if (mnl->even_odd_flag) {
    ...
    /* solve (Q+)*(Q-)*P((Q+)*(Q-)) *x=y */
    cg_her(mnl->w_fields[0], mnl->w_fields[1], 1000, mnl->accprec, g_relative_precision_flag,
           VOLUME / 2, Qtm_pm_Ptm_pm_psi);

    /*  phi= Bdagger phi  */
    for (j = 0; j < (mnl->MDPolyDegree / 2); j++) {
      assign(mnl->w_fields[1], mnl->w_fields[0], VOLUME / 2);
      Qtm_pm_sub_const_nrm_psi(mnl->w_fields[0], mnl->w_fields[1],
    ...
    if (mnl->MDPolyDetRatio == 1) {
      ...
      mnl->iter0 = cg_her(mnl->w_fields[1], mnl->w_fields[0], mnl->maxiter, mnl->accprec,
