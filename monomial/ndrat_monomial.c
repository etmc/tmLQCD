...
void ndrat_derivative(const int id, hamiltonian_field_t * const hf) {
  ...
  if(mnl->even_odd_flag) {
    ...
    if(g_debug_level > 1 && g_proc_id == 0) {
      printf("# Entering non-eo part of %s monomial\n", mnl->name);
    }
    ...
    if(mnl->type == NDCLOVERRAT) {
	 ...
	 //!FIXME NDCLOVERRAT needs implementation w/o even-odd
         printf("Error: %s monomial derivative not implemented w/o even-odd! Aborting... \n", mnl->name);
	 exit(0);
      }
    }
	...
	// needs phmc_Cpol = 1 to work for ndrat!
	Q_tau1_sub_const_ndpsi_D_psi(mnl->w_fields[0], mnl->w_fields[1],
			      g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j], 
			      -I*mnl->rat.mu[j], 1., mnl->EVMaxInv);
...
void ndrat_heatbath(const int id, hamiltonian_field_t * const hf) {
  ...
  if(mnl->even_odd_flag){
    ...
    if(mnl->type == NDCLOVERRAT) {
      if(g_proc_id == 0) {
	 //!FIXME NDCLOVERRAT needs implementation w/o even-odd
         printf("Error: %s monomial heatbath not implemented w/o even-odd! Aborting... \n", mnl->name);
	 exit(0);
      }
    }    
    ...
    for(int j = (mnl->rat.np-1); j > -1; j--) {
      // Q_h * tau^1 - i nu_j
      // this needs phmc_Cpol = 1 to work!
      Q_tau1_sub_const_ndpsi_D_psi(g_chi_up_spinor_field[mnl->rat.np], g_chi_dn_spinor_field[mnl->rat.np],
			      g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j], 
			      I*mnl->rat.nu[j], 1., mnl->EVMaxInv);
      
      assign_add_mul(mnl->pf, g_chi_up_spinor_field[mnl->rat.np], I*mnl->rat.rnu[j], VOLUME);
      assign_add_mul(mnl->pf2, g_chi_dn_spinor_field[mnl->rat.np], I*mnl->rat.rnu[j], VOLUME);
    } 
    g_mu = g_mu_save;
  }
...
double ndrat_acc(const int id, hamiltonian_field_t * const hf) {
  ...
  if(mnl->even_odd_flag){
    ...
    if(mnl->type == NDCLOVERRAT) {
      if(g_proc_id == 0) {
	 //!FIXME NDCLOVERRAT needs implementation w/o even-odd
         printf("Error: %s monomial acceptance not implemented w/o even-odd! Aborting... \n", mnl->name);
	 exit(0);
      }
    }    
    ...
    for(int j = (mnl->rat.np-1); j > -1; j--) {
      assign_add_mul_r(mnl->w_fields[0], g_chi_up_spinor_field[j], 
		      mnl->rat.rmu[j], VOLUME);
      assign_add_mul_r(mnl->w_fields[1], g_chi_dn_spinor_field[j], 
		      mnl->rat.rmu[j], VOLUME);
    }

    mnl->energy1 = scalar_prod_r(mnl->pf, mnl->w_fields[0], VOLUME, 1);
    mnl->energy1 += scalar_prod_r(mnl->pf2, mnl->w_fields[1], VOLUME, 1);    
...
int init_ndrat_monomial(const int id) {
  ...
  if (mnl->even_odd_flag){
	...
	if(g_proc_id == 0) {
      ...
      if((mnl->type != NDRAT)&&(mnl->type != NDRATCOR)) {
	  if(g_proc_id == 0) {
	    printf("Error: %s monomial not implemented w/o even-odd! Aborting... \n", mnl->name);
	    exit(0);
	  }      
      ...
      if(init_chi_spinor_field(VOLUMEPLUSRAND, (mnl->rat.np+1)) != 0) {
	fprintf(stderr, "Not enough memory for Chi fields! Aborting...\n");
	exit(0);
      }
    }    
