...
#pragma omp parallel for
  ...
  mnl->iter1 += solve_mms_nd(g_chi_up_spinor_field, g_chi_dn_spinor_field, mnl->pf, mnl->pf2,
                             &(mnl->solver_params));
...
#pragma omp parallel for
    ...
    for (int j = 0; j < mnl->rat.np; j++) {
      if (mnl->type == NDCLOVERRAT) {
        ...
        Qsw_tau1_sub_const_ndpsi(mnl->w_fields[0], mnl->w_fields[1], g_chi_up_spinor_field[j],
                                 g_chi_dn_spinor_field[j], -I * mnl->rat.mu[j], 1., mnl->EVMaxInv);
        tm_stopwatch_pop(&g_timers, 0, 1, "");

        /* Get the even parts X_j,e */
        /* H_eo_... includes tau_1 */
        tm_stopwatch_push(&g_timers, "H_eo_sw_ndpsi", "");
        H_eo_sw_ndpsi(mnl->w_fields[2], mnl->w_fields[3], g_chi_up_spinor_field[j],
        ...
        Q_tau1_sub_const_ndpsi(mnl->w_fields[0], mnl->w_fields[1], g_chi_up_spinor_field[j],
                               g_chi_dn_spinor_field[j], -I * mnl->rat.mu[j], 1., mnl->EVMaxInv);
        tm_stopwatch_pop(&g_timers, 0, 1, "");
        ...
        H_eo_tm_ndpsi(mnl->w_fields[2], mnl->w_fields[3], g_chi_up_spinor_field[j],
                      g_chi_dn_spinor_field[j], EO);
        tm_stopwatch_pop(&g_timers, 0, 1, "");
      }
      /* X_j,e^dagger \delta M_eo Y_j,o */
      deriv_Sb(EO, mnl->w_fields[2], mnl->w_fields[0], hf, mnl->rat.rmu[j] * mnl->forcefactor);
      deriv_Sb(EO, mnl->w_fields[3], mnl->w_fields[1], hf, mnl->rat.rmu[j] * mnl->forcefactor);
      ...
      deriv_Sb(OE, g_chi_up_spinor_field[j], mnl->w_fields[4], hf,
               mnl->rat.rmu[j] * mnl->forcefactor);
      deriv_Sb(OE, g_chi_dn_spinor_field[j], mnl->w_fields[5], hf,
               mnl->rat.rmu[j] * mnl->forcefactor);

      if (mnl->type == NDCLOVERRAT) {
        // even/even sites sandwiched by tau_1 gamma_5 Y_e and gamma_5 X_e
        sw_spinor_eo(EE, mnl->w_fields[5], mnl->w_fields[2], mnl->rat.rmu[j] * mnl->forcefactor);
        // odd/odd sites sandwiched by tau_1 gamma_5 Y_o and gamma_5 X_o
        sw_spinor_eo(OO, g_chi_up_spinor_field[j], mnl->w_fields[1],
                     mnl->rat.rmu[j] * mnl->forcefactor);

        // even/even sites sandwiched by tau_1 gamma_5 Y_e and gamma_5 X_e
        sw_spinor_eo(EE, mnl->w_fields[4], mnl->w_fields[3], mnl->rat.rmu[j] * mnl->forcefactor);
        // odd/odd sites sandwiched by tau_1 gamma_5 Y_o and gamma_5 X_o
...
void ndrat_heatbath(const int id, hamiltonian_field_t *const hf) {
  ...
  mnl->solver_params.sdim = VOLUME / 2;
  mnl->solver_params.rel_prec = g_relative_precision_flag;
  mnl->iter0 = solve_mms_nd_plus(g_chi_up_spinor_field, g_chi_dn_spinor_field, mnl->pf, mnl->pf2,
                                 &(mnl->solver_params));
...
double ndrat_acc(const int id, hamiltonian_field_t *const hf) {
  ...
  mnl->solver_params.sdim = VOLUME / 2;
  mnl->solver_params.rel_prec = g_relative_precision_flag;
  mnl->iter0 += solve_mms_nd(g_chi_up_spinor_field, g_chi_dn_spinor_field, mnl->pf, mnl->pf2,
                             &(mnl->solver_params));
  ...
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    assign_add_mul_r(mnl->w_fields[0], g_chi_up_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
    assign_add_mul_r(mnl->w_fields[1], g_chi_dn_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
  }
...
int init_ndrat_monomial(const int id) {
  ...
  if (mnl->type == RAT || mnl->type == CLOVERRAT || mnl->type == RATCOR ||
