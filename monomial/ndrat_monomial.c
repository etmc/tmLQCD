...
void ndrat_derivative(const int id, hamiltonian_field_t * const hf) {
  ...
  if(mnl->type == NDCLOVERRAT) {
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "su3_zero");
  ...
  for(int j = (mnl->rat.np-1); j > -1; j--) {
    if(mnl->type == NDCLOVERRAT) {
      ...
      tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "Qsw_tau1_sub_const_ndpsi");
      ...
      H_eo_sw_ndpsi(mnl->w_fields[2], mnl->w_fields[3], 
		    g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j]);
      tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "H_eo_sw_ndpsi");
      ...
      Q_tau1_sub_const_ndpsi(mnl->w_fields[0], mnl->w_fields[1],
			     g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j], 
			     -I*mnl->rat.mu[j], 1., mnl->EVMaxInv);
      tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "Q_tau1_sub_const_ndpsi");
      ...
      tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "H_eo_tm_ndpsi");
    }
...
double ndrat_acc(const int id, hamiltonian_field_t * const hf) {
  ...
  tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "scalar_prod_r");
  if(g_proc_id == 0) {
    if(g_debug_level > 3) {
