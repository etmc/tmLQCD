...
void cloverdetratio_derivative_orig(const int no, hamiltonian_field_t *const hf) {
  ...
  if (mnl->solver == BICGSTAB && g_proc_id == 0) {
    fprintf(stderr,
            "Bicgstab currently not implemented, using CG instead! (detratio_monomial.c)\n");
  }
  ...
  /* Invert Q_{+} Q_{-} */
  /* X_W -> w_fields[1] */
  chrono_guess(mnl->w_fields[1], mnl->w_fields[2], mnl->csg_field, mnl->csg_index_array, mnl->csg_N,
...
#pragma omp parallel for
  ...
  if (mnl->solver == BICGSTAB && g_proc_id == 0) {
    fprintf(stderr,
            "Bicgstab currently not implemented, using CG instead! (cloverdetratio_monomial.c)\n");
  }
...
void cloverdetratio_heatbath(const int id, hamiltonian_field_t *const hf) {
  ...
  if (mnl->HB_solver == MG) {
    mnl->iter0 = solve_degenerate(mnl->pf, mnl->w_fields[1], mnl->HB_solver_params, mnl->HB_maxiter,
...
double cloverdetratio_acc(const int id, hamiltonian_field_t *const hf) {
  ...
  chrono_guess(mnl->w_fields[0], mnl->w_fields[1], mnl->csg_field, mnl->csg_index_array, mnl->csg_N,
               mnl->csg_n, VOLUME / 2, &Qtm_plus_psi);
