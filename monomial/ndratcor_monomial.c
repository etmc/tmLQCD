...
void check_C_ndpsi(spinor* const k_up, spinor* const k_dn, spinor* const l_up, spinor* const l_dn,
                   const int id, hamiltonian_field_t* const hf, solver_params_t* solver_params);

// applies (Q^2 R^2 -1) phi
void apply_Z_ndpsi(spinor* const k_up, spinor* const k_dn, spinor* const l_up, spinor* const l_dn,
                   const int id, hamiltonian_field_t* const hf, solver_params_t* solver_params);
...
void ndratcor_heatbath(const int id, hamiltonian_field_t* const hf) {
  ...
  double coefs[6] = {1. / 4.,      -3. / 32.,    7. / 128.,
                     -77. / 2048., 231. / 8192., -1463. / 65536.};  // series of (1+x)^(1/4)
  double coefs_check[6] = {1. / 2.,    -1. / 8.,  1. / 16.,
                           -5. / 128., 7. / 256., -21. / 1024.};  // series of (1+x)^(1/2)
  ...
  delta = coefs_check[0] * (scalar_prod_r(mnl->pf, up0, VOLUME / 2, 1) +
                            scalar_prod_r(mnl->pf2, dn0, VOLUME / 2, 1));
  tm_stopwatch_pop(&g_timers, 0, 1, "");
  ...
  if (delta * delta > mnl->accprec) {
    ...
    for (int i = 2; i < 8; i++) {
      ...
      tm_stopwatch_push(&g_timers, "scalar_prod_r", "");
      delta = coefs_check[i - 1] *
              (scalar_prod_r(Zup, up0, VOLUME / 2, 1) + scalar_prod_r(Zup, dn0, VOLUME / 2, 1));
      tm_stopwatch_pop(&g_timers, 0, 1, "");
      if (g_debug_level > 2 && g_proc_id == 0)
        printf("# NDRATCOR heatbath: c_%d*(R * Z^%d * R) = %e\n", i, i, delta);
      // debug for showing that the old check was giving a smaller delta
...
double ndratcor_acc(const int id, hamiltonian_field_t* const hf) {
  ...
  apply_Z_ndpsi(up0, dn0, mnl->pf, mnl->pf2, id, hf, &(mnl->solver_params));
  tm_stopwatch_push(&g_timers, "scalar_prod_r", "");
  delta = coefs[0] * (scalar_prod_r(mnl->pf, up0, VOLUME / 2, 1) +
                      scalar_prod_r(mnl->pf2, dn0, VOLUME / 2, 1));
  tm_stopwatch_pop(&g_timers, 0, 1, "");
  mnl->energy1 += delta;
  if (g_debug_level > 2 && g_proc_id == 0)
  ...
  for (int i = 2; i < 8; i++) {
    ...
    tm_stopwatch_push(&g_timers, "scalar_prod_r", "");
    delta = coefs[i - 1] *
            (scalar_prod_r(up0, up1, VOLUME / 2, 1) + scalar_prod_r(dn0, dn1, VOLUME / 2, 1));
    tm_stopwatch_pop(&g_timers, 0, 1, "");
    mnl->energy1 += delta;
    if (g_debug_level > 2 && g_proc_id == 0)
      printf("# NDRATCOR acc step: c_%d*(phi * Z^%d * phi) = %e\n", i, i, delta);
...
void apply_Z_ndpsi(spinor* const k_up, spinor* const k_dn, spinor* const l_up, spinor* const l_dn,
  ...
  mnl->iter0 +=
      solve_mms_nd(g_chi_up_spinor_field, g_chi_dn_spinor_field, l_up, l_dn, solver_params);

  // apply R to the pseudo-fermion fields
  assign(k_up, l_up, VOLUME / 2);
  assign(k_dn, l_dn, VOLUME / 2);
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    assign_add_mul_r(k_up, g_chi_up_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
    assign_add_mul_r(k_dn, g_chi_dn_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
  }

  // apply R a second time
  mnl->iter0 +=
      solve_mms_nd(g_chi_up_spinor_field, g_chi_dn_spinor_field, k_up, k_dn, solver_params);
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    assign_add_mul_r(k_up, g_chi_up_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
    assign_add_mul_r(k_dn, g_chi_dn_spinor_field[j], mnl->rat.rmu[j], VOLUME / 2);
  }
  mul_r(g_chi_up_spinor_field[mnl->rat.np], mnl->rat.A * mnl->rat.A, k_up, VOLUME / 2);
  mul_r(g_chi_dn_spinor_field[mnl->rat.np], mnl->rat.A * mnl->rat.A, k_dn, VOLUME / 2);
  // apply Q^2 and compute the residue
  tm_stopwatch_push(&g_timers, "M_ndpsi", "");
  solver_params->M_ndpsi(k_up, k_dn, g_chi_up_spinor_field[mnl->rat.np],
                         g_chi_dn_spinor_field[mnl->rat.np]);
  tm_stopwatch_pop(&g_timers, 0, 1, "");
...
void check_C_ndpsi(spinor* const k_up, spinor* const k_dn, spinor* const l_up, spinor* const l_dn,
  ...
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    ...
    if (mnl->type == NDCLOVERRATCOR || mnl->type == NDCLOVERRAT) {
      ...
      Qsw_tau1_sub_const_ndpsi(g_chi_up_spinor_field[mnl->rat.np],
                               g_chi_dn_spinor_field[mnl->rat.np], g_chi_up_spinor_field[j],
                               g_chi_dn_spinor_field[j], I * mnl->rat.nu[j], 1., mnl->EVMaxInv);
      tm_stopwatch_pop(&g_timers, 0, 1, "");
    } else {
      ...
      Q_tau1_sub_const_ndpsi(g_chi_up_spinor_field[mnl->rat.np], g_chi_dn_spinor_field[mnl->rat.np],
                             g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j], I * mnl->rat.nu[j],
                             1., mnl->EVMaxInv);
      tm_stopwatch_pop(&g_timers, 0, 1, "");
    }
