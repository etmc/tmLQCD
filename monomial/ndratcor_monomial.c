...
void ndratcor_heatbath(const int id, hamiltonian_field_t * const hf) {
  ...
  tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "sclar_prod_r");
  ...
  if(g_debug_level > 3) {
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "square_norm");
    if(g_proc_id == 0) {
...
void apply_Z_ndpsi(spinor * const k_up, spinor * const k_dn,
  ...
  tm_stopwatch_pop(&g_timers, 0, 1, current_mnl, "M_ndpsi");
