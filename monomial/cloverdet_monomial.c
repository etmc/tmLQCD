...
#pragma omp parallel for
  ...
  mnl->iter1 +=
      solve_degenerate(mnl->w_fields[1], mnl->pf, mnl->solver_params, mnl->maxiter, mnl->forceprec,
...
void cloverdet_heatbath(const int id, hamiltonian_field_t *const hf) {
  ...
  chrono_add_solution(mnl->pf, mnl->csg_field, mnl->csg_index_array, mnl->csg_N, &mnl->csg_n, N);
...
double cloverdet_acc(const int id, hamiltonian_field_t *const hf) {
  ...
  if (mnl->solver == MG || mnl->solver == BICGSTAB) {
    chrono_guess(mnl->w_fields[1], mnl->pf, mnl->csg_field, mnl->csg_index_array, mnl->csg_N,
                 mnl->csg_n, N, mnl->Qp);
    mnl->iter0 +=
        solve_degenerate(mnl->w_fields[0], mnl->pf, mnl->solver_params, mnl->maxiter, mnl->accprec,
