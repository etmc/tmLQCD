...
void check_C_psi(spinor* const k_up, spinor* const l_up, const int id,
                 hamiltonian_field_t* const hf, solver_params_t* solver_params);

// applies (Q^2 R^2 -1) phi
double apply_Z_psi(spinor* const k_up, spinor* const l_up, const int id,
                   hamiltonian_field_t* const hf, solver_params_t* solver_params);
...
double apply_Z_psi(spinor* const k_up, spinor* const l_up, const int id,
  ...
  mnl->iter0 += solve_mms_tm(g_chi_up_spinor_field, k_up, solver_params);
  ...
  // apply Q^2
  tm_stopwatch_push(&g_timers, "M_psi", "");
...
void check_C_psi(spinor* const k_up, spinor* const l_up, const int id,
  ...
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    if (mnl->type == CLOVERRATCOR || mnl->type == CLOVERRAT) {
      ...
      // Q_tau1_sub_const_ndpsi(g_chi_up_spinor_field[mnl->rat.np],
      // g_chi_dn_spinor_field[mnl->rat.np], 			     g_chi_up_spinor_field[j], g_chi_dn_spinor_field[j],
      //		     I*mnl->rat.nu[j], 1., mnl->EVMaxInv);
    }
