...
void detratio_derivative(const int no, hamiltonian_field_t * const hf) {
  ...
  if(mnl->even_odd_flag) {
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, __func__, "Qtm_plus_psi");
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, __func__, "Qtm_minus_psi");
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, __func__, "Q_plus_psi");
...
void detratio_heatbath(const int id, hamiltonian_field_t * const hf) {
  ...
  if(mnl->even_odd_flag) {
    ...
    if((mnl->solver == CG) || (mnl->solver == MIXEDCG) || (mnl->solver == RGMIXEDCG)){
      ...
      tm_stopwatch_pop(&g_timers, 0, 1, __func__, "Q_minus_psi");
      chrono_add_solution(mnl->pf, mnl->csg_field, mnl->csg_index_array,
			  mnl->csg_N, &mnl->csg_n, VOLUME/2);
