...
#pragma omp parallel for
  ...
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    ...
    if (mnl->type == CLOVERRAT) {
      ...
      deriv_Sb(OE, mnl->w_fields[0], mnl->w_fields[2], hf, mnl->rat.rmu[j] * mnl->forcefactor);
      ...
      sw_spinor_eo(OO, mnl->w_fields[0], g_chi_up_spinor_field[j],
                   mnl->rat.rmu[j] * mnl->forcefactor);
...
void rat_heatbath(const int id, hamiltonian_field_t* const hf) {
  ...
  mnl->iter0 = solve_mms_tm(g_chi_up_spinor_field, mnl->pf, &(mnl->solver_params));
  ...
  for (int j = (mnl->rat.np - 1); j > -1; j--) {
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, "");
    assign_add_mul(g_chi_up_spinor_field[mnl->rat.np], g_chi_up_spinor_field[j],
                   -I * mnl->rat.nu[j], VOLUME / 2);
