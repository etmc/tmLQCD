...
int init_monomials(const int V, const int even_odd_flag) {
  ...
  for(int i = 0; i < no_monomials; i++) {
	...
	if(g_proc_id == 0 && g_debug_level > 1) {
      ...
      else if(monomial_list[i].type == CLOVERDETRATIORWD) {
	monomial_list[i].accfunction = &cloverdetratio_rwaccd;
	monomial_list[i].even_odd_flag = 1;
	monomial_list[i].Qsq = &Qsw_pm_psi;
	monomial_list[i].Qp = &Qsw_plus_psi;
	monomial_list[i].Qm = &Qsw_minus_psi;
	init_swpm(VOLUME);
	if(g_proc_id == 0 && g_debug_level > 1) {
	  printf("# Initialised monomial of type CLOVERDETRATIORWD, no_monomials= %d, currently only available for reweighting!\n", no_monomials);
	}
      }
