...
int add_monomial(const int type) {
  ...
  monomial_list[no_monomials].solver_params.sloppy_precision =
      _default_operator_sloppy_precision_flag;
  ...
  monomial_list[no_monomials].solver_params.refinement_precision =
      _default_operator_sloppy_precision_flag;
...
int init_monomials(const int V, const int even_odd_flag) {
  ...
  for (int i = 0; i < no_monomials; i++) {
    ...
    if ((monomial_list[i].type == NDPOLY) || (monomial_list[i].type == NDDETRATIO) ||
        (monomial_list[i].type == NDCLOVERDETRATIO) || (monomial_list[i].type == NDCLOVER) ||
        (monomial_list[i].type == NDRAT) || (monomial_list[i].type == NDRATCOR) ||
        (monomial_list[i].type == NDCLOVERRATCOR) || (monomial_list[i].type == NDCLOVERRAT))
      no++;
  }
  if (no_monomials > 0) {
      ...
      if (monomial_list[i].type == DET) {
        ...
        if (monomial_list[i].external_library == QUDA_LIB) {
          if (monomial_list[i].solver_params.external_inverter != QUDA_INVERTER) {
            tm_debug_printf(0, 0,
                            "Error: CLOVERDET monomial with UseExternalLibrary = quda is not "
                            "supported without UseExternalInverter = quda\n");
            exit(1);
          }
        }
        ...
        if (g_proc_id == 0 && g_debug_level > 1) {
          printf("# Initialised monomial %s of type CLOVERDET, no_monomials= %d\n",
                 monomial_list[i].name, no_monomials);
        }
        ...
        if (monomial_list[i].external_library == QUDA_LIB) {
          if (monomial_list[i].solver_params.external_inverter != QUDA_INVERTER) {
            tm_debug_printf(0, 0,
                            "Error: CLOVERDETRATIO monomial with UseExternalLibrary = quda is not "
                            "supported without UseExternalInverter = quda\n");
            exit(1);
          }
        }
        ...
        if (g_proc_id == 0 && g_debug_level > 1) {
          printf(
              "# Initialised monomial %s of type CLOVERDETRATIORW, no_monomials= %d."
        ...
        if (monomial_list[i].external_library == QUDA_LIB) {
          if (monomial_list[i].solver_params.external_inverter != QUDA_INVERTER) {
            tm_debug_printf(0, 0,
                            "Error: NDCLOVERRAT monomial with UseExternalLibrary = quda is not "
                            "supported without UseExternalInverter = quda\n");
            exit(1);
          }
        }
        ...
        if (g_proc_id == 0 && g_debug_level > 1) {
          printf(
              "# Initialised monomial %s of type NDDETRATIO, no_monomials= %d."
              " Currently only available for reweighting!\n",
              monomial_list[i].name, no_monomials);
        }
        ...
        if (g_proc_id == 0 && g_debug_level > 1) {
          printf(
              "# Initialised monomial %s of type NDCLOVERDETRATIO, no_monomials= %d."
              " Currently only available for reweighting!\n",
              monomial_list[i].name, no_monomials);
        }
      }
...
int init_poly_monomial(const int V, const int id) {
  ...
  if ((void*)(mnl->MDPoly_chi_spinor_fields =
                  (spinor**)calloc(mnl->MDPolyDegree / 2 + 2, sizeof(spinor*))) == NULL) {
    printf("malloc errno in init_poly_monomial pf fields: %d\n", errno);
    errno = 0;
    return (2);
  }

  (mnl->MDPoly_chi_spinor_fields)[0] =
      (spinor*)(((unsigned long int)(_pf) + ALIGN_BASE) & ~ALIGN_BASE);
  ...
  if (strlen(monomial_list[id].MDPolyRootsFile) == 0) {
    sprintf(monomial_list[id].MDPolyRootsFile, "%s_deg_%d_eps_%1.16e.roots", "1overX_poly",
  ...
  if (monomial_list[id].MDPolyLocNormConst == -1.0) {
    ...
    sprintf(filename, "%s_deg_%d_eps_%1.16e.const", "1overX_poly", monomial_list[id].MDPolyDegree,
            eps);
  ...
  if ((void*)(mnl->MDPolyRoots =
                  (_Complex double*)calloc(mnl->MDPolyDegree, sizeof(_Complex double))) == NULL) {
