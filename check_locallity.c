  ...
  for (j = 0; j < Nmeas; j++) {
    ...
    if (even_odd_flag) {
      i = invert_eo(g_spinor_field[2], g_spinor_field[3], g_spinor_field[0], g_spinor_field[1],
                    ...
                    sub_evs_cg_flag, even_odd_flag, 0, NULL, -1, NO_EXT_INV, SLOPPY_DOUBLE,
                    NO_COMPRESSION);
      ...
      for (y = 0; y < LY; y++) {
        ...
        for (z = 0; z < LZ; z++) {
          ...
          for (t = 0; t < T; t++) {
            ...
            printf("%e %e\n", creal(g_spinor_field[DUM_DERI + 1][g_ipt[t][x][y][z]].s0.c0),
                   cimag(g_spinor_field[DUM_DERI + 1][g_ipt[t][x][y][z]].s0.c0));
            nrm = sqrt(nrm);
            printf("%1.12e\n", nrm);
