...
void HOPPING_ASYNC (dev_su3_2v * gf, 
  ...
  /*
  if ( (VolumeEO-4*tSliceEO) % blocksize == 0 ) {
    gridsize1  = (VolumeEO-4*tSliceEO) / blocksize;
  }
  else {
    gridsize1  = (int) ( ((VolumeEO-4*tSliceEO)/blocksize) + 1);
  }
  
  if ( (2*tSliceEO) % blocksize == 0 ) {
    gridsize2  = (2*tSliceEO) / blocksize;
  }
  else {
    gridsize2  = (int) ( ((2*tSliceEO)/blocksize) + 1);
  }
  */
  		  ...
  		  //   if (nStreams%2 != 0 ) nStreams = nStreams + 1;
  		  // int nStreams = 2;
  		  /*
  		  dev_spinor * RAND1;
  		  dev_spinor * RAND2;
  		  dev_spinor * RAND3;	// page-locked
  		  dev_spinor * RAND4;
  		  ...
  		  */
  		  // int offset = tSliceEO / (nStreams/2);
  		#endif
	...
	// applies to the parts which don't need communication
  	...
  	dev_Hopping_Matrix_ASYNC <<<gridsize2, blocksize>>> ( gf,
  		...
  		cudaStreamSynchronize(stream[1]);
  				...
  				//cudaEventElapsedTime(&comm_time1, comm_start1, comm_stop1);
  		...
  		for (int i = 0; i < nStreams; i++) {
  		  ...
  		  MPI_Recv (RAND3+6*i*offset, 24*offset, MPI_FLOAT, g_nb_t_up, i,
  		            g_cart_grid, &stat[i]);
  		...
  		cudaStreamSynchronize(stream[nStreams+1]);
  				...
  				//cudaEventElapsedTime(&comm_time2, comm_start2, comm_stop2);
  		...
  		for (int i = 0; i < nStreams; i++) {
  		  ...
  		  MPI_Recv (RAND4+6*i*offset, 24*offset, MPI_FLOAT, g_nb_t_dn, nStreams+i,
  		            g_cart_grid, &stat[nStreams+i]);
