...
void xchange_deri(su3adj** const df) {
  ...
  MPI_Sendrecv(&df[gI_m1_0_0_0][0].d1, 1, deri_time_slice_cont, g_nb_t_dn, 43,
               &ddummy[gI_Lm1_0_0_0][0].d1, 1, deri_time_slice_cont, g_nb_t_up, 43, g_cart_grid,
               &status);
  ...
  MPI_Sendrecv(&df[gI_0_m1_0_0][0], 1, deri_x_slice_cont, g_nb_x_dn, 44, &ddummy[gI_0_Lm1_0_0][0],
               1, deri_x_slice_gath, g_nb_x_up, 44, g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (y = 0; y < LY; y++) {
      for (z = 0; z < LZ; z++) {
  ...
  /* send the data to the neighbour on the right is needed for the */
  /* clover case, so this needs fixing here! */
#endif /* (defined PARALLELXT || defined PARALLELXYT || defined PARALLELXYZT || defined PARALLELX \
          || defined PARALLELXY || defined PARALLELXYZ ) */

#if (defined PARALLELXYT || defined PARALLELXYZT || defined PARALLELXY || defined PARALLELXYZ)
  /* send the data to the neighbour on the left in y direction */
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Sendrecv((void*)df[gI_0_0_m1_0], 1, deri_y_slice_cont, g_nb_y_dn, 45,
               (void*)ddummy[gI_0_0_Lm1_0], 1, deri_y_slice_gath, g_nb_y_up, 45, g_cart_grid,
               &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
  ...
  /* send the data to the neighbour on the right is needed for the */
  /* clover case, so this needs fixing here! */
#endif /* (defined PARALLELXYT || defined PARALLELXYZT || defined PARALLELXY || defined \
          PARALLELXYZ ) */

#if (defined PARALLELXYZT || defined PARALLELXYZ)
  /* send the data to the neighbour on the left in z direction */
  /* recieve the data from the neighbour on the right in z direction */
  MPI_Sendrecv((void*)df[gI_0_0_0_m1], 1, deri_z_slice_cont, g_nb_z_dn, 46,
               (void*)ddummy[gI_0_0_0_Lm1], 1, deri_z_slice_gath, g_nb_z_up, 46, g_cart_grid,
               &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
...
void xchange_deri(su3adj** const df) {
  ...
  MPI_Sendrecv((void*)df[VOLUME + RAND + 2 * LY * LZ], 1, deri_xt_edge_cont, g_nb_t_dn, 492,
               (void*)ddummy[0], 1, deri_xt_edge_cont, g_nb_t_up, 492, g_cart_grid, &status);
  ...
  /* xt-edge */
  MPI_Sendrecv((void*)df[VOLUME + RAND], 1, deri_xt_edge_cont, g_nb_t_up, 493, (void*)ddummy[0], 1,
               deri_xt_edge_cont, g_nb_t_dn, 493, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LZ * LY + 2 * T * LZ], 1, deri_yx_edge_cont, g_nb_x_dn,
               494, (void*)ddummy[0], 1, deri_yx_edge_cont, g_nb_x_up, 494, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LZ * LY], 1, deri_yx_edge_cont, g_nb_x_up, 495,
               (void*)ddummy[0], 1, deri_yx_edge_cont, g_nb_x_dn, 495, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 2 * LX * LZ], 1,
               deri_ty_edge_cont, g_nb_y_dn, 496, (void*)ddummy[0], 1, deri_ty_edge_cont, g_nb_y_up,
               496, g_cart_grid, &status);
  ...
  /* ty-edge */
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ], 1, deri_ty_edge_cont, g_nb_y_up,
               497, (void*)ddummy[0], 1, deri_ty_edge_cont, g_nb_y_dn, 497, g_cart_grid, &status);
  ...
  /* recieve the data from the neighbour on the right in x direction */
  /* xz-edge */
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ + 2 * T * LY], 1,
               deri_zx_edge_cont, g_nb_x_dn, 498, (void*)ddummy[0], 1, deri_zx_edge_cont, g_nb_x_up,
               498, g_cart_grid, &status);
  ...
  /* recieve the data from the neighbour on the left in x direction */
  /* xz-edge */
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ], 1,
               deri_zx_edge_cont, g_nb_x_up, 499, (void*)ddummy[0], 1, deri_zx_edge_cont, g_nb_x_dn,
               499, g_cart_grid, &status);
  ...
  /* tz-edge */
  MPI_Sendrecv(
      (void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ + 4 * T * LY + 2 * LX * LY],
      1, deri_tz_edge_cont, g_nb_z_dn, 500, (void*)ddummy[0], 1, deri_tz_edge_cont, g_nb_z_up, 500,
      g_cart_grid, &status);
  ...
  /* tz-edge */
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ + 4 * T * LY], 1,
               deri_tz_edge_cont, g_nb_z_up, 501, (void*)ddummy[0], 1, deri_tz_edge_cont, g_nb_z_dn,
               501, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ + 4 * T * LY +
                         4 * LX * LY + 2 * T * LX],
               1, deri_zy_edge_cont, g_nb_y_dn, 502, (void*)ddummy[0], 1, deri_zy_edge_cont,
               g_nb_y_up, 502, g_cart_grid, &status);
  ...
  /* zy-edge */
  MPI_Sendrecv(
      (void*)df[VOLUME + RAND + 4 * LY * LZ + 4 * T * LZ + 4 * LX * LZ + 4 * T * LY + 4 * LX * LY],
      1, deri_zy_edge_cont, g_nb_y_up, 503, (void*)ddummy[0], 1, deri_zy_edge_cont, g_nb_y_dn, 503,
      g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[(T + 1) * LX * LY * LZ], 1, deri_time_slice_cont, g_nb_t_dn, 40,
               (void*)ddummy[0], 1, deri_time_slice_cont, g_nb_t_up, 40, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[T * LX * LY * LZ], 1, deri_time_slice_cont, g_nb_t_up, 41,
               (void*)ddummy[0], 1, deri_time_slice_cont, g_nb_t_dn, 41, g_cart_grid, &status);
  ...
  MPI_Sendrecv((void*)df[(T + 2) * LX * LY * LZ + T * LY * LZ], 1, deri_x_slice_cont, g_nb_x_dn, 42,
               (void*)ddummy[0], 1, deri_x_slice_cont, g_nb_x_up, 42, g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (y = 0; y < LY; y++) {
      for (z = 0; z < LZ; z++) {
  ...
  MPI_Sendrecv((void*)df[(T + 2) * LX * LY * LZ], 1, deri_x_slice_cont, g_nb_x_up, 43,
               (void*)ddummy[0], 1, deri_x_slice_cont, g_nb_x_dn, 43, g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (y = 0; y < LY; y++) {
      for (z = 0; z < LZ; z++) {
  ...
  /* send the data to the neighbour on the left in y direction */
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Sendrecv((void*)df[VOLUME + 2 * LZ * (LX * LY + T * LY) + T * LX * LZ], 1, deri_y_slice_cont,
               g_nb_y_dn, 44, (void*)ddummy[0], 1, deri_y_slice_cont, g_nb_y_up, 44, g_cart_grid,
               &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
  ...
  MPI_Sendrecv((void*)df[VOLUME + 2 * LZ * (LX * LY + T * LY)], 1, deri_y_slice_cont, g_nb_y_up, 45,
               (void*)ddummy[0], 1, deri_y_slice_cont, g_nb_y_dn, 45, g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
      for (z = 0; z < LZ; z++) {
  ...
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Sendrecv(
      (void*)df[VOLUME + 2 * LX * LY * LZ + 2 * T * LY * LZ + 2 * T * LX * LZ + T * LX * LY], 1,
      deri_z_slice_cont, g_nb_z_dn, 46, (void*)ddummy[0], 1, deri_z_slice_cont, g_nb_z_up, 46,
      g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
  ...
  MPI_Sendrecv((void*)df[VOLUME + 2 * LX * LY * LZ + 2 * T * LY * LZ + 2 * T * LX * LZ], 1,
               deri_z_slice_cont, g_nb_z_up, 47, (void*)ddummy[0], 1, deri_z_slice_cont, g_nb_z_dn,
               47, g_cart_grid, &status);
  /* add ddummy to df */
  for (t = 0; t < T; t++) {
    for (x = 0; x < LX; x++) {
      for (y = 0; y < LY; y++) {
