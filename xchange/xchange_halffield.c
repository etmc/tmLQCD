...
void init_xchange_halffield() {
  ...
  MPI_Send_init((void*)(sendBuffer + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 91,
                g_cart_grid, &prequests[4]);

  MPI_Recv_init((void*)(recvBuffer + LX * LY * LZ + T * LY * LZ / 2), T * LY * LZ * 12 / 2,
                MPI_DOUBLE, g_nb_x_dn, 91, g_cart_grid, &prequests[5]);

  /* send the data to the neighbour on the left in x direction */
  /* recieve the data from the neighbour on the right in x direction */
  MPI_Send_init((void*)(sendBuffer + LX * LY * LZ + T * LY * LZ / 2), T * LY * LZ * 12 / 2,
                MPI_DOUBLE, g_nb_x_dn, 92, g_cart_grid, &prequests[6]);

  MPI_Recv_init((void*)(recvBuffer + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 92,
  ...
  MPI_Recv_init((void*)(recvBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
                T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_dn, 101, g_cart_grid, &prequests[9]);

  /* send the data to the neighbour on the leftt in y direction */
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Send_init((void*)(sendBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
                T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_dn, 102, g_cart_grid, &prequests[10]);
...
#pragma pomp inst begin(xchangehalf)
  ...
  /* send the data to the neighbour on the right in t direction */
  /* recieve the data from the neighbour on the left in t direction */
  MPI_Isend((void*)(sendBuffer + g_HS_shift_t), LX * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_t_up, 81,
            g_cart_grid, &requests[0]);
  MPI_Irecv((void*)(recvBuffer + g_HS_shift_t + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2,
            MPI_DOUBLE, g_nb_t_dn, 81, g_cart_grid, &requests[1]);
  /* send the data to the neighbour on the left in t direction */
  /* recieve the data from the neighbour on the right in t direction */
  MPI_Isend((void*)(sendBuffer + g_HS_shift_t + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2,
            MPI_DOUBLE, g_nb_t_dn, 82, g_cart_grid, &requests[2]);
  MPI_Irecv((void*)(recvBuffer + g_HS_shift_t), LX * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_t_up, 82,
  ...
  /* send the data to the neighbour on the right in x direction */
  /* recieve the data from the neighbour on the left in x direction */
  MPI_Isend((void*)(sendBuffer + g_HS_shift_x), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 91,
  ...
  MPI_Irecv((void*)(recvBuffer + g_HS_shift_x), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 92,
            g_cart_grid, &requests[7]);
#endif
#if (defined PARALLELXYT || defined PARALLELXYZT || defined PARALLELXY || defined PARALLELXYZ)
  /* send the data to the neighbour on the right in y direction */
  /* recieve the data from the neighbour on the left in y direction */
  MPI_Isend((void*)(sendBuffer + g_HS_shift_y), T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_up, 101,
            g_cart_grid, &requests[8]);
  ...
  MPI_Irecv((void*)(recvBuffer + g_HS_shift_y), T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_up, 102,
            g_cart_grid, &requests[11]);
#endif
#if (defined PARALLELXYZT || defined PARALLELXYZ)
  /* send the data to the neighbour on the right in z direction */
  /* recieve the data from the neighbour on the left in z direction */
  MPI_Isend((void*)(sendBuffer + g_HS_shift_z), T * LX * LY * 12 / 2, MPI_DOUBLE, g_nb_z_up, 503,
            g_cart_grid, &requests[12]);
  ...
  MPI_Irecv((void*)(recvBuffer + g_HS_shift_z), T * LX * LY * 12 / 2, MPI_DOUBLE, g_nb_z_up, 504,
            g_cart_grid, &requests[15]);
#endif
...
#pragma pomp inst begin(xchangehalf)
  ...
  MPI_Isend((void*)(sendBuffer), LX * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_t_up, 81, g_cart_grid,
            &requests[0]);
  MPI_Irecv((void*)(recvBuffer + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_t_dn,
            81, g_cart_grid, &requests[1]);

  /* send the data to the neighbour on the left in t direction */
  /* recieve the data from the neighbour on the right in t direction */
  MPI_Isend((void*)(sendBuffer + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_t_dn,
            82, g_cart_grid, &requests[2]);
  ...
  /* send the data to the neighbour on the right in x direction */
  /* recieve the data from the neighbour on the left in x direction */
  MPI_Isend((void*)(sendBuffer + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 91,
            g_cart_grid, &requests[4]);
  ...
  MPI_Isend((void*)(sendBuffer + LX * LY * LZ + T * LY * LZ / 2), T * LY * LZ * 12 / 2, MPI_DOUBLE,
            g_nb_x_dn, 92, g_cart_grid, &requests[6]);
  MPI_Irecv((void*)(recvBuffer + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_DOUBLE, g_nb_x_up, 92,
            g_cart_grid, &requests[7]);
#endif
  ...
  MPI_Irecv((void*)(recvBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
            T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_dn, 101, g_cart_grid, &requests[9]);

  /* send the data to the neighbour on the leftt in y direction */
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Isend((void*)(sendBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
            T * LX * LZ * 12 / 2, MPI_DOUBLE, g_nb_y_dn, 102, g_cart_grid, &requests[10]);
  ...
  MPI_Isend((void*)(sendBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ), T * LX * LY * 12 / 2,
            MPI_DOUBLE, g_nb_z_up, 503, g_cart_grid, &requests[12]);
  ...
  MPI_Irecv((void*)(recvBuffer + LX * LY * LZ + T * LY * LZ + T * LX * LZ), T * LX * LY * 12 / 2,
            MPI_DOUBLE, g_nb_z_up, 504, g_cart_grid, &requests[15]);
#endif
...
#pragma pomp inst begin(xchangehalf32)
  ...
  /* send the data to the neighbour on the right in t direction */
  /* recieve the data from the neighbour on the left in t direction */
  MPI_Isend((void*)(sendBuffer32), LX * LY * LZ * 12 / 2, MPI_FLOAT, g_nb_t_up, 81, g_cart_grid,
            &requests[0]);
  MPI_Irecv((void*)(recvBuffer32 + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2, MPI_FLOAT, g_nb_t_dn,
            81, g_cart_grid, &requests[1]);

  /* send the data to the neighbour on the left in t direction */
  /* recieve the data from the neighbour on the right in t direction */
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ / 2), LX * LY * LZ * 12 / 2, MPI_FLOAT, g_nb_t_dn,
            82, g_cart_grid, &requests[2]);
  ...
  /* send the data to the neighbour on the right in x direction */
  /* recieve the data from the neighbour on the left in x direction */
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_FLOAT, g_nb_x_up, 91,
            g_cart_grid, &requests[4]);
  ...
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ + T * LY * LZ / 2), T * LY * LZ * 12 / 2, MPI_FLOAT,
            g_nb_x_dn, 92, g_cart_grid, &requests[6]);
  MPI_Irecv((void*)(recvBuffer32 + LX * LY * LZ), T * LY * LZ * 12 / 2, MPI_FLOAT, g_nb_x_up, 92,
            g_cart_grid, &requests[7]);
#endif
  ...
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ + T * LY * LZ), T * LX * LZ * 12 / 2, MPI_FLOAT,
            g_nb_y_up, 101, g_cart_grid, &requests[8]);
  MPI_Irecv((void*)(recvBuffer32 + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
            T * LX * LZ * 12 / 2, MPI_FLOAT, g_nb_y_dn, 101, g_cart_grid, &requests[9]);

  /* send the data to the neighbour on the leftt in y direction */
  /* recieve the data from the neighbour on the right in y direction */
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ + T * LY * LZ + T * LX * LZ / 2),
            T * LX * LZ * 12 / 2, MPI_FLOAT, g_nb_y_dn, 102, g_cart_grid, &requests[10]);
  ...
  MPI_Isend((void*)(sendBuffer32 + LX * LY * LZ + T * LY * LZ + T * LX * LZ), T * LX * LY * 12 / 2,
            MPI_FLOAT, g_nb_z_up, 503, g_cart_grid, &requests[12]);
  ...
  MPI_Irecv((void*)(recvBuffer32 + LX * LY * LZ + T * LY * LZ + T * LX * LZ), T * LX * LY * 12 / 2,
            MPI_FLOAT, g_nb_z_up, 504, g_cart_grid, &requests[15]);
#endif
