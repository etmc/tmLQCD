 ...
 * value */
typedef enum tm_ExitCode_t { TM_EXIT_SUCCESS = 0, TM_EXIT_INVALID_CMDLINE_ARG = 166 } tm_ExitCode_t;

/* enumeration type for the sloppy prec. of the inverter */
typedef enum SloppyPrecision_s { SLOPPY_DOUBLE = 0, SLOPPY_SINGLE, SLOPPY_HALF } SloppyPrecision;
...
typedef enum ExternalInverter_s { NO_EXT_INV = 0, QUDA_INVERTER, QPHIX_INVERTER } ExternalInverter;

/* enumeration type for the external eigensolver */
typedef enum ExternalEigSolver_s { NO_EXT_EIGSOLVER = 0, QUDA_EIGSOLVER } ExternalEigSolver;

/* enumeration type for the external inverter */
typedef enum ExternalLibrary_s { NO_EXT_LIB = 0, QUDA_LIB } ExternalLibrary;

typedef enum backup_restore_t { TM_BACKUP_GLOBALS = 0, TM_RESTORE_GLOBALS } backup_restore_t;

typedef enum real_imag_t { TM_REAL = 0, TM_IMAG } real_imag_t;
...
static inline void update_tm_gauge_id(tm_GaugeState_t* gauge_state, const double step) {
  if (gauge_state->loaded) {
    ...
    tm_debug_printf(0, 4, "# [update_tm_gauge_id]: gauge id of %s stepped by %.12f to %.12f\n",
                    gauge_state->name, step, gauge_state->gauge_id);
  } else {
    gauge_state->loaded = 1;
    gauge_state->gauge_id = 0;
    gauge_state->halo_state.exchanged = 0;
    tm_debug_printf(0, 4, "# [update_tm_gauge_id]: gauge id of %s reset to 0\n", gauge_state->name);
  }
}
...
static inline void update_tm_gauge_exchange(tm_GaugeState_t* gauge_state) {
  if (gauge_state->loaded) {
    ...
    tm_debug_printf(0, 4, "# [update_tm_gauge_exchange]: %s tagged as freshly exchanged\n",
                    gauge_state->name);
  }
}
