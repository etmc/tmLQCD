			  ...
			  for (z = 0; z < dZ; z++) {
				  ...
				  for(bt = 0; bt < nblks_t; bt++)
					  for(bx = 0; bx < nblks_x; bx++)
						  for(by = 0; by < nblks_y; by++)
							  for(bz = 0; bz < nblks_z; bz++){
							     _spinor_assign(*(rec_field + index_a(dT*bt + t, dX*bx + x, dY*by + y, dZ*bz + z)), *(psi[block_id] + ctr_t))
                                 block_id++;
							  }
				  ctr_t++;
			  }
			  ...
			  for (z = 0; z < dZ; z++) {
				  ...
				  for(bt = 0; bt < nblks_t; bt++)
					  for(bx = 0; bx < nblks_x; bx++)
						  for(by = 0; by < nblks_y; by++)
							  for(bz = 0; bz < nblks_z; bz++){
							     _spinor_assign(*(rec_field + index_a(dT*bt + t, dX*bx + x, dY*by + y, dZ*bz + z)), *(block_list[block_id].basis[id] + ctr_t))
                                 block_id++;
							  }
				  ctr_t++;
			  }
