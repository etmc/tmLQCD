    ...
    for (int i = 0; i < no_operators; i++) {
      ...
      if (optr->type == TMWILSON || optr->type == WILSON) {
        ...
        if (optr->solver == INCREIGCG) {
          if (g_cart_id == 0 && optr->DownProp) {
            fprintf(stderr,
                    "Warning: When even-odd preconditioning is used, the eigenvalues for +mu and "
                    "-mu will be little different\n");
            fprintf(stderr, "Incremental EigCG solver will still work however.\n");
          }
          if (g_cart_id == 0 && optr->even_odd_flag == 0)
            fprintf(
                stderr,
                "Incremental EigCG solver is added only with Even-Odd preconditioning!. Forcing\n");
      ...
      if (optr->external_inverter == QUDA_INVERTER) {
        ...
        if (g_proc_id == 0) {
          fprintf(stderr,
                  "Error: You're trying to use QUDA but this build was not configured for QUDA "
                  "usage.\n");
          exit(-2);
        }
#endif
...
void op_invert(const int op_id, const int index_start, const int write_prop) {
  ...
  if (optr->type == TMWILSON || optr->type == WILSON || optr->type == CLOVER) {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (optr->type != CLOVER) {
        ...
        optr->iterations =
            invert_eo(optr->prop0, optr->prop1, optr->sr0, optr->sr1, optr->eps_sq, optr->maxiter,
                      optr->solver, optr->rel_prec, 0, optr->even_odd_flag, optr->no_extra_masses,
        ...
        optr->iterations = invert_clover_eo(
            optr->prop0, optr->prop1, optr->sr0, optr->sr1, optr->eps_sq, optr->maxiter,
            optr->solver, optr->rel_prec, optr->even_odd_flag, optr->solver_params, &g_gauge_field,
            optr->applyQsq, optr->applyQm, optr->external_inverter, optr->sloppy_precision,
            optr->compression_type);
        /* check result */
        optr->applyM(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1], optr->prop0,
                     optr->prop1);
      }
    ...
    for (i = 0; i < SourceInfo.no_flavours; i++) {
      if (optr->type != DBCLOVER) {
        optr->iterations = invert_doublet_eo(
            optr->prop0, optr->prop1, optr->prop2, optr->prop3, optr->sr0, optr->sr1, optr->sr2,
            optr->sr3, optr->eps_sq, optr->maxiter, optr->solver, optr->rel_prec,
            optr->even_odd_flag, optr->solver_params, optr->external_inverter,
            optr->sloppy_precision, optr->compression_type);
        // checking solution
        M_full_ndpsi(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI + 2],
                     g_spinor_field[DUM_DERI + 3], g_spinor_field[DUM_DERI + 4], optr->prop0,
                     optr->prop1, optr->prop2, optr->prop3);
      } else {
        optr->iterations = invert_cloverdoublet_eo(
            optr->prop0, optr->prop1, optr->prop2, optr->prop3, optr->sr0, optr->sr1, optr->sr2,
            optr->sr3, optr->eps_sq, optr->maxiter, optr->solver, optr->rel_prec,
            optr->even_odd_flag, optr->solver_params, optr->external_inverter,
            optr->sloppy_precision, optr->compression_type);
      ...
      if (i == 0 && SourceInfo.no_flavours == 2 && SourceInfo.type != SRC_TYPE_VOL) {
        ...
        mul_one_pm_itau2(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 2], optr->sr0,
                         optr->sr2, -1., VOLUME / 2);
        mul_one_pm_itau2(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI + 3], optr->sr1,
                         optr->sr3, -1., VOLUME / 2);

        mul_one_pm_itau2(optr->sr0, optr->sr2, g_spinor_field[DUM_DERI + 2],
                         g_spinor_field[DUM_DERI], +1., VOLUME / 2);
        mul_one_pm_itau2(optr->sr1, optr->sr3, g_spinor_field[DUM_DERI + 3],
                         g_spinor_field[DUM_DERI + 1], +1., VOLUME / 2);
      }
    ...
    g_mu = 0.;
    m_ov = optr->m;
    eigenvalues(&optr->no_ev, 5000, optr->ev_prec, 0, optr->ev_readwrite, nstore,
                optr->even_odd_flag);
    /*     ov_check_locality(); */
  ...
  if (g_strict_residual_check) {
    double rel_nrm =
        optr->rel_prec
            ? (square_norm(optr->sr0, VOLUME / 2, 1) + square_norm(optr->sr1, VOLUME / 2, 1))
            : 1.0;
    if (optr->type == DBTMWILSON || optr->type == DBCLOVER) {
      rel_nrm +=
          optr->rel_prec
              ? (square_norm(optr->sr2, VOLUME / 2, 1) + square_norm(optr->sr3, VOLUME / 2, 1))
              : 1.0;
    }
    ...
    if (optr->reached_prec > 1.5 * (optr->eps_sq / rel_nrm)) {
      ...
      fflush(stdout);
      fatal_error("Reached precision larger that target precision by a factor of more than 1.5!",
                  "op_invert");
    }
  }
...
void op_write_prop(const int op_id, const int index_start, const int append_) {
  ...
  // timeslice soruces are usually used for smearing/fuzzing and dilution, this is tracked via
  // SourceInfo.ix in the filename
  if (SourceInfo.type == SRC_TYPE_POINT || SourceInfo.type == SRC_TYPE_TS) {
    if (PropInfo.splitted) {
      if (T_global > 99)
        sprintf(filename, "%s.%.4d.%.3d.%.2d.%s", PropInfo.basename, SourceInfo.nstore,
                SourceInfo.t, SourceInfo.ix, ending);
      else
        sprintf(filename, "%s.%.4d.%.2d.%.2d.%s", PropInfo.basename, SourceInfo.nstore,
                SourceInfo.t, SourceInfo.ix, ending);
    } else {
      if (T_global > 99)
        sprintf(filename, "%s.%.4d.%.3d.%s", PropInfo.basename, SourceInfo.nstore, SourceInfo.t,
                ending);
      else
        sprintf(filename, "%s.%.4d.%.2d.%s", PropInfo.basename, SourceInfo.nstore, SourceInfo.t,
                ending);
    }
  } else if (SourceInfo.type == SRC_TYPE_VOL) {
    sprintf(filename, "%s.%.4d.%.5d.%s", PropInfo.basename, SourceInfo.nstore, SourceInfo.sample,
            ending);
  } else if (SourceInfo.type == SRC_TYPE_PION_TS || SourceInfo.type == SRC_TYPE_GEN_PION_TS) {
    sprintf(filename, "%s.%.4d.%.5d.%.2d.%s", PropInfo.basename, SourceInfo.nstore,
            SourceInfo.sample, SourceInfo.t, ending);
  }

  if (!PropInfo.splitted || append_) append = 1;
  ...
  if (PropInfo.splitted || SourceInfo.ix == index_start) {
    ...
    inverterInfo = construct_paramsInverterInfo(optr->reached_prec, optr->iterations, optr->solver,
                                                optr->no_flavours);
  ...
  if (PropInfo.format == 1) {
    ...
    write_spinor(writer, &operator_list[op_id].sr0, &operator_list[op_id].sr1, 1,
                 SourceInfo.precision);
    if (optr->no_flavours == 2) {
      write_spinor(writer, &operator_list[op_id].sr2, &operator_list[op_id].sr3, 1,
                   SourceInfo.precision);
    }
    free(sourceFormat);
  }
  ...
  if (optr->no_flavours == 2) {
    write_spinor(writer, &operator_list[op_id].prop2, &operator_list[op_id].prop3, 1,
                 optr->prop_precision);
  }
  write_spinor(writer, &operator_list[op_id].prop0, &operator_list[op_id].prop1, 1,
               optr->prop_precision);
