...
int main(int argc, char *argv[]) {
  ...
  if (!piononly) {
    for (is = 0; is < 4; is++) {
      for (ic = 0; ic < 3; ic++) {
        if (!filenameflag && !appendflag) {
          if (T_global > 99) {
            sprintf(spinorfilename, "%s.%.4d.%.4d.%.3d.%.2d", filename, nstore, sample, t0,
                    3 * is + ic);
          } else {
            sprintf(spinorfilename, "%s.%.4d.%.4d.%.2d.%.2d", filename, nstore, sample, t0,
                    3 * is + ic);
          }
        } else if (!filenameflag && appendflag) {
          if (T_global > 99)
            sprintf(spinorfilename, "%s.%.4d.%.4d.%.3d", filename, nstore, sample, t0);
          else
        ...
        source_generation_nucleon(g_spinor_field[0], g_spinor_field[1], is, ic, t0, ts, ss, sample,
                                  nstore, typeflag);
    ...
    if (!ext_sourceflag) {
      ...
      printf("Generating source %s!\n", spinorfilename);
      fflush(stdout);
      source_generation_pion_only(g_spinor_field[0], g_spinor_field[1], t0, sample, nstore,
                                  random_seed);
      ...
      if (!filenameflag) {
        if (T_global > 99)
          sprintf(spinorfilename, "%s.%.4d.%.4d.%.3d.inverted", filename, nstore, sample, t0);
        else
      ...
      extended_pion_source(g_spinor_field[2], g_spinor_field[3], g_spinor_field[0],
                           g_spinor_field[1], t0, (g_nproc_t * T) / 2, 0., 0., 0.);
      if (!filenameflag) {
        if (T_global > 99)
          sprintf(spinorfilename, "g%s.%.4d.%.4d.%.3d", filename, nstore, sample, t0);
        else
