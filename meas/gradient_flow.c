...
void step_gradient_flow(su3 **x0, su3 **x1, su3 **x2, su3 **z, const unsigned int type,
                        const double eps) {
...
#pragma omp for
      ...
      for (int x = 0; x < VOLUME; ++x) {
        for (int mu = 0; mu < 4; ++mu) {
          ...
          // exists also in the inverter if the measurement is to be carried out there
...
void gradient_flow_measurement(const int traj, const int id, const int ieo) {
  ...
  if (g_proc_id == 0) {
    ...
    if (outfile == NULL) {
      ...
      snprintf(error_message, 200, "Couldn't open %s for writing during measurement %d!", filename,
               id);
