...
void heavy_correlators_measurement(const int traj, const int id, const int ieo, const int i1,
    ...
    if (b1 || b2) {
        if (g_proc_id == 0) {
            ...
            if (b2) {
                fprintf(stderr,
                        "Warning! optr2 should be one of the following: DBTMWILSON, DBCLOVER\n");
            }
            fprintf(stderr, "Cannot perform correlator online measurement!\n");
        }
        tm_stopwatch_pop(&g_timers, 0, 0, "");  // timer for profiling
    ...
    for (int sample = 0; sample < max_samples; sample++) {
        for (int ts = 0; ts < max_time_slices; ts++) {
            ...
            if (max_samples == 1 && max_time_slices == 1) {
                ...
                snprintf(filename, TM_OMEAS_FILENAME_LENGTH, "%s.t%03d.%06d", "heavy_mesons", ts,
                         traj);
            } else {
                snprintf(filename, TM_OMEAS_FILENAME_LENGTH, "%s.s%03d.%06d", "heavy_mesons",
                         sample, traj);
            }
            ...
            if (g_debug_level > 1 && g_proc_id == 0) {
                printf("# timeslice set to %d (T=%d) for online measurement\n", t0, g_nproc_t * T);
                printf(
                    "# online measurements parameters: kappa = %.12f, mubar = %.12f, epsbar = "
                    "%.12f\n",
              ...
              for (size_t F = 0; F < 2; F++) {       // flavor projection index
                for (size_t i_eo = 0; i_eo < 2; i_eo++) {    // even-odd
                  ...
                  mul_one_pm_itau2_and_div_by_sqrt2(
                      ...
                      arr_eo_spinor[0][1][beta][F][i_eo][0], arr_eo_spinor[0][1][beta][F][i_eo][1], +1.0,
                      VOLUME / 2);
...
#if defined TM_USE_MPI
                      MPI_Reduce(
                        &res_hihj_g1g2[hi][hj][g1][g2], &mpi_res_hihj_g1g2[hi][hj][g1][g2], 1, 
                        MPI_DOUBLE, MPI_SUM, 0, g_mpi_time_slices);
          ...
          if (g_mpi_time_rank == 0) {
            ...
            for (size_t hi = 0; hi < 2; hi++) {
              for (size_t hj = 0; hj < 2; hj++) {
                for (size_t g1 = 0; g1 < 2; g1++) {
                  for (size_t g2 = 0; g2 < 2; g2++) {
                    MPI_Gather(sC_hihj_g1g2[hi][hj][g1][g2], T, MPI_DOUBLE, C_hihj_g1g2[hi][hj][g1][g2],
                          T, MPI_DOUBLE, 0, g_mpi_SV_slices);
                  }
                }
              }
            }
