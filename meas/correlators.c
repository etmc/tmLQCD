	...
	for (int sample = 0; sample < max_samples; sample++) {
      ...
      */
      ...
      /* now we sum only over local space for every t */
          ...
          for (size_t beta = 0; beta < 4; beta++) {  // spin dilution
            ...
            for (size_t F = 0; F < 2; F++) {  // flavor projection
              for (size_t hi = 0; hi < 2; hi++) {
				...
				double vol_fact = (g_nproc_x * LX) / (g_nproc_y * LY) / (g_nproc_z * LZ);
#if defined TM_USE_MPI
        MPI_Reduce(&res, &mpi_res, 1, MPI_DOUBLE, MPI_SUM, 0, g_mpi_time_slices);
        res = mpi_res;
        MPI_Reduce(&respa, &mpi_respa, 1, MPI_DOUBLE, MPI_SUM, 0, g_mpi_time_slices);
        ...
        MPI_Reduce(&resp4, &mpi_resp4, 1, MPI_DOUBLE, MPI_SUM, 0, g_mpi_time_slices);
        resp4 = mpi_resp4;
        sCpp[t] = +res / vol_fact / 2. / optr1->kappa / optr1->kappa;
        sCpa[t] = -respa / vol_fact / 2. / optr1->kappa / optr1->kappa;
        sCp4[t] = +resp4 / vol_fact / 2. / optr1->kappa / optr1->kappa;
#else
        Cpp[t] = +res / vol_fact / 2. / optr1->kappa /
                 optr1->kappa;
        Cpa[t] = -respa / vol_fact / 2. / optr1->kappa / optr1->kappa;
        Cp4[t] = +resp4 / vol_fact / 2. / optr1->kappa / optr1->kappa;
#endif
      ...
      if (g_mpi_time_rank == 0) {
        ...
        for (size_t hi = 0; hi < 2; hi++) {
          for (size_t hj = 0; hj < 2; hj++) {
            for (size_t g1 = 0; g1 < 2; g1++) {
              for (size_t g2 = 0; g2 < 2; g2++) {
                MPI_Gather(&sC_hihj_g1g2[hi][hj][g1][g2][0], T, MPI_DOUBLE, &C_hihj_g1g2[hi][hj][g1][g2][0], T, MPI_DOUBLE, 0, g_mpi_SV_slices);
              }
            }
          }
        }
