...
// It is faster and tests prove also to be safe (read Appendix A of arxiv:1801.##### by S.Bacchio et
// al.)
...
int mg_update_gauge =
    1;  // set to zero if gaugefield is up to date, set to one if it has to be updated
...
int mg_no_shifts =
    0;  // number of shifts to invert with MG. MMS-CG is used for the others at larger mass.
double mg_mms_mass = 0.1;  // mass shift value for switching from MMS-CG to MG. MMS-CG is used for
                           // larger masses than the value.
...
static inline int MG_check(spinor *const phi_new, spinor *const phi_old, const int N,
                           const double precision, matrix_mult f) {
  ...
  if (residual > precision && residual < acc_factor * precision) {
    if (g_proc_id == 0)
      printf(
          "WARNING: solution accepted even if the residual wasn't complitely acceptable (%e > %e) "
          "\n",
          residual, precision);
...
static inline int MG_check_nd(spinor *const up_new, spinor *const dn_new, spinor *const up_old,
                              spinor *const dn_old, const int N, const double precision,
                              matrix_mult_nd f) {
  double differ[2], residual;
  spinor **check_vect = NULL;
  double acc_factor = 4;
#ifdef VARIANT_FOR_SHIFTED
  if ((f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0 and
                                   // shift
...
static inline int MG_mms_check_nd(spinor **const up_new, spinor **const dn_new,
                                  spinor *const up_old, spinor *const dn_old, const double *shifts,
                                  const int no_shifts, const int N, double *precision,
                                  matrix_mult_nd f) {
  double differ[2], residual;
  spinor **check_vect = NULL;
  double acc_factor = 2;
  ...
  for (int i = 0; i < no_shifts; i++) {
    ...
    if (residual > precision[i] && residual < acc_factor * precision[i]) {
      if (g_proc_id == 0)
        printf(
            "WARNING: solution accepted even if the residual wasn't complitely acceptable (%e > "
            "%e) \n",
...
static int MG_pre_solve(su3 **gf) {
  ...
  //  mg_dtau_update == 0.0  : updating at every change of configuration -> valid as well if
  //  configuration changed outside the HMC mg_rho_update < 0.0    : parameter ignore mg_rho_update
  //  == rho   : updating only if this condition and the others are satisfied
...
static int MG_solve(spinor *const phi_new, spinor *const phi_old, const double precision,
  ...
  if (f == Msw_psi ||        //          Schur complement with mu=0 on odd sites
    ...
    if (N != VOLUME / 2 && g_proc_id == 0)
      printf(
          "WARNING: expected N == VOLUME/2 for the required operator in MG_solve. Continuing with "
          "N == VOLUME\n");
    ...
    if (N != VOLUME && g_proc_id == 0)
      printf(
          "WARNING: expected N == VOLUME for the required operator in MG_solve. Continuing with N "
          "== VOLUME/2\n");
  } else if (g_proc_id == 0)
  ...
  if (g_proc_id == 0) {
    ...
    if (!mg_status.success) printf("ERROR: the solver did not converge!\n");
  }
...
static int MG_solve_nd(spinor *up_new, spinor *dn_new, spinor *const up_old, spinor *const dn_old,
  ...
  // In case of initial guess and squared operator, we do the inversion in two step and we need two
  // more vectors
  if (init_guess &&
      (f == Qtm_pm_ndpsi ||        // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
       f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0 and
                                   // shift
  ...
  if (no_solver_field > 0) init_solver_field(&solver_field, VOLUMEPLUSRAND, no_solver_field);
  ...
  if (init_guess && N == VOLUME / 2) {
    ...
    if (f == Qtm_pm_ndpsi ||        // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
        f == Qsw_pm_ndpsi ||        // (Gamma5 Dh tau1)^2 - Schur complement squared
        f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
                                    // and shift
        f == Qsw_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
        f == Qtm_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and
                                           // plus shift
        f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and
                                           // minus shift
...
#ifdef VARIANT_FOR_SHIFTED
      if ((f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
                                       // and shift
           f == Qsw_pm_ndpsi_shift)    // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
          && g_shift != 0) {
        if (f == Qtm_pm_ndpsi_shift) {  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw =
                                        // 0 and shift
    ...
    } else if (f == Qtm_pm_ndpsi_shift) {  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw
                                           // = 0 and shift
      Qtm_tau1_ndpsi_sub_Ishift(tmp12, tmp11, up_new, dn_new);  // tau1 exchange tmp11 <-> tmp12
    } else if (f ==
               Qsw_pm_ndpsi_shift) {  // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
  ...
  if (f == Qtm_ndpsi ||         //  Gamma5 Dh    - Schur complement with csw = 0
      ...
      f == Qtm_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and plus
                                         // shift
      f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and minus
                                         // shift
      f == Qsw_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with plus shift
      f == Qsw_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with minus shift
      f == Qtm_pm_ndpsi ||        // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
  ...
  else if (f == Qtm_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and
                                              // plus shift
           f == Qsw_tau1_ndpsi_add_Ishift)    // Gamma5 Dh tau1 - Schur complement with plus shift
    MG_update_mubar_epsbar(g_mubar, g_epsbar, sqrt(g_shift));
  else if (f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and
                                              // minus shift
           f == Qsw_tau1_ndpsi_sub_Ishift)    // Gamma5 Dh tau1 - Schur complement with minus shift
    MG_update_mubar_epsbar(g_mubar, g_epsbar, -sqrt(g_shift));
  ...
  if (f == Qtm_ndpsi ||         //  Gamma5 Dh    - Schur complement with csw = 0
    ...
    if (init_guess) {
      ...
      DDalphaAMG_solve_doublet_with_guess((double *)new1, (double *)old1, (double *)new2,
                                          (double *)old2, precision, &mg_status);
    } else {
  ...
  } else if (f == Qtm_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0
                                                // and plus shift
             f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0
                                                // and minus shift
             f == Qsw_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with plus shift
             f ==
                 Qsw_tau1_ndpsi_sub_Ishift) {  // Gamma5 Dh tau1 - Schur complement with minus shift
    mul_gamma5(old1, VOLUME);
    mul_gamma5(old2, VOLUME);
    ...
    if (init_guess) {
      ...
      mul_r(new1, 1 / mg_scale, new1, VOLUME);
      mul_r(new2, 1 / mg_scale, new2, VOLUME);
      DDalphaAMG_solve_doublet_with_guess((double *)new2, (double *)old1, (double *)new1,
                                          (double *)old2, precision, &mg_status);
    } else {
    ...
    if (N == VOLUME) {  // in case of VOLUME/2 old is a just local vector
      ...
      mul_gamma5(old2, VOLUME);
             ...
             f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw =
                                         // 0 and shift
             f == Qsw_pm_ndpsi ||        // (Gamma5 Dh tau1)^2 - Schur complement squared
             f == Qsw_pm_ndpsi_shift) {  // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
    ...
    if (init_guess) {
      ...
      DDalphaAMG_solve_doublet_with_guess((double *)new2tmp, (double *)old1, (double *)new1tmp,
                                          (double *)old2, precision / 2, &mg_status);
#ifdef VARIANT_FOR_SHIFTED
      if ((f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw = 0
                                       // and shift
           f == Qsw_pm_ndpsi_shift)    // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
        ...
        DDalphaAMG_solve_doublet_with_guess((double *)new2, (double *)old1, (double *)new1,
                                            (double *)old2, precision / 2, &mg_status);
        assign_mul_add_mul(new1, -_Complex_I / 2. / sqrt(g_shift), new1tmp,
                           _Complex_I / 2. / sqrt(g_shift), VOLUME);
        assign_mul_add_mul(new2, -_Complex_I / 2. / sqrt(g_shift), new2tmp,
                           _Complex_I / 2. / sqrt(g_shift), VOLUME);
      } else
#endif
      {
        ...
        if (f == Qtm_pm_ndpsi_shift ||  // (Gamma5 Dh tau1)^2 - Schur complement squared with csw =
            ...
            f == Qsw_pm_ndpsi_shift)    // (Gamma5 Dh tau1)^2 - Schur complement squared with shift
          MG_update_mubar_epsbar(g_mubar, g_epsbar, -sqrt(g_shift));
        DDalphaAMG_solve_doublet_with_guess((double *)new2, (double *)new1tmp, (double *)new1,
                                            (double *)new2tmp, precision / 2, &mg_status);
      }
      if (N == VOLUME) {  // in case of VOLUME/2 old is a just local vector
        mul_gamma5(old1, VOLUME);
        mul_gamma5(old2, VOLUME);
      ...
      mg_scale *= mg_scale;
      DDalphaAMG_solve_doublet_squared_odd((double *)new2, (double *)old2, (double *)new1,
                                           (double *)old1, precision, &mg_status);
    }
  } else if (f == D_ndpsi) {  //  Dh
    if (init_guess) {
      // Removing normalization from initial guess
  ...
  if (no_solver_field > 0) finalize_solver(solver_field, no_solver_field);
...
static int MG_mms_solve_nd(spinor **const up_new, spinor **const dn_new, spinor *const up_old,
  ...
  // Checking if the operator is in the list and compatible with N
  if (f == Qtm_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and plus
                                         // shift
      f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0 and minus
                                         // shift
      f == Qsw_tau1_ndpsi_add_Ishift ||  // Gamma5 Dh tau1 - Schur complement with plus shift
  ...
  } else if (f == Qtm_tau1_ndpsi_sub_Ishift ||  // Gamma5 Dh tau1 - Schur complement with csw = 0
                                                // and minus shift
             f ==
                 Qsw_tau1_ndpsi_sub_Ishift) {  // Gamma5 Dh tau1 - Schur complement with minus shift
    MG_update_mubar_epsbar(g_mubar, g_epsbar, -shifts[0]);
...
void MG_init() {
  ...
  if (reproduce_randomnumber_flag) {
    ...
    for (int i = 0; i < g_nproc; i++) mg_init.rnd_seeds[i] = random_seed + i * 1000;
  } else
    mg_init.rnd_seeds = NULL;

  DDalphaAMG_initialize(&mg_init, &mg_params, &mg_status);

  if (reproduce_randomnumber_flag) free(mg_init.rnd_seeds);
  ...
  // with mixed_precision = 2 the library adapt the solving precision according to the vector
  // components
  if (mg_mixed_prec)
...
void MG_update_mubar_epsbar(double mubar_tmLQCD, double epsbar_tmLQCD, double shift_tmLQCD) {
  ...
  if (mubar != mg_params.mu || mg_params.mu_odd_shift != 0.0 || mg_params.mu_even_shift != 0.0 ||
      epsbar != mg_params.epsbar || shift != mg_params.epsbar_ig5_odd_shift ||
      mg_params.epsbar_ig5_even_shift != 0.0 || mg_params.smoother_iterations != 2) {
    // Taking advantage of this function for updating printing in HMC
    if (g_debug_level > 0)
      mg_params.print = 1;
    else
...
void MG_reset() {
  if (mg_do_setup == 0) DDalphaAMG_free();
...
void MG_finalize() { DDalphaAMG_finalize(); }
...
int MG_solver(spinor *const phi_new, spinor *const phi_old, const double precision,
  ...
  if (success && g_debug_level > 2) success = MG_check(phi_new, phi_old, N, mg_prec, f);
...
int MG_solver_nd(spinor *const up_new, spinor *const dn_new, spinor *const up_old,
                 ...
                 const int rel_prec, const int N, su3 **gf, matrix_mult_nd f) {
  int success = 0;
  double mg_prec = rel_prec
                       ? sqrt(precision)
                       : sqrt(precision / (square_norm(up_old, N, 1) + square_norm(dn_old, N, 1)));
  ...
  if (success && g_debug_level > 2) {
    ...
    if (!success) {
      ...
      if (success) success = MG_check_nd(up_new, dn_new, up_old, dn_old, N, mg_prec, f);
    }
  }
...
int MG_solver_nd_eo(spinor *const Even_new_up, spinor *const Odd_new_up, spinor *const Even_new_dn,
                    spinor *const Odd_new_dn, spinor *const Even_up, spinor *const Odd_up,
                    spinor *const Even_dn, spinor *const Odd_dn, const double precision,
                    const int max_iter, const int rel_prec, const int N, su3 **gf,
                    matrix_mult_full_nd f_full) {
  int iter_count;
  spinor **solver_field = NULL;
  matrix_mult_nd f;
  ...
  iter_count = MG_solver_nd(solver_field[2], solver_field[3], solver_field[0], solver_field[1],
                            precision, max_iter, rel_prec, VOLUME, gf, f);
...
int MG_mms_solver_nd(spinor **const up_new, spinor **const dn_new, spinor *const up_old,
                     spinor *const dn_old, const double *shifts, const int no_shifts,
                     const double *precision, const int max_iter, const int rel_prec, const int N,
                     su3 **gf, matrix_mult_nd f) {
  int success = 0;
  double mg_prec[no_shifts];
  if (rel_prec) {
    for (int i = 0; i < no_shifts; i++) mg_prec[i] = sqrt(precision[i]);
  } else {
    double nrhs = square_norm(up_old, N, 1) + square_norm(dn_old, N, 1);
    for (int i = 0; i < no_shifts; i++) mg_prec[i] = sqrt(precision[i] / nrhs);
  }
