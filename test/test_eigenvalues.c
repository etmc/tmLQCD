...
int main(int argc, char *argv[]) {
  ...
  if (g_proc_id == 0) {
    ...
    printf(
        "# Using in chronological inverter for spinor_field 1,2,3 a history of %d, %d, %d, "
        "respectively\n",
        g_csg_N[0], g_csg_N[2], g_csg_N[4]);

    fprintf(parameterfile, "The lattice size is %d x %d x %d x %d\n", (int)(g_nproc_t * T),
            (int)(g_nproc_x * LX), (int)(LY), (int)(LZ));
    ...
    fprintf(parameterfile,
            "int_n_gauge = %d, int_n_ferm1 = %d, int_n_ferm2 = %d, int_n_ferm3 = %d\n ", int_n[0],
            int_n[1], int_n[2], int_n[3]);
    ...
    if (integtyp == 5)
      fprintf(parameterfile, "higher order and leap-frog (multiple time scales)\n");
  ...
  if (g_proc_id == 0) {
    ...
    fprintf(countfile,
            "!!! Timestamp %ld, Nsave = %d, g_mu = %e, g_mu1 = %e, g_mu_2 = %e, g_mu3 = %e, beta = "
            "%f, kappa = %f, C1 = %f, int0 = %d, int1 = %d, int2 = %d, int3 = %d, g_eps_sq_force = "
            "%e, g_eps_sq_acc = %e, ",
            t1.tv_sec, Nsave, g_mu, g_mu1, g_mu2, g_mu3, g_beta, g_kappa, g_rgi_C1, int_n[0],
            int_n[1], int_n[2], int_n[3], g_eps_sq_force, g_eps_sq_acc);
    fprintf(countfile, "Nsteps = %d, dtau = %e, tau = %e, integtyp = %d, rel. prec. = %d\n", Nsteps,
            dtau, tau, integtyp, g_relative_precision_flag);
    fclose(countfile);
  }
