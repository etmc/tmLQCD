...
void gauge_momenta(double step) {
  ...
  sprintf(filename,"debug_staples.s");
#ifdef PARALLELT
  sprintf(filename,"debug_staples.pt.%d", g_proc_id);
#endif
#ifdef PARALLELXT
  sprintf(filename,"debug_staples.pxt.%d", g_proc_id);
#endif
...
/*   for(i = 0; i < VOLUME; i++){ */
	...
	_su3_times_su3d(w, *z, v);
	_trace_lambda(deriv, w);
	_trace_su3_times_su3d(d, *z, v);
	fprintf(debugfile,"%d %d %d %d %d %e\n", 
		g_proc_coords[0]*T+t, g_proc_coords[1]*LX+x, y, zz, mu, d);
	fflush(stdout);
#ifdef _COMP_FORCE
	sum2 = _su3adj_square_norm(deriv);
	sum1+= sum2;
