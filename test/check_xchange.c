...
#define REQC 12
  ...
  for (x1 = 0; x1 < LX; x1++) {
    for (x2 = 0; x2 < LY; x2++) {
     ...
     (defined PARALLELX) || (defined PARALLELXY) || (defined PARALLELXYZ))
  for (x0 = 0; x0 < T; x0++) {
    for (x2 = 0; x2 < LY; x2++) {
      for (x3 = 0; x3 < LZ; x3++) {
...
#if ((defined PARALLELXYT) || (defined PARALLELXYZT) || (defined PARALLELXY) || \
     (defined PARALLELXYZ))
  for (x0 = 0; x0 < T; x0++) {
    for (x1 = 0; x1 < LX; x1++) {
      for (x3 = 0; x3 < LZ; x3++) {
  ...
  for (x0 = 0; x0 < T; x0++) {
    for (x1 = 0; x1 < LX; x1++) {
      for (x2 = 0; x2 < LY; x2++) {
        set_spinor_point(&g_spinor_field[0][g_lexic2eosub[g_ipt[x0][x1][x2][0]]],
                         g_cart_id); /* only even */
  ...
  for (x0 = 0; x0 < T; x0++) {
    for (x1 = 0; x1 < LX; x1++) {
      for (x2 = 0; x2 < LY; x2++) {
        ...
        set_spinor_point(&g_spinor_field[0][g_lexic2eosub[g_ipt[x0][x1][x2][LZ - 1]]],
                         g_cart_id); /* only even */
      }
    }
  }
    ...
    for (x1 = -startvaluex; x1 < (LX+startvaluex); x1++){
      for (x2 = -startvaluey; x2 < (LY+startvaluey); x2++){
        for (x3 = -startvaluez; x3 < (LZ+startvaluez); x3++){
          ...
          if(x2 > LY) bndcntu2++;
          if(x3 > LZ) bndcntu2++;
          if((bndcntu+bndcntd<=2) && (bndcntu2+bndcntd2<=1) && (bndcntu2*bndcntd==0) &&
  (bndcntu*bndcntd2==0)){ i=Index(x0,x1,x2,x3); x = (double*) g_gauge_field[i]; if(g_proc_id==0)
  fprintf(stdout,"debuG-%d: %g , %d,%d,%d,%d , %d\n",g_proc_id,*x,x0,x1,x2,x3,i); fflush(stdout); }
  else { if(g_proc_id==0) fprintf(stdout,"outside-%d: nan, %d,%d,%d,%d\n",g_proc_id,x0,x1,x2,x3);
            fflush(stdout);
          }
        }
      }
  ...
  */
  ...
  /* The edges */
#if (defined PARALLELXT || defined PARALLELXYT || defined PARALLELXYZT)
  fprintf(stdout, "rank:%d; (c0,c1,c2,c3)=(%d,%d,%d,%d)\n", g_proc_id, g_proc_coords[0],
          g_proc_coords[1], g_proc_coords[2], g_proc_coords[3]);
  fflush(stdout);
  ...
  for (x1 = 0; x1 < LX; x1++) {
    for (x2 = 0; x2 < LY; x2++) {
      for (x3 = 0; x3 < LZ; x3++) {
        ...
        for (mu = 0; mu < 4; mu++) {
          if (df0[ix][mu].d1 != (double)g_nb_t_up || df0[ix][mu].d2 != (double)g_nb_t_up ||
              df0[ix][mu].d3 != (double)g_nb_t_up || df0[ix][mu].d4 != (double)g_nb_t_up ||
              df0[ix][mu].d5 != (double)g_nb_t_up || df0[ix][mu].d6 != (double)g_nb_t_up ||
              df0[ix][mu].d7 != (double)g_nb_t_up || df0[ix][mu].d8 != (double)g_nb_t_up) {
            printf("Exchange of derivatives is working not correctly (1)!\n");
            printf("%d %d %d %d %f %d %d\n", ix, x1, x2, x3, df0[ix][mu].d1, g_nb_t_up, mu,
                   (T - 1 + x1 + x2 + x3) % 2);
  ...
  for (x0 = 0; x0 < T - 1; x0++) {
    for (x2 = 0; x2 < LY; x2++) {
      for (x3 = 0; x3 < LZ; x3++) {
        ...
        for (mu = 0; mu < 4; mu++) {
          if (df0[ix][mu].d1 != (double)g_nb_x_up || df0[ix][mu].d2 != (double)g_nb_x_up ||
              df0[ix][mu].d3 != (double)g_nb_x_up || df0[ix][mu].d4 != (double)g_nb_x_up ||
              df0[ix][mu].d5 != (double)g_nb_x_up || df0[ix][mu].d6 != (double)g_nb_x_up ||
              df0[ix][mu].d7 != (double)g_nb_x_up || df0[ix][mu].d8 != (double)g_nb_x_up) {
  ...
  for (x0 = 0; x0 < T - 1; x0++) {
    for (x1 = 1; x1 < LX - 1; x1++) {
      for (x3 = 0; x3 < LZ; x3++) {
        ...
        for (mu = 0; mu < 4; mu++) {
          if (df0[ix][mu].d1 != (double)g_nb_y_up || df0[ix][mu].d2 != (double)g_nb_y_up ||
              df0[ix][mu].d3 != (double)g_nb_y_up || df0[ix][mu].d4 != (double)g_nb_y_up ||
              df0[ix][mu].d5 != (double)g_nb_y_up || df0[ix][mu].d6 != (double)g_nb_y_up ||
              df0[ix][mu].d7 != (double)g_nb_y_up || df0[ix][mu].d8 != (double)g_nb_y_up) {
  ...
  for (x0 = 0; x0 < T - 1; x0++) {
    for (x1 = 1; x1 < LX - 1; x1++) {
      for (x2 = 0; x2 < LY - 1; x2++) {
        ...
        for (mu = 0; mu < 4; mu++) {
          if (df0[ix][mu].d1 != (double)g_nb_z_up || df0[ix][mu].d2 != (double)g_nb_z_up ||
              df0[ix][mu].d3 != (double)g_nb_z_up || df0[ix][mu].d4 != (double)g_nb_z_up ||
              df0[ix][mu].d5 != (double)g_nb_z_up || df0[ix][mu].d6 != (double)g_nb_z_up ||
              df0[ix][mu].d7 != (double)g_nb_z_up || df0[ix][mu].d8 != (double)g_nb_z_up) {
...
int check_xchange() {
  ...
  for (k = 0; k < 1; k++) {
    ...
    x = (double*)&g_gauge_field[VOLUME + 2 * LX * LY * LZ + 2 * T * LY * LZ + 2 * T * LX * LZ +
                                T * LX * LY][0];
    for (i = 0; i < T * LX * LY * 72; i++, x++) {
      if ((int)(*x) != g_nb_z_dn) {
        printf("The exchange down of gaugefields in z direction down\n");
    ...
    if (g_dbw2rand > 0) {
      ...
      x = (double*)&g_gauge_field[VOLUMEPLUSRAND + 2 * LX * LY * LZ + 2 * T * LY * LZ + T * LX * LZ]
                                 [0];
      for (i = 0; i < T * LX * LZ * 72; i++, x++) {
        if ((int)(*x) != g_nb_y_dn) {
          printf("The exchange down of gaugefields in y direction\n");
      ...
      x = (double*)&g_gauge_field[VOLUMEPLUSRAND + 2 * LX * LY * LZ + 2 * T * LY * LZ +
                                  2 * T * LX * LZ + T * LX * LY][0];
      for (i = 0; i < T * LX * LY * 72; i++, x++) {
        if ((int)(*x) != g_nb_z_dn) {
          printf("The exchange down of gaugefields in y direction\n");
      ...
      x = (double*)
          g_gauge_field[VOLUMEPLUSRAND + RAND + 8 * LY * LZ + 8 * T * LZ + 8 * LX * LZ + LX * LY];
      for (i = 0; i < LX * LY * 72; i++, x++) {
        if ((int)(*x) != mp) {
          printf("The exchange of gaugefields edges (tz) in direction +z-2t\n");
          printf("between %d and %d is not correct\n", g_cart_id, mp);
