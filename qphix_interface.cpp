...
void _initQphix(int argc, char **argv, tm_QPhiXParams_t params, int c12, QphixPrec_t precision_, QphixPrec_t inner_precision_) {
  ...
  for (int dim = 0; dim < 4; dim++) {
    ...
    if (dim == 0) {
      ...
      dim_phase[0] = -((double*)(&phase_1))[1] / g_kappa;
      dim_phase[1] = -((double*)(&phase_1))[2] / g_kappa;
    } else if (dim == 1) {
      dim_tbc = (fabs(X2) > DBL_EPSILON);
      dim_phase[0] = -((double*)(&phase_2))[1] / g_kappa;
      dim_phase[1] = -((double*)(&phase_2))[2] / g_kappa;
    } else if (dim == 2) {
      dim_tbc = (fabs(X3) > DBL_EPSILON);
      dim_phase[0] = -((double*)(&phase_3))[1] / g_kappa;
      dim_phase[1] = -((double*)(&phase_3))[2] / g_kappa;
    } else if (dim == 3) {
      dim_tbc = (fabs(X0) > DBL_EPSILON);
      dim_phase[0] = -((double*)(&phase_0))[1] / g_kappa;
      dim_phase[1] = -((double*)(&phase_0))[2] / g_kappa;
    }
    use_tbc[dim] = dim_tbc;
