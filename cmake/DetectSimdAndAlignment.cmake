...
#
# Exposed cache variables: SIMD_ARCH_FAMILY : x86 / ARM / PPC / UNKNOWN
# SIMD_LEVEL       : AVX512 / AVX2 / SSE2 / NEON / ALTIVEC / SCALAR
# SIMD_ALIGNMENT   : integer, in bytes (16, 32, 64, ...)
#
# Optional (if you want a configured header): SIMD_CONFIG_HEADER : path to the
# generated header (see bottom).
#
# Usage: include(cmake/DetectSimdAndAlignment.cmake) message(STATUS "SIMD:
# ${SIMD_ARCH_FAMILY} ${SIMD_LEVEL}, alignment=${SIMD_ALIGNMENT}")
#
# # Example: propagate as defines target_compile_definitions(my_target PRIVATE
# SIMD_ALIGNMENT=${SIMD_ALIGNMENT} SIMD_LEVEL_${SIMD_LEVEL} )
...
if(NOT DEFINED SIMD_ARCH_FAMILY)
  ...
  set(SIMD_ARCH_FAMILY
      "${_detected_arch}"
      CACHE STRING "SIMD architecture family")
endif()

# Defaults
set(SIMD_LEVEL
    "SCALAR"
    CACHE STRING "Detected SIMD level")
set(SIMD_ALIGNMENT
    16
    CACHE STRING "Alignment in bytes")
set(SIMD_HAS_FLOAT
    ON
    CACHE BOOL "Float SIMD support")
set(SIMD_HAS_DOUBLE
    ON
    CACHE BOOL "Double SIMD support")
...
elseif(SIMD_ARCH_FAMILY STREQUAL "ARM")
  ...
  elseif(_arm_proc MATCHES "armv8")
    ...
    set(CMAKE_REQUIRED_FLAGS
        "${_SIMD_SAVED_REQUIRED_FLAGS} -march=armv8-a+simd")
    check_source_runs(
      CXX
      "
            #include <arm_neon.h>
  ...
  elseif(_arm_proc MATCHES "armv7")
    cmake_push_check_state(RESET)
    set(CMAKE_REQUIRED_FLAGS
        "${_SIMD_SAVED_REQUIRED_FLAGS} -mfpu=neon -march=armv7-a")
    check_source_runs(
      CXX
      "
            #include <arm_neon.h>
  ...
  # --------------------------------------
  # 1. POWERPC - COMPLETE COVERAGE (NEW!)
  # --------------------------------------
elseif(SIMD_ARCH_FAMILY STREQUAL "PPC")
  ...
  # === Power10+ (512-bit vectors, POWER10) Note: Power10 needs -mcpu=power10 or
  # -mtune=power10
  ...
  if(_HAVE_POWER10)
    ...
    set(SIMD_ALIGNMENT
        64
        CACHE STRING "" FORCE) # 512-bit = 64 bytes
    set(SIMD_HAS_FLOAT
        ON
        CACHE BOOL "" FORCE)
    set(SIMD_HAS_DOUBLE
  ...
  if(_HAVE_VSX_POWER9)
    ...
    set(SIMD_ALIGNMENT
        32
        CACHE STRING "" FORCE) # 256-bit = 32 bytes
    set(SIMD_HAS_FLOAT
        ON
        CACHE BOOL "" FORCE)
    set(SIMD_HAS_DOUBLE
  ...
  cmake_push_check_state(RESET)
  set(CMAKE_REQUIRED_FLAGS
      "${_SIMD_SAVED_REQUIRED_FLAGS} -maltivec -mabi=altivec")
  check_source_runs(
    CXX
    "
        #include <altivec.h>
  ...
  if(_HAVE_ALTIVEC)
    ...
    set(SIMD_HAS_DOUBLE
        OFF
        CACHE BOOL "" FORCE) # AltiVec: float primary
    _simd_restore_flags()
    return()
  endif()

  # --------------------------------------
  # 1. NVIDIA GH200 (sm_89)
  # --------------------------------------
