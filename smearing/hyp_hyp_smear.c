...
void hyp_smear(hyp_control *control, gauge_field_t in)
{
  ...
  {
    smear_first_stage(control->staples[0], control->coef[0], in);
    smear_second_stage(control->staples[1], control->coef[1], control->staples[0], in);
    smear_third_stage(control->U[1], control->coef[2], control->staples[1], in);
    in = control->U[1];
  }
