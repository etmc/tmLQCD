...
void hyp_staples_exclude_one(gauge_field_array_t buff_out, gauge_field_array_t buff_in)
{
  static su3 tmp;
...
#define _ADD_STAPLES_TO_COMPONENT(component, to, excl, via, x) \
  { \
    _su3_times_su3d(tmp, buff_in.field_array[I2_ ## to ## _ ## excl ## via / 4].field[g_iup[x][via]][I2_ ## to ## _ ## excl ## via % 4], buff_in.field_array[I2_ ## via ## _ ## to ## excl / 4].field[g_iup[x][to]][I2_ ## via ## _ ## to ## excl % 4]); \
    _su3_times_su3_acc(buff_out.field_array[component / 4].field[x][component % 4], buff_in.field_array[I2_ ## via ## _ ## to ## excl / 4].field[x][I2_ ## via ## _ ## to ## excl % 4], tmp); \
    _su3_times_su3(tmp, buff_in.field_array[I2_ ## to ## _ ## excl ## via / 4].field[g_idn[x][via]][I2_ ## to ## _ ## excl ## via % 4], buff_in.field_array[I2_ ## via ## _ ## to ## excl / 4].field[g_iup[g_idn[x][via]][to]][I2_ ## via ## _ ## to ## excl % 4]); \
    _su3d_times_su3_acc(buff_out.field_array[component / 4].field[x][component % 4], buff_in.field_array[I2_ ## via ## _ ## to ## excl / 4].field[g_idn[x][via]][I2_ ## via ## _ ## to ## excl % 4], tmp); \
  }
