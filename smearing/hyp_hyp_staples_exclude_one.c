...
#define _ADD_STAPLES_TO_COMPONENT(component, to, excl, via, x)                                    \
  {                                                                                               \
    _su3_times_su3d(tmp,                                                                          \
                    buff_in[I2_##to##_##excl##via / 4][g_iup[x][via]][I2_##to##_##excl##via % 4], \
                    buff_in[I2_##via##_##to##excl / 4][g_iup[x][to]][I2_##via##_##to##excl % 4]); \
    _su3_times_su3_acc(buff_out[component / 4][x][component % 4],                                 \
                       buff_in[I2_##via##_##to##excl / 4][x][I2_##via##_##to##excl % 4], tmp);    \
    _su3_times_su3(                                                                               \
        tmp, buff_in[I2_##to##_##excl##via / 4][g_idn[x][via]][I2_##to##_##excl##via % 4],        \
        buff_in[I2_##via##_##to##excl / 4][g_iup[g_idn[x][via]][to]][I2_##via##_##to##excl % 4]); \
    _su3d_times_su3_acc(                                                                          \
        buff_out[component / 4][x][component % 4],                                                \
        buff_in[I2_##via##_##to##excl / 4][g_idn[x][via]][I2_##via##_##to##excl % 4], tmp);       \
  }
