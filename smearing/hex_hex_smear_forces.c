...
#pragma omp parallel
  for (unsigned int iter = control->iterations; iter > 0; --iter)
  {
    ...
    add_terms_to_forces_V(smeared_force, control->trace_stage_3[iter - 1], control->coeff[2], control->U[iter], control->U[iter - 1]);
    
    construct_Sigma_2(control->trace_stage_2[iter - 1], control->coeff[2], control->V_stage_2[iter - 1], control->trace_stage_3[iter - 1]);
    construct_Z_2(control->trace_stage_2[iter - 1], control->U[iter - 1] );
    add_terms_to_forces_2(smeared_force, control->trace_stage_2[iter - 1], control->coeff[1], control->V_stage_2[iter - 1], control->U[iter - 1]);
    
    construct_Sigma_1(control->trace_stage_1[iter - 1], control->coeff[1], control->V_stage_1[iter - 1], control->trace_stage_2[iter - 1]);
    construct_Z_1(control->trace_stage_1[iter - 1], control->U[iter - 1]);
    add_terms_to_forces_1(smeared_force, control->trace_stage_1[iter - 1], control->coeff[0], control->V_stage_1[iter - 1], control->U[iter - 1]);
    
    construct_Sigma_0(Sigma_U, control->coeff[0], control->U[iter - 1], control->trace_stage_1[iter - 1]);
