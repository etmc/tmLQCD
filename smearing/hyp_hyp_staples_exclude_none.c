...
#define _ADD_STAPLES_TO_COMPONENT(component, to, via, x)                                         \
  {                                                                                              \
    _su3_times_su3d(tmp, buff_in[I1_##to##_##via / 4][g_iup[x][via]][I1_##to##_##via % 4],       \
                    buff_in[I1_##via##_##to / 4][g_iup[x][to]][I1_##via##_##to % 4]);            \
    _su3_times_su3_acc(buff_out[component / 4][x][component % 4],                                \
                       buff_in[I1_##via##_##to / 4][x][I1_##via##_##to % 4], tmp);               \
    _su3_times_su3(tmp, buff_in[I1_##to##_##via / 4][g_idn[x][via]][I1_##to##_##via % 4],        \
                   buff_in[I1_##via##_##to / 4][g_iup[g_idn[x][via]][to]][I1_##via##_##to % 4]); \
    _su3d_times_su3_acc(buff_out[component / 4][x][component % 4],                               \
                        buff_in[I1_##via##_##to / 4][g_idn[x][via]][I1_##via##_##to % 4], tmp);  \
  }
