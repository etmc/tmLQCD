...
int write_first_messages(FILE* parameterfile, char const* const executable,
                         char const* const git_hash) {
  char message[1024];
  snprintf(message, 1024,
           "This is the %s code for twisted mass Wilson QCD\n\nVersion %s, commit %s\n", executable,
  ...
  printf("# The lattice size is %d x %d x %d x %d\n", (int)(T * g_nproc_t), (int)(LX * g_nproc_x),
         (int)(LY * g_nproc_y), (int)(LZ * g_nproc_z));
  ...
  fprintf(parameterfile, "# The lattice size is %d x %d x %d x %d\n", (int)(g_nproc_t * T),
          (int)(g_nproc_x * LX), (int)(g_nproc_y * LY), (int)(g_nproc_z * LZ));
  ...
  if (strcmp(executable, "invert") == 0) {
    ...
    fprintf(parameterfile, "# beta = %.12f, mu = %.12f, kappa = %.12f\n", g_beta,
