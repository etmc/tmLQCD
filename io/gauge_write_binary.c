...
int write_binary_gauge_data(LemonWriter* lemonwriter, const int prec, DML_Checksum* checksum) {
  ...
  if (status != LEMON_SUCCESS) {
    free(filebuffer);
    fprintf(stderr,
            "LEMON write error occurred with status = %d, while writing in gauge_write_binary.c!\n",
            status);
    return (-2);
  }
  ...
  if (status != MPI_SUCCESS) {
    fprintf(stderr,
            "DML Checksum accumulation error occurred with status = %d, while writing in "
      ...
      for (y = 0; y < LY * g_nproc_y; y++) {
        ...
        for (x = 0; x < LX * g_nproc_x; x++) {
          ...
          if (g_cart_id == 0) {
            ...
            if (status < 0) {
              fprintf(stderr,
                      "LIME write error occurred with status = %d, while writing in "
                      "gauge_write_binary.c!\n",
                      status);
