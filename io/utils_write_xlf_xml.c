...
void write_xlf_info_xml(WRITER *writer, paramsXlfInfo const *info) {
  ...
  if (message == (char *)NULL)
    kill_with_error(writer->fp, g_cart_id,
                    "Memory allocation error in write_xlf_info_xml. Aborting\n");
  ...
  if (info->kappa != 0.0) {
    sprintf(message,
            ...
            "  <date>%s</date>\n"
            "</xlf-info>",
            info->plaq, info->counter, info->beta, info->kappa, info->mu, info->c2_rec, info->time,
            info->package_version, info->mubar, info->epsilonbar, info->date);
    ...
    sprintf(message,
            ...
            "  <date>%s</date>\n"
            "</xlf-info>",
            info->plaq, info->counter, info->beta, info->kappa, info->mu, info->c2_rec, info->date);
  }
  bytes = strlen(message);
