...
void write_inverter_info(WRITER *writer, paramsInverterInfo const *info) {
  ...
  if (info->mms) {
    sprintf(message,
            ...
            "inverter version = %s\n"
            "date = %s",
            info->inverter, info->epssq, info->iter, info->kappa, info->cgmms_mass, info->mu,
    ...
    if (!info->heavy) {
      sprintf(message,
              ...
              "inverter version = %s\n"
              "date = %s",
              info->inverter, info->epssq, info->iter, info->kappa, info->mu, info->package_version,
              info->date);
    } else {
      sprintf(message,
              "solver = %s\n"
              ...
              "inverter version = %s\n"
              "date = %s",
              info->inverter, info->epssq, info->iter, info->kappa, info->mubar, info->epsbar,
              info->package_version, info->date);
    }
  }
