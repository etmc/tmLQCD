...
int read_binary_spinor_data(spinor *const s, spinor *const r, LemonReader *lemonreader,
  ...
  if (status < 0 && status != LEMON_EOR) {
    fprintf(stderr, "lemonReadLatticeParallelMapped returned error %d in spinor_read_binary.c",
            status);
    free(filebuffer);
    return (-2);
  }
    ...
    for (z = 0; z < LZ; z++) {
      for (y = 0; y < LY; y++) {
        for (x = 0; x < LX; x++) {
          ...
          p = ((t + x + y + z + g_proc_coords[3] * LZ + g_proc_coords[2] * LY +
                g_proc_coords[1] * LX + g_proc_coords[0] * T) %
               2)
                  ? r
                  : s;
          if (prec == 32)
  ...
  for (t = 0; t < T; t++) {
    for (z = 0; z < LZ; z++) {
      for (y = 0; y < LY; y++) {
        ...
        limeReaderSeek(
            limereader,
            (n_uint64_t)(g_proc_coords[1] * LX + (((g_proc_coords[0] * T + t) * g_nproc_z * LZ +
                                                   g_proc_coords[3] * LZ + z) *
                                                      g_nproc_y * LY +
                                                  g_proc_coords[2] * LY + y) *
                                                     LX * g_nproc_x) *
        ...
        for (x = 0; x < LX; x++) {
          ...
          if (status < 0 && status != LIME_EOR) {
            fprintf(stderr,
                    "LIME read error occurred with status = %d while reading in "
