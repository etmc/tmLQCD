...
#ifdef TM_USE_MPI
void check_mpi_comm_membership(MPI_Comm commself, MPI_Comm commcheck, const char *name_a,
...
void spinor_fft_transpose_xp_t(spinor *fieldout, spinor *fieldin, int dim0, int dim1, int forward,
                               double mulp);
...
void spinor_fft_reduce_2d(spinor *localSpinorField, int *collectionRank, spinor ***field_collection,
  ...
  for (i = 0; i < 4; i++) sendRecvCoord[i] = g_proc_coords[i];
  ...
  if (g_proc_coords[which[0]] == 0 && g_proc_coords[which[1]] == 0) {
    ...
    *collectionRank = TRUE;
    ...
    for (sendRecvCoord[which[0]] = 0; sendRecvCoord[which[0]] < dims[which[0]];
         sendRecvCoord[which[0]]++) {
      for (sendRecvCoord[which[1]] = 0; sendRecvCoord[which[1]] < dims[which[1]];
           sendRecvCoord[which[1]]++) {
        if (sendRecvCoord[which[0]] != 0 || sendRecvCoord[which[1]] != 0) {
          MPI_Cart_rank(g_cart_grid, sendRecvCoord, &recvRank);

          MPI_Irecv(accu_field[sendRecvCoord[which[0]] * dims[which[1]] +
                               sendRecvCoord[which[1]]] /* buffer */,
                    1,                /* how may */
    ...
    /* create fftw plan */
    local_2d_fft_forward = spinor_fftw_plan2d(fft_field[0], accu_field[0], T * dims[0],
                                              LX * dims[1], LY * LZ, 1, FFTW_ESTIMATE);
    ...
    free_spinor_field_array(&memory_buffer_fft_field);
    memory_buffer_fft_field = NULL;
    ...
    *collectionRank = FALSE;
    ...
    MPI_Isend(localSpinorField, 1, mpi_local_spinor, sendRank, MSG_LOCALDATA, g_cart_grid,
              &request);
...
#else
  if (g_proc_id == 0) fprintf(stderr, "Error: Please choose FOUR dimensional parallelization!!!\n");
...
void spinor_fft_redist_2d(spinor *localSpinorField, int collectionRank, spinor **field_collection,
  ...
  for (int i = 0; i < 4; i++) sendRecvCoord[i] = g_proc_coords[i];
  ...
  if (collectionRank == TRUE) {
    ...
    /* create fftw plan */
    local_2d_fft_backward = spinor_fftw_plan2d(accu_field[0], fft_field[0], T * dims[0],
                                               LX * dims[1], LY * LZ, 0, FFTW_ESTIMATE);
    ...
    /* transpose in xp-t space */
    spinor_fft_transpose_xp_t(accu_field[0], fft_field[0], dims[0], dims[1], FALSE,
                              1. / (double)(T * dims[0] * LX * dims[1]));
    ...
    for (sendRecvCoord[which[0]] = 0; sendRecvCoord[which[0]] < dims[which[0]];
      ...
      for (sendRecvCoord[which[1]] = 0; sendRecvCoord[which[1]] < dims[which[1]];
        ...
        if (sendRecvCoord[which[0]] != 0 || sendRecvCoord[which[1]] != 0) {
          ...
          MPI_Isend(accu_field[sendRecvCoord[which[0]] * dims[which[1]] +
                               sendRecvCoord[which[1]]] /* buffer */,
                    1,                /* how may */
                    mpi_local_spinor, /* mpi data type */
    ...
    memory_buffer_fft_field = NULL;
    fft_field = NULL;
    free_spinor_field_array(&memory_buffer_accu_field);
    memory_buffer_accu_field = NULL;
    accu_field = NULL;
    ...
    MPI_Irecv(localSpinorField, 1, mpi_local_spinor, recvRank, MSG_LOCALDATA, g_cart_grid,
              &request);
...
void check_mpi_comm_membership(MPI_Comm commself, MPI_Comm commcheck, const char *name_a,
  ...
  switch (result) {
    case MPI_CONGRUENT:
      fprintf(logFile, "CONGRUENT\n");
      break;
    case MPI_IDENT:
      fprintf(logFile, "IDENTICAL\n");
      break;
    case MPI_SIMILAR:
      fprintf(logFile, "SIMILAR\n");
      break;
    case MPI_UNEQUAL:
      fprintf(logFile, "UNEQUAL\n");
      break;
    default:
      fprintf(logFile, "unknown relation ??\n");
      break;
  }
}
#endif
    ...
    if (remaining_dims[i] == TRUE) {
      switch (i) {
        ...
        case 3:
          ...
          break;
        default:
          fprintf(logFile, " sorry we are in QCD, unknown dimension -> extra dimensions ??");
          break;
      }
    }
  }
