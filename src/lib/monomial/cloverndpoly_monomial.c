...
#pragma omp parallel for
  ...
  for (j = (mnl->MDPolyDegree - 1); j > 0; j--) {
    ...
    H_eo_sw_ndpsi(mnl->w_fields[0], mnl->w_fields[1], g_chi_up_spinor_field[j - 1],
    ...
    H_eo_sw_ndpsi(mnl->w_fields[2], mnl->w_fields[3], g_chi_up_spinor_field[mnl->MDPolyDegree],
                  g_chi_dn_spinor_field[mnl->MDPolyDegree]);
    ...
    sw_spinor_eo(OO, g_chi_up_spinor_field[j - 1], g_chi_dn_spinor_field[mnl->MDPolyDegree],
                 mnl->forcefactor);

    // even/even sites sandwiched by tau_1 gamma_5 Y_e and gamma_5 X_e
    sw_spinor_eo(EE, mnl->w_fields[2], mnl->w_fields[1], mnl->forcefactor);
    // odd/odd sites sandwiched by tau_1 gamma_5 Y_o and gamma_5 X_o
    sw_spinor_eo(OO, g_chi_dn_spinor_field[j - 1], g_chi_up_spinor_field[mnl->MDPolyDegree],
                 mnl->forcefactor);
  }
...
void cloverndpoly_heatbath(const int id, hamiltonian_field_t *const hf) {
  ...
  Qsw_ndpsi(g_chi_up_spinor_field[1], g_chi_dn_spinor_field[1], g_chi_up_spinor_field[0],
  ...
  for (j = 1; j < (mnl->MDPolyDegree); j++) {
    Qsw_tau1_sub_const_ndpsi(up0, dn0, up1, dn1, mnl->MDPolyRoots[mnl->MDPolyDegree - 2 + j],
                             phmc_Cpol, phmc_invmaxev);
    dummy = up1;
    up1 = up0;
    up0 = dummy;
    dummy = dn1;
  ...
  Ptilde_ndpsi(up0, dn0, mnl->PtildeCoefs, mnl->PtildeDegree, up1, dn1, &Qsw_pm_ndpsi);
