...
double nddetratio_acc(const int id, hamiltonian_field_t* const hf) {
  ...
  if (mnl->type == NDDETRATIO) {
    // NDDETRATIO does not have clover term so we need to set c_sw to zero otherwise QUDA will use
    // it
    g_c_sw = 0;
  }
  ...
  if (mnl->solver == MG) {
    ...
    solve_mms_nd(out0, out1, mnl->pf, mnl->pf2, &(mnl->solver_params));
    for (int j = 0; j < mnl->rat.np; j++) {
      out0[j] = NULL;
      out1[j] = NULL;
    }
    ...
    Q_dagger_ndpsi(mnl->w_fields[2], mnl->w_fields[3], mnl->w_fields[0], mnl->w_fields[1]);
  ...
  Q_ndpsi(mnl->w_fields[0], mnl->w_fields[1], mnl->w_fields[2], mnl->w_fields[3]);
  tm_stopwatch_pop(&g_timers, 0, 1, "");
