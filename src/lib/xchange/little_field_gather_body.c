...
void _PSWITCH(little_field_gather)(_C_TYPE *w) {
  int ib;
  _C_TYPE *wt_buf = NULL, *wx_buf = NULL, *wy_buf = NULL, *wz_buf = NULL, *w_buf = NULL,
          *w_source = NULL, *w_dest = NULL;
  ...
  if (g_nproc_t > 1) {
    ...
    request++;
    MPI_Irecv(wt_buf + nb_blocks * g_N_s, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_t_dn, T_UP,
              g_cart_grid, &lrequests[request]);
    request++;
    /* Send t down */
    MPI_Isend(w, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_t_dn, T_DN, g_cart_grid, &lrequests[request]);
    request++;
    MPI_Irecv(wt_buf, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_t_up, T_DN, g_cart_grid,
              &lrequests[request]);
    request++;
  }
  if (g_nproc_x > 1) {
    ...
    request++;
    MPI_Irecv(wx_buf + nb_blocks * g_N_s, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_x_dn, X_UP,
              g_cart_grid, &lrequests[request]);
    request++;
    /* Send x down */
    MPI_Isend(w, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_x_dn, X_DN, g_cart_grid, &lrequests[request]);
    request++;
    MPI_Irecv(wx_buf, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_x_up, X_DN, g_cart_grid,
              &lrequests[request]);
    request++;
  }
  if (g_nproc_y > 1) {
    ...
    request++;
    MPI_Irecv(wy_buf + nb_blocks * g_N_s, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_y_dn, Y_UP,
              g_cart_grid, &lrequests[request]);
    request++;
    /* Send y down */
    MPI_Isend(w, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_y_dn, Y_DN, g_cart_grid, &lrequests[request]);
    request++;
    MPI_Irecv(wy_buf, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_y_up, Y_DN, g_cart_grid,
              &lrequests[request]);
    request++;
  }
  if (g_nproc_z > 1) {
    ...
    request++;
    MPI_Irecv(wz_buf + nb_blocks * g_N_s, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_z_dn, Z_UP,
              g_cart_grid, &lrequests[request]);
    request++;
    /* Send z down */
    MPI_Isend(w, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_z_dn, Z_DN, g_cart_grid, &lrequests[request]);
    request++;
    MPI_Irecv(wz_buf, nb_blocks * g_N_s, _MPI_C_TYPE, g_nb_z_up, Z_DN, g_cart_grid,
              &lrequests[request]);
    request++;
  }
            ...
            switch (pm) {
              ...
              case T_DN: /* Direction -t */
                ...
                if (bt == 0) {
                  ...
                  if (g_nproc_t > 1)
                    w_source = wt_buf + ib + nb_blocks * g_N_s;
                  else
                    w_source = w + ib;
                }
                // got it from the MPI exchange
              ...
              case X_DN: /* Direction -x */
                ...
                if (bx == 0) {
                  ...
                  if (g_nproc_x > 1)
                    w_source = wx_buf + ib + nb_blocks * g_N_s;
                  else
                    w_source = w + ib;
                }
                // got it from the MPI exchange
              ...
              case Y_DN: /* Direction -y */
                ...
                if (by == 0) {
                  ...
                  if (g_nproc_y > 1)
                    w_source = wy_buf + ib + nb_blocks * g_N_s;
                  else
                    w_source = w + ib;
                }
                // got it from the MPI exchange
              ...
              case Z_DN: /* Direction -z */
                ...
                if (bz == 0) {
                  ...
                  if (g_nproc_z > 1)
                    w_source = wz_buf + ib + nb_blocks * g_N_s;
                  else
                    w_source = w + ib;
                }
                // got it from the MPI exchange
            ...
            if ((bt + bx + by + bz) % 2 == eo) {
              ...
              switch (pm) {
                ...
                case X_DN: /* Direction -x */
                  ...
                  if (bx == 0) {
                    ib = index_block_eo[block_index(bt, nblks_x - 1, by, bz)] * g_N_s;
                    if (g_nproc_x > 1)
                      w_source = wx_buf + ib + nb_blocks * g_N_s;
                    else
                      w_source = w + ib;
