...
static void (*boundary_D[8])(spinor *const r, spinor *const s, su3 *u) = {
    boundary_D_0, boundary_D_1, boundary_D_2, boundary_D_3,
...
int init_blocks(const int nt, const int nx, const int ny, const int nz) {
  ...
  if (g_proc_id == 0 && g_debug_level > 0) {
    printf(
        "# Number of deflation blocks per MPI process = %d\n  n_block_t = %d\n  n_block_x = %d\n  "
        "n_block_y = %d\n  n_block_z = %d\n",
        nb_blocks, nblks_t, nblks_x, nblks_y, nblks_z);
  ...
  if ((void *)(basis = (spinor *)aligned_malloc_zero(
                   (nb_blocks + 1) * g_N_s * (VOLUME / nb_blocks + spinpad) * sizeof(spinor))) ==
      NULL) {
    CALLOC_ERROR_CRASH;
  }
  ...
  if ((void *)(bipt__ = (int ***)calloc((T / nblks_t + 2) * (LX / nblks_x + 2), sizeof(int *))) ==
      NULL)
    return (4);
  if ((void *)(bipt_ = (int **)calloc((T / nblks_t + 2) * (LX / nblks_x + 2) * (LY / nblks_y + 2),
                                      sizeof(int *))) == NULL)
    return (3);
  if ((void *)(bipt = (int *)calloc(
                   (T / nblks_t + 2) * (LX / nblks_x + 2) * (LY / nblks_y + 2) * (LZ / nblks_z + 2),
                   sizeof(int))) == NULL)
    return (8);
  if ((void *)(index_block_eo =
                   (int *)calloc(nblks_t * nblks_x * nblks_y * nblks_z, sizeof(int))) == NULL)
    return (8);
  bipt_[0] = bipt;
  bipt__[0] = bipt_;
  block_ipt[0] = bipt__;
  ...
  for (i = 0; i < nb_blocks; ++i) {
    ...
    for (j = 0; j < 4; j++) {
      block_list[i].coordinate[j] =
          nblks_dir[j] * g_proc_coords[j] + block_list[i].mpilocal_coordinate[j];
    }
    ...
    if (g_proc_id == 0 && g_debug_level > 4) {
      printf("# Block id %d even odd id %d coordinate (%d %d %d %d)\n", i, block_list[i].evenodd,
             block_list[i].coordinate[0], block_list[i].coordinate[1], block_list[i].coordinate[2],
             block_list[i].coordinate[3]);
    }
    for (j = 0; j < g_N_s; j++) {
    ...
    if ((void *)(block_list[i].little_dirac_operator =
                     aligned_malloc_zero(9 * g_N_s * g_N_s * sizeof(_Complex double))) == NULL)
      CALLOC_ERROR_CRASH;
    if ((void *)(block_list[i].little_dirac_operator_32 =
                     aligned_malloc_zero(9 * g_N_s * g_N_s * sizeof(_Complex float))) == NULL)
      CALLOC_ERROR_CRASH;
    if ((void *)(block_list[i].little_dirac_operator_eo =
                     aligned_malloc_zero(9 * g_N_s * g_N_s * sizeof(_Complex double))) == NULL)
      CALLOC_ERROR_CRASH;
    if ((void *)(block_list[i].little_dirac_operator_eo_32 =
                     aligned_malloc_zero(9 * g_N_s * g_N_s * sizeof(_Complex float))) == NULL)
      CALLOC_ERROR_CRASH;
  }
            ...
            for (int bx = 0; bx < nblks_x; bx++) {
              for (int by = 0; by < nblks_y; by++) {
                for (int bz = 0; bz < nblks_z; bz++) {
                  ...
                  memcpy(block_list[i].u + ix_new + 1, &g_gauge_field[g_idn[ix][0]][0],
                         sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 2, &g_gauge_field[ix][1], sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 3, &g_gauge_field[g_idn[ix][1]][1],
                         sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 4, &g_gauge_field[ix][2], sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 5, &g_gauge_field[g_idn[ix][2]][2],
                         sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 6, &g_gauge_field[ix][3], sizeof(su3));
                  memcpy(block_list[i].u + ix_new + 7, &g_gauge_field[g_idn[ix][3]][3],
                         sizeof(su3));
                  i++;
                }
              }
            ...
            for (int bx = 0; bx < nblks_x; bx++) {
              for (int by = 0; by < nblks_y; by++) {
                for (int bz = 0; bz < nblks_z; bz++) {
                  ...
                  memcpy(block_list[i].u_32 + ix_new + 1, &g_gauge_field_32[g_idn[ix][0]][0],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 2, &g_gauge_field_32[ix][1], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 3, &g_gauge_field_32[g_idn[ix][1]][1],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 4, &g_gauge_field_32[ix][2], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 5, &g_gauge_field_32[g_idn[ix][2]][2],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 6, &g_gauge_field_32[ix][3], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ix_new + 7, &g_gauge_field_32[g_idn[ix][3]][3],
                         sizeof(su3_32));
                  i++;
                }
              }
            ...
            for (int bx = 0; bx < nblks_x; bx++) {
              for (int by = 0; by < nblks_y; by++) {
                for (int bz = 0; bz < nblks_z; bz++) {
                  ...
                  memcpy(block_list[i].u_32 + ixeo + 1, &g_gauge_field_32[g_idn[ix][0]][0],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 2, &g_gauge_field_32[ix][1], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 3, &g_gauge_field_32[g_idn[ix][1]][1],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 4, &g_gauge_field_32[ix][2], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 5, &g_gauge_field_32[g_idn[ix][2]][2],
                         sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 6, &g_gauge_field_32[ix][3], sizeof(su3_32));
                  memcpy(block_list[i].u_32 + ixeo + 7, &g_gauge_field_32[g_idn[ix][3]][3],
                         sizeof(su3_32));
                  i++;
                }
              }
...
int check_blocks_geometry(block *blk) {
  ...
  if (itest[blk->volume + blk->spinpad - 1] !=
      2 * (blk->BLX * blk->BLY * blk->BLZ + blk->BT * blk->BLX * blk->BLY +
           blk->BT * blk->BLY * blk->BLZ + blk->BT * blk->BLX * blk->BLZ)) {
    if (g_proc_id == 0) {
      printf("error in block geometry, boundary points wrong %d != %d\n",
             itest[blk->volume + blk->spinpad - 1],
             2 * (blk->BLX * blk->BLY * blk->BLZ + blk->BT * blk->BLX * blk->BLY +
                  blk->BT * blk->BLY * blk->BLZ + blk->BT * blk->BLX * blk->BLZ));
    }
  }
          ...
          if (t != T / nblks_t - 1) {
            if (*ipt != block_ipt[t + 1][x][y][z] && g_proc_id == 0)
              printf("Shit +t! %d %d %d %d %d != %d at %d\n", t, x, y, z, *ipt,
                     block_ipt[t + 1][x][y][z], i);
          } else if (*ipt != VOLUME / nb_blocks)
            printf("Shit +t! %d %d %d %d %d != %d at %d\n", t, x, y, z, *ipt, VOLUME / nb_blocks,
                   i);
          ipt++;
          if (t != 0) {
            if (*ipt != block_ipt[t - 1][x][y][z] && g_proc_id == 0)
          ...
          if ((x + y + z + t) % 2 == 0) {
            ...
            if (t != T / nblks_t - 1) {
              if (*ipt != block_ipt[t + 1][x][y][z] / 2 && g_proc_id == 0)
                printf("Shit +t! (%d %d %d %d): %d != %d at %d\n", t, x, y, z, *ipt,
                       block_ipt[t + 1][x][y][z] / 2, i);
            } else if (*ipt != VOLUME / nb_blocks / 2)
              printf("Shit +t! (%d %d %d %d): %d != %d at %d\n", t, x, y, z, *ipt,
                     VOLUME / nb_blocks / 2, i);
            ipt++;
            if (t != 0) {
              if (*ipt != block_ipt[t - 1][x][y][z] / 2 && g_proc_id == 0)
                printf("Shit -t! (%d %d %d %d): %d != %d at %d\n", t, x, y, z, *ipt,
...
int init_blocks_geometry() {
  ...
  for (int ix = 0; ix < VOLUME / nb_blocks; ++ix) {
    ...
    int eo = ((ix % dZ) + (ix / ystride) % dY + (ix / (xstride)) % dX + ix / (tstride)) % 2;
    if (eo == 0) {
  ...
  if (g_debug_level > 4) {
    for (i = 0; i < g_N_s; i++) {
      for (j = 0; j < g_N_s; j++) {
        coeff = scalar_prod(parent->basis[i], parent->basis[j], parent->volume, 0);
        if (g_proc_id == 0)
          printf("basis id = %d <%d, %d> = %1.3e +i %1.3e\n", parent->id, j, i, creal(coeff),
                 cimag(coeff));
      }
    }
  }
...
void block_contract_basis(int const idx, int const vecnum, int const dir, spinor *const psi) {
  int l;
  for (l = 0; l < g_N_s; ++l) {
    block_list[idx].little_dirac_operator[dir * g_N_s * g_N_s + vecnum * g_N_s + l] = scalar_prod(
        block_list[idx].basis[l], psi + idx * (VOLUME / nb_blocks + 1), VOLUME / nb_blocks, 0);
  }
}
      ...
      for (int i = 0 * g_N_s; i < 9 * g_N_s; ++i) {
        ...
        for (int j = 0; j < g_N_s; ++j) {
          printf("%s%1.3e %s %1.3e i",
                 ...
                 creal(block_list[0].little_dirac_operator[i * g_N_s + j]) >= 0
                     ? creal(block_list[0].little_dirac_operator[i * g_N_s + j])
                     : -creal(block_list[0].little_dirac_operator[i * g_N_s + j]),
                 cimag(block_list[0].little_dirac_operator[i * g_N_s + j]) >= 0 ? "+" : "-",
                 cimag(block_list[0].little_dirac_operator[i * g_N_s + j]) >= 0
                     ? cimag(block_list[0].little_dirac_operator[i * g_N_s + j])
                     : -cimag(block_list[0].little_dirac_operator[i * g_N_s + j]));
          if (j != g_N_s - 1) {
            printf(",\t");
          }
      ...
      for (int i = 0 * g_N_s; i < 9 * g_N_s; ++i) {
        ...
        for (int j = 0; j < g_N_s; ++j) {
          printf("%s%1.3e %s %1.3e i",
                 ...
                 creal(block_list[1].little_dirac_operator[i * g_N_s + j]) >= 0
                     ? creal(block_list[1].little_dirac_operator[i * g_N_s + j])
                     : -creal(block_list[1].little_dirac_operator[i * g_N_s + j]),
                 cimag(block_list[1].little_dirac_operator[i * g_N_s + j]) >= 0 ? "+" : "-",
                 cimag(block_list[1].little_dirac_operator[i * g_N_s + j]) >= 0
                     ? cimag(block_list[1].little_dirac_operator[i * g_N_s + j])
                     : -cimag(block_list[1].little_dirac_operator[i * g_N_s + j]));
          if (j != g_N_s - 1) {
            printf(",\t");
          }
    ...
    for (int blk = 0; blk < nb_blocks; blk++) {
      ...
      for (int i = 0; i < g_N_s; i++) {
        ...
        for (int j = 0; j < g_N_s; j++) {
          ...
          block_list[blk].little_dirac_operator_32[i * g_N_s + j] =
              (_Complex float)M[i * g_N_s + j];

          block_list[block_list[blk].evenodd_id].little_dirac_operator_eo[i * g_N_s + j] =
              M[i * g_N_s + j];
        }
      }
    }
...
#pragma omp for
        ...
        for (int block_id = 0; block_id < nb_blocks; block_id++) {
          ...
          bx = ((block_id - bz - by * nblks_z) % (nblks_x * nblks_y * nblks_z)) /
               (nblks_z * nblks_y);
          bt = (block_id - bz - by * nblks_z - bx * nblks_z * nblks_y) /
               (nblks_z * nblks_y * nblks_x);
            ...
            for (int x = x_start; x < x_end; x++) {
              for (int y = y_start; y < y_end; y++) {
                for (int z = z_start; z < z_end; z++) {
                  ...
                  // Other cases are treated in a standard way using the boundary of the scracth
                  // array
...
int split_global_field_GEN_ID(block *const block_list, const int id, spinor *const field,
                              const int nb_blocks) {
  int j, ctr_t = 0;
  int x, y, z, t;
  int bx, by, bz, bt, block_id;
  ...
  if (g_proc_id == 0 && g_debug_level > 8) {
    for (j = 0; j < nb_blocks; j++)
      printf("Basis norm %2d = %1.3e\n", j,
             square_norm(block_list[j].basis[id], VOLUME / nb_blocks, 0));
  }
  return 0;
}
...
void copy_global_to_block_eo(spinor *const beven, spinor *const bodd, spinor *const globalfield,
                             const int blk) {
  int t, x, y, z;
  int i, it, ix, iy, iz;
  int even = 0, odd = 0;
...
void copy_block_eo_to_global(spinor *const globalfield, spinor *const beven, spinor *const bodd,
                             const int blk) {
  int t, x, y, z;
  int i, it, ix, iy, iz;
  int even = 0, odd = 0;
...
void reconstruct_global_field_GEN(spinor *const rec_field, spinor **const psi,
                                  const int nb_blocks) {
#ifdef TM_USE_OMP
#pragma omp parallel for
#endif
  for (int ix = 0; ix < dT * dX * dY * dZ; ix++) {
    ...
    for (int block_id = 0; block_id < nb_blocks; block_id++) {
      ...
      int bx =
          ((block_id - bz - by * nblks_z) % (nblks_x * nblks_y * nblks_z)) / (nblks_z * nblks_y);
      int bt =
          (block_id - bz - by * nblks_z - bx * nblks_z * nblks_y) / (nblks_z * nblks_y * nblks_x);
...
/* Reconstructs a global field from the little basis of nb_blocks blocks taken from
 * block_list[*].basis[id] */
void reconstruct_global_field_GEN_ID(spinor *const rec_field, block *const block_list, const int id,
                                     const int nb_blocks) {
#ifdef TM_USE_OMP
#pragma omp parallel for
#endif
...
void add_eo_block_to_global(spinor *const globalfield, spinor *const beven, spinor *const bodd,
                            const int blk) {
  int t, x, y, z;
  int i, it, ix, iy, iz;
  int even = 0, odd = 0;
...
void add_eo_block_32_to_global(spinor *const globalfield, spinor32 *const beven,
                               spinor32 *const bodd, const int blk) {
  int i, it, ix, iy, iz;
  int even = 0, odd = 0;
  spinor32 *tmp = NULL;
