...
void invert_overlap(const int op_id, const int index_start) {
  ...
  if (optr->solver == 13) {
    optr->iterations =
        sumr(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], optr->maxiter, optr->eps_sq);
    ...
    if (use_preconditioning == 1 && g_precWS != NULL) {
      ...
      /* 	iter = cg_her(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI+1], max_iter,
       * precision,  */
      /* 		    rel_prec, VOLUME, &Q_pm_psi_prec); */
      optr->iterations =
          cg_her(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1], optr->maxiter,
                 optr->eps_sq, optr->rel_prec, VOLUME, &Qov_sq_psi_prec);
      ...
      optr->iterations = cg_her(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1],
                                optr->maxiter, optr->eps_sq, optr->rel_prec, VOLUME, &Qov_sq_psi);
    }
