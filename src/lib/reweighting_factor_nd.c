...
double reweighting_factor_nd(const int N, const int repro) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    double temp1 = phmc_ptilde_cheby_coef[0];
    phmc_ptilde_cheby_coef[0] = temp1 - 1;

    Ptilde_ndpsi(g_chi_up_spinor_field[3], g_chi_dn_spinor_field[3], phmc_ptilde_cheby_coef,
                 phmc_ptilde_n_cheby, g_chi_up_spinor_field[2], g_chi_dn_spinor_field[2],
                 &Qtm_pm_ndpsi);
