...
void _setOneFlavourSolverParam(const double kappa, const double c_sw, const double mu,
                               const int solver_type, const int even_odd, const double eps_sq,
                               const int maxiter, const int single_parity_solve, const int QpQm);
...
void _setDefaultQudaParam(void) {
  ...
  if (g_debug_level > 3) inv_param.verbosity_precondition = QUDA_SUMMARIZE;
  if (g_debug_level > 5) inv_param.verbosity_precondition = QUDA_VERBOSE;
  ...
  inv_param.gamma_basis =
      QUDA_CHIRAL_GAMMA_BASIS;  // CHIRAL -> UKQCD does not seem to be supported right now...
...
void _initQuda() {
  if (quda_initialized) return;

  if (g_debug_level > 0)
    if (g_proc_id == 0)
      printf("\n# TM_QUDA: Detected QUDA version %d.%d.%d\n\n", QUDA_VERSION_MAJOR,
             QUDA_VERSION_MINOR, QUDA_VERSION_SUBMINOR);
  if (QUDA_VERSION_MAJOR == 0 && QUDA_VERSION_MINOR < 7) {
    fprintf(stderr,
            "Error: minimum QUDA version required is 0.7.0 (for support of chiral basis and "
            "removal of bug in mass normalization with preconditioning).\n");
    exit(-2);
  }
  ...
  if (quda_input.enable_device_memory_pool) {
    ...
    tm_debug_printf(0, 0,
                    "# TM_QUDA: Setting environment variable QUDA_ENABLE_DEVICE_MEMORY_POOL=1\n");
  } else {
    setenv("QUDA_ENABLE_DEVICE_MEMORY_POOL", "0", 1);
    tm_debug_printf(0, 0,
                    "# TM_QUDA: Setting environment variable QUDA_ENABLE_DEVICE_MEMORY_POOL=0\n");
  }

  if (quda_input.enable_pinned_memory_pool) {
    setenv("QUDA_ENABLE_PINNED_MEMORY_POOL", "1", 1);
    tm_debug_printf(0, 0,
                    "# TM_QUDA: Setting environment variable QUDA_ENABLE_PINNED_MEMORY_POOL=1\n");
  } else {
...
void _endQuda() {
  if (quda_initialized) {
    ...
    freeGaugeQuda();
    freeCloverQuda();  // this is safe even if there is no Clover field loaded, at least it was in
                       // QUDA v0.7.2
        ...
        for (int x2 = 0; x2 < LY; x2++)
          ...
          memcpy(&(gauge_quda[3][quda_idx]), &(gaugefield[tm_idx][0]), 18 * gSize);
            ...
            if (compression == NO_COMPRESSION && quda_input.fermionbc == TM_QUDA_THETABC) {
              ...
              // when compression is not used, we can still force naive anti-periodic boundary
              // conditions
            } else {
              if (quda_input.fermionbc == TM_QUDA_APBC &&
                  x0 + g_proc_coords[0] * T == g_nproc_t * T - 1) {
                for (int i = 0; i < 18; i++) {
    ...
    for (int x1 = 0; x1 < LX; x1++)
      for (int x2 = 0; x2 < LY; x2++)
        for (int x3 = 0; x3 < LZ; x3++) {
          ...
          if (doublet) {
            memcpy(&(sp[24 * (oddBit * VOLUME + j / 2)]), &(tempSpinor[24 * tm_idx]),
                   24 * sizeof(double));
            memcpy(&(sp[24 * (oddBit * VOLUME + j / 2 + VOLUME / 2)]),
                   &(tempSpinor[24 * (tm_idx + VOLUME)]), 24 * sizeof(double));
          } else {
            memcpy(&(sp[24 * (oddBit * VOLUME / 2 + j / 2)]), &(tempSpinor[24 * tm_idx]),
                   24 * sizeof(double));
          }
        }
    ...
    for (int x1 = 0; x1 < LX; x1++)
      for (int x2 = 0; x2 < LY; x2++)
        for (int x3 = 0; x3 < LZ; x3++) {
          ...
          if (doublet) {
            memcpy(&(sp[24 * tm_idx]), &(tempSpinor[24 * (oddBit * VOLUME + j / 2)]),
                   24 * sizeof(double));
            memcpy(&(sp[24 * (tm_idx + VOLUME)]),
                   &(tempSpinor[24 * (oddBit * VOLUME + j / 2 + VOLUME / 2)]), 24 * sizeof(double));
          } else {
            memcpy(&(sp[24 * tm_idx]), &(tempSpinor[24 * (oddBit * VOLUME / 2 + j / 2)]),
                   24 * sizeof(double));
          }
        }
  tm_stopwatch_pop(&g_timers, 0, 0, "TM_QUDA");
              ...
              for (int col = 0; col < 3; col++) {
                for (int reim = 0; reim < 2; reim++) {
                  sp[24 * q_eo_idx + 6 * q_spin + 2 * col + reim] =
                      change_sign[q_spin] *
                      tempSpinor[24 * tm_eo_idx + 6 * tm_spin + 2 * col + reim];
                  if (doublet) {
                    sp[24 * (q_eo_idx + Vh) + 6 * q_spin + 2 * col + reim] =
                        change_sign[q_spin] *
                        tempSpinor[24 * (tm_eo_idx + Vh) + 6 * tm_spin + 2 * col + reim];
                  }
                }
              }
            }
                ...
                for (int reim = 0; reim < 2; reim++) {
                  ...
                  if (doublet) {
                    sp[24 * (tm_eo_idx + Vh) + 6 * tm_spin + 2 * col + reim] =
                        change_sign[q_spin] *
                        tempSpinor[24 * (q_eo_idx + Vh) + 6 * q_spin + 2 * col + reim];
                  }
                }
              }
            }
...
void set_boundary_conditions(CompressionType *compression, QudaGaugeParam *gauge_param) {
  ...
  if (fabs(X1) > 0.0 || fabs(X2) > 0.0 || fabs(X3) > 0.0 ||
      (fabs(X0) > 2 * DBL_EPSILON && fabs(fabs(X0) - 1.0) > 2 * DBL_EPSILON)) {
    if (*compression != NO_COMPRESSION) {
      if (g_proc_id == 0) {
        printf(
            "\n# TM_QUDA: WARNING you can't use compression %d with boundary conditions for "
            "fermion fields (t,x,y,z)*pi: (%f,%f,%f,%f) \n",
...
void set_sloppy_prec(const SloppyPrecision sloppy_precision,
                     const SloppyPrecision refinement_precision, QudaGaugeParam *gauge_param,
                     QudaInvertParam *inv_param) {
  // choose sloppy prec.
...
int invert_quda_direct(double *const propagator, double const *const source, const int op_id) {
  tm_stopwatch_push(&g_timers, __func__, "");
  ...
  set_sloppy_prec(optr->sloppy_precision, optr->solver_params.refinement_precision, &gauge_param,
                  &inv_param);
  ...
  _setOneFlavourSolverParam(optr->kappa, optr->c_sw, optr->mu, optr->solver, optr->even_odd_flag,
                            optr->eps_sq, optr->maxiter, 0, 0);
  ...
  if (optr->iterations >= optr->maxiter) return (-1);

  return (optr->iterations);
}

int invert_eo_quda(spinor *const Even_new, spinor *const Odd_new, spinor *const Even,
                   ...
                   solver_params_t solver_params, SloppyPrecision sloppy_precision,
...
int invert_doublet_eo_quda(spinor *const Even_new_s, spinor *const Odd_new_s,
  ...
  _setTwoFlavourSolverParam(g_kappa, g_c_sw, g_mubar, g_epsbar, solver_flag, even_odd_flag,
                            precision, max_iter, 0 /* not a single parity solve */,
                            0 /* not a QpQm solve */);
...
void M_full_quda(spinor *const Even_new, spinor *const Odd_new, spinor *const Even,
                 spinor *const Odd) {
  inv_param.kappa = g_kappa;
...
void M_quda(spinor *const P, spinor *const Q) {
  ...
  // inv_param.Ls=1;
  // custom
  _setOneFlavourSolverParam(g_kappa, g_c_sw, g_mu,
                            1,  // solver flag
                            1,  // even_odd
                            1e-12, 1000, 1, 0);
...
void _setOneFlavourSolverParam(const double kappa, const double c_sw, const double mu,
  ...
  if (solver_type == BICGSTAB) {
    if (g_proc_id == 0) {
      printf("# TM_QUDA: Using BiCGstab!\n");
      fflush(stdout);
    }
    inv_param.inv_type = QUDA_BICGSTAB_INVERTER;
  } else if (solver_type == MG) {
    if (g_proc_id == 0) {
      printf("# TM_QUDA: Using MG!\n");
      fflush(stdout);
    }
...
void _updateQudaMultigridPreconditioner() {
  if (check_quda_mg_setup_state(&quda_mg_setup_state, &quda_gauge_state, &quda_clover_state,
                                &quda_input) == TM_QUDA_MG_SETUP_RESET) {
    ...
    if (fabs(quda_input.mg_setup_2kappamu) > 2 * DBL_EPSILON &&
        fabs(fabs(quda_input.mg_setup_2kappamu / 2.0 / g_kappa) -
             fabs(quda_mg_param.invert_param->mu)) > 2 * DBL_EPSILON) {
  ...
  } else if (check_quda_mg_setup_state(&quda_mg_setup_state, &quda_gauge_state, &quda_clover_state,
                                       &quda_input) == TM_QUDA_MG_SETUP_REFRESH) {
    ...
    if (quda_mg_setup_state.force_refresh) {
      quda_mg_setup_state_update(&quda_mg_setup_state, &quda_gauge_state, &quda_clover_state, g_mu,
                                 g_kappa, g_c_sw);
    } else {
      set_quda_mg_setup_state(&quda_mg_setup_state, &quda_gauge_state, &quda_clover_state);
    }
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, "TM_QUDA");
                                       ...
                                       &quda_input) == TM_QUDA_MG_SETUP_UPDATE) {
...
void _setTwoFlavourSolverParam(const double kappa, const double c_sw, const double mu,
  ...
  if (inv_param.inv_type_precondition == QUDA_MG_INVERTER) {
    tm_debug_printf(
        0, 0,
        "# TM_QUDA: using MG solver to invert operator with 2kappamu = %.12f 2kappaeps = %.12f\n",
        mu, epsilon);
    quda_mg_param.invert_param = &mg_inv_param;
    _setQudaMultigridParam(&quda_mg_param);
    _updateQudaMultigridPreconditioner();
...
void _setMGInvertParam(QudaInvertParam *mg_inv_param, const QudaInvertParam *const inv_param) {
  ...
  } else if (inv_param->matpc_type == QUDA_MATPC_EVEN_EVEN_ASYMMETRIC ||
             inv_param->matpc_type == QUDA_MATPC_EVEN_EVEN ||
...
void _setQudaMultigridParam(QudaMultigridParam *mg_param) {
  ...
  for (int level = 0; level < mg_param->n_level; level++) {
    for (int dim = 0; dim < 4; dim++) {
      ...
      if (level < mg_param->n_level - 1 &&
        ...
        tm_debug_printf(0, 0,
                        ...
                        "Adjust your block sizes or parallelisation, all local lattice extents on "
                        "all levels must be even!\n",
    ...
    // when the Schwarz-alternating smoother is used, this can be set to NO, otherwise it must be
    // YES
    ...
    mg_param->coarse_grid_solution_type[level] =
        (level == 0 && inv_param.solve_type == QUDA_DIRECT_SOLVE) ? QUDA_MAT_SOLUTION
                                                                  : QUDA_MATPC_SOLUTION;
    ...
    mg_param->smoother_solver_ca_lambda_min[level] =
        quda_input.mg_smoother_solver_ca_lambda_min[level];
    mg_param->smoother_solver_ca_lambda_max[level] =
        quda_input.mg_smoother_solver_ca_lambda_max[level];
    ...
    if (quda_input.mg_use_eig_solver[level] == QUDA_BOOLEAN_YES) {
      ...
      mg_eig_param[level].n_conv =
          quda_input.mg_eig_nEv[level];  // require convergence of all eigenvalues
      mg_eig_param[level].n_ev_deflate =
          mg_eig_param[level].n_conv;  // deflate all converged eigenvalues
...
int invert_eo_degenerate_quda(spinor *const out, spinor *const in, const double precision,
  ...
  _setOneFlavourSolverParam(
      g_kappa, g_c_sw, g_mu, solver_flag, even_odd_flag,
  ...
  if (iterations >= max_iter) {
    if (solver_flag == MG) {
      ...
      if (quda_mg_setup_was_force_refreshed == 0) {
        tm_debug_printf(0, 0,
                        "# TM_QUDA: MG did not converge in %d iterations, force-refreshing setup "
                        "and trying again!\n",
                        max_iter);
        ...
        if (ret_value >= max_iter) {
          ...
          char errmsg[200];
          snprintf(errmsg, 200,
                   "QUDA-MG solver failed to converge in %d iterations even after forced setup "
                   "refresh. Terminating!",
                   max_iter);
...
int invert_eo_quda_oneflavour_mshift(spinor **const out, spinor *const in, const double precision,
                                     const int max_iter, const int solver_flag, const int rel_prec,
                                     const int even_odd_flag, solver_params_t solver_params,
  ...
  _setOneFlavourSolverParam(g_kappa, g_c_sw, g_mu, solver_flag, even_odd_flag, precision, max_iter,
                            1 /*single_parity_solve */, 1 /*always QpQm*/);
...
int invert_eo_quda_twoflavour_mshift(spinor **const out_up, spinor **const out_dn,
  ...
  _setTwoFlavourSolverParam(g_kappa, g_c_sw, g_mubar, g_epsbar, solver_flag, even_odd_flag,
                            precision, max_iter, 1 /*single_parity_solve */, 1 /*always QpQm*/);
...
#pragma omp parallel for collapse(2)
  ...
  for (size_t v = 0; v < VOLUME; v++)
    for (int mu = 0; mu < 4; mu++) {
      ...
      der[v][mu].d8 += -(2 * c22 - c00 - c11) *
                       0.288675134594813;  // imag (2*22 - 00 - 11)/sqrt(3)/2
...
void compute_gauge_derivative_quda(monomial *const mnl, hamiltonian_field_t *const hf) {
  ...
  for (int i = 0; i < num_paths; i++) {
    ...
    if (i < 6)
      loop_coeff[i] =
          -0.66666666666 * g_beta * mnl->c0;  // 2/3 conversion factor to match tmLQCD results
    // Rect coeffs
    else
...
int get_lvl_tuning_steps(const tm_QudaMGTuningPlan_t *const tuning_plan, const int lvl) {
  return (tuning_plan->mg_mu_factor_steps[lvl] > 0 ? tuning_plan->mg_mu_factor_steps[lvl] : 1) *
         ...
         (tuning_plan->mg_smoother_tol_steps[lvl] > 0 ? tuning_plan->mg_smoother_tol_steps[lvl]
                                                      : 1) *
...
void update_tunable_params(tm_QudaMGTunableParams_t *tunable_params,
                           ...
                           const tm_QudaMGTuningDirection_t tuning_dir, const int lvl) {
  switch (tuning_dir) {
    case TM_MG_TUNE_MU_FACTOR: {
    ...
    case TM_MG_TUNE_COARSE_SOLVER_MAXITER: {
      if (tunable_params->mg_coarse_solver_maxiter[lvl] +
              tuning_plan->mg_coarse_solver_maxiter_delta[lvl] >=
          0) {
        tunable_params->mg_coarse_solver_maxiter[lvl] +=
            tuning_plan->mg_coarse_solver_maxiter_delta[lvl];
      }
      break;
    }
    case TM_MG_TUNE_COARSE_SOLVER_TOL: {
      if (tunable_params->mg_coarse_solver_tol[lvl] + tuning_plan->mg_coarse_solver_tol_delta[lvl] >
          0) {
...
void print_dbl_array(const double *const arr, const int n_elem) {
  for (int i = 0; i < n_elem; i++) {
    printf("%s%.6f%s", i == 0 ? "(" : "", arr[i], i < (n_elem - 1) ? ", " : ")");
  }
}

void print_int_array(const int *const arr, const int n_elem) {
  for (int i = 0; i < n_elem; i++) {
    printf("%s%d%s", i == 0 ? "(" : "", arr[i], i < (n_elem - 1) ? ", " : ")");
  }
}
...
void print_tunable_params_pair(const tm_QudaMGTunableParams_t *const old,
                               const tm_QudaMGTunableParams_t *const new, const int n_level) {
...
void print_tunable_params(const tm_QudaMGTunableParams_t *const par, const int n_level) {
  if (g_proc_id == 0) {
    printf("\n");
    printf("%25s: ", "mg_mu_factor");
...
int find_best_params(const tm_QudaMGTuningPlan_t *const tuning_plan,
                     const tm_QudaMGTunableParams_t *const par_arr, const int n, const int n_level,
                     const int print) {
...
tm_QudaMGTuningDirection_t update_tuning_dir(const tm_QudaMGTuningPlan_t *const tuning_plan,
                                             ...
                                             const int lvl, const int cur_dir_steps_done) {
  for (int i = (int)tuning_dir; i < TM_MG_TUNE_INVALID; i++) {
    switch (i) {
      case TM_MG_TUNE_MU_FACTOR:
        if ((i == (int)tuning_dir && tuning_plan->mg_mu_factor_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_mu_factor_steps[lvl] > 0))
          return TM_MG_TUNE_MU_FACTOR;
        break;
      case TM_MG_TUNE_COARSE_SOLVER_TOL:
        if ((i == (int)tuning_dir &&
             tuning_plan->mg_coarse_solver_tol_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_coarse_solver_tol_steps[lvl] > 0))
          return TM_MG_TUNE_COARSE_SOLVER_TOL;
        break;
      case TM_MG_TUNE_COARSE_SOLVER_MAXITER:
        if ((i == (int)tuning_dir &&
             tuning_plan->mg_coarse_solver_maxiter_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_coarse_solver_maxiter_steps[lvl] > 0))
          return TM_MG_TUNE_COARSE_SOLVER_MAXITER;
        break;
      case TM_MG_TUNE_NU_POST:
        if ((i == (int)tuning_dir && tuning_plan->mg_nu_post_steps[lvl] > cur_dir_steps_done) ||
            (i != tuning_dir && tuning_plan->mg_nu_post_steps[lvl] > 0))
          return TM_MG_TUNE_NU_POST;
        break;
      case TM_MG_TUNE_NU_PRE:
        if ((i == (int)tuning_dir && tuning_plan->mg_nu_pre_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_nu_pre_steps[lvl] > 0))
          return TM_MG_TUNE_NU_PRE;
        break;
      case TM_MG_TUNE_SMOOTHER_TOL:
        if ((i == (int)tuning_dir &&
             tuning_plan->mg_smoother_tol_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_smoother_tol_steps[lvl] > 0))
          return TM_MG_TUNE_SMOOTHER_TOL;
        break;
      case TM_MG_TUNE_OMEGA:
        if ((i == (int)tuning_dir && tuning_plan->mg_omega_steps[lvl] > cur_dir_steps_done) ||
            (i != (int)tuning_dir && tuning_plan->mg_omega_steps[lvl] > 0))
...
void quda_mg_tune_params(void *spinorOut, void *spinorIn, const int max_iter) {
  ...
  tm_QudaMGTunableParams_t *tunable_params =
      calloc(quda_mg_tuning_plan.mg_tuning_iterations, sizeof(tm_QudaMGTunableParams_t));
  ...
  for (i = 1; i < quda_mg_tuning_plan.mg_tuning_iterations; i++) {
    ...
    cur_tuning_dir = update_tuning_dir(&quda_mg_tuning_plan, cur_tuning_dir, cur_tuning_lvl,
    ...
    update_tunable_params(&tunable_params[i], &quda_mg_tuning_plan, cur_tuning_dir, cur_tuning_lvl);
    ...
    if (tunable_params[i].tts / tunable_params[best_idx].tts >
            quda_mg_tuning_plan.mg_tuning_tolerance &&
...
void compute_cloverdet_derivative_quda(monomial *const mnl, hamiltonian_field_t *const hf,
  ...
  double coeff[1] = {4. * mnl->kappa * mnl->kappa};  // minus because in p(QUDA)=-Y (tmLQCD) , the
                                                     // factor 4 is experimentally observed
...
void eigsolveQuda(_Complex double *evals, int n_evals, double tol, int blksize, int blkwise,
                  int max_iterations, int maxmin, const double precision, const int max_iter,
                  const int polydeg, const double amin, const double amax, const int n_kr,
                  const int solver_flag, const int rel_prec, const int even_odd_flag,
                  const SloppyPrecision refinement_precision, SloppyPrecision sloppy_precision,
                  CompressionType compression, const int oneFlavourFlag) {
  tm_stopwatch_push(&g_timers, __func__, "");
  ...
  if (oneFlavourFlag) {
    _setOneFlavourSolverParam(g_kappa, g_c_sw, g_mu, solver_flag, even_odd_flag, precision,
                              max_iter, 1 /*single_parity_solve */, 1 /*always QpQm*/);
  } else {
    _setTwoFlavourSolverParam(g_kappa, g_c_sw, g_mubar, g_epsbar, solver_flag, even_odd_flag,
                              precision, max_iter, 1 /*single_parity_solve */, 1 /*always QpQm*/);
  }
