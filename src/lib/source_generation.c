...
void extended_pion_source(spinor* const P, spinor* const Q, spinor* const R, spinor* const S,
                          const int t0, const int ts, const double px, const double py,
                          const double pz) {
...
void source_generation_pion_only(spinor* const P, spinor* const Q, const int t, const int sample,
...
void source_generation_pion_zdir(spinor* const P, spinor* const Q, const int z, const int sample,
                                 const int nstore) {
...
void source_generation_nucleon(spinor* const P, spinor* const Q, const int is, const int ic,
                               const int t, const int nt, const int nx, const int sample,
                               const int nstore, const int meson) {
...
void full_source_spinor_field_point(spinor* const full_spinor, const int is, const int ic,
...
void eo_source_spinor_field_point(spinor* const even_cb_spinor, spinor* const odd_cb_spinor,
  ...
  if ((global_txyz_src_pos[0] >= g_nproc_t * T || global_txyz_src_pos[0] < 0) ||
    ...
    snprintf(error_message, 500,
             ...
             global_txyz_src_pos[0], global_txyz_src_pos[1], global_txyz_src_pos[2],
  ...
  if ((g_proc_coords[0] == global_txyz_src_pos[0] / T) &&
    ...
    const int idx = g_ipt[global_txyz_src_pos[0] % T][global_txyz_src_pos[1] % LX]
                         [global_txyz_src_pos[2] % LY][global_txyz_src_pos[3] % LZ];

    const int eo_idx = g_lexic2eosub[idx];

    spinor* s = (g_lexic2eo[idx] < VOLUME / 2 ? even_cb_spinor : odd_cb_spinor) + eo_idx;
...
void full_source_spinor_field_spin_diluted_oet_ts(spinor* const full_spinor, const int src_ts,
                                                  const int src_d, const int sample,
                                                  const int nstore, const unsigned int oet_seed) {
  spinor** temp_cb_spinors;
  init_solver_field(&temp_cb_spinors, VOLUME / 2, 2);
  eo_source_spinor_field_spin_diluted_oet_ts(temp_cb_spinors[0], temp_cb_spinors[1], src_ts, src_d,
                                             sample, nstore, oet_seed);
  convert_eo_to_lexic(full_spinor, temp_cb_spinors[0], temp_cb_spinors[1]);
  finalize_solver(temp_cb_spinors, 2);
}

void eo_source_spinor_field_spin_diluted_oet_ts(spinor* const even_cb_spinor,
                                                spinor* const odd_cb_spinor, const int src_ts,
                                                const int src_d, const int sample, const int nstore,
