...
void six_invert(int* ifail, _Complex double a[6][6]) {
  ...
  *ifail = 0;
  for (k = 0; k < nm1; ++k) {
    s = 0.0;
    for (j = k + 1; j <= nm1; ++j) s += conj(a[j][k]) * a[j][k];
    ...
    for (j = k + 1; j <= nm1; ++j) {
      z = 0.0;
      for (i = k; i <= nm1; ++i) z += conj(a[i][k]) * a[i][j];
      z /= p[k];
      for (i = k; i <= nm1; ++i) a[i][j] -= z * a[i][k];
    }
  }
  sigma = a[nm1][nm1];
  q = conj(sigma) * sigma;
  ...
  a[nm1][nm1] = d[nm1];
  for (k = nm1 - 1; k >= 0; k--) {
    for (j = k; j <= nm1; j++) u[j] = a[j][k];
    a[k][k] = d[k];
    for (j = k + 1; j <= nm1; j++) a[j][k] = 0.0;
    for (i = 0; i <= nm1; i++) {
      z = 0.0;
      for (j = k; j <= nm1; j++) z += a[i][j] * u[j];
      z /= p[k]; /* normalization */

      for (j = k; j <= nm1; j++) a[i][j] -= conj(u[j]) * z; /* reflection */
    }
  }
}
