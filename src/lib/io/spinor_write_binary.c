...
int write_binary_spinor_data(spinor *const s, spinor *const r, LemonWriter *lemonwriter,
    ...
    for (z = 0; z < LZ; z++) {
      for (y = 0; y < LY; y++) {
        for (x = 0; x < LX; x++) {
          ...
          if ((z + zG + y + yG + x + xG + t + tG) % 2 == 0)
            p = s;
          else
            p = r;

          if (prec == 32)
            be_to_cpu_assign_double2single((float *)(filebuffer + bufoffset), (double *)(p + i),
                                           sizeof(spinor) / 8);
          else
            be_to_cpu_assign((double *)(filebuffer + bufoffset), (double *)(p + i),
                             sizeof(spinor) / 8);
  ...
  status =
      lemonWriteLatticeParallelMapped(lemonwriter, filebuffer, bytes, latticeSize, scidacMapping);

  if (status != LEMON_SUCCESS) {
    free(filebuffer);
    fprintf(stderr,
            "LEMON write error occurred with status = %d, while in write_binary_spinor_data "
            "(spinor_write_binary.c)!\n",
            status);
    return (-2);
  }
...
int write_binary_spinor_data(spinor *const s, spinor *const r, LimeWriter *limewriter,
  ...
  if (prec == 32)
    bytes = (n_uint64_t)sizeof(spinor) / 2;
  else
    bytes = (n_uint64_t)sizeof(spinor);
      ...
      for (y = 0; y < LY * g_nproc_y; y++) {
        ...
        for (x = 0; x < LX * g_nproc_x; x++) {
          ...
          if (g_cart_id == 0) {
            ...
            rank =
                (DML_SiteRank)(((t0 * LZ * g_nproc_z + z) * LY * g_nproc_y + y) * LX * g_nproc_x +
                               x);
            ...
            if (g_cart_id == id) {
              ...
              if (prec == 32) {
                MPI_Recv((void *)tmp2, sizeof(spinor) / 8, MPI_FLOAT, id, tag, g_cart_grid,
                         &mstatus);
                DML_checksum_accum(checksum, rank, (char *)tmp2, sizeof(spinor) / 2);
                status = limeWriteRecordData((void *)tmp2, &bytes, limewriter);
              } else {
                MPI_Recv((void *)tmp, sizeof(spinor) / 8, MPI_DOUBLE, id, tag, g_cart_grid,
                         &mstatus);
                DML_checksum_accum(checksum, rank, (char *)tmp, sizeof(spinor));
                status = limeWriteRecordData((void *)tmp, &bytes, limewriter);
              }
            ...
            if (status < 0) {
              fprintf(stderr,
                      "LIME write error occurred with status = %d, while in "
...
int write_binary_spinor_data_l(spinor *const s, LemonWriter *lemonwriter, DML_Checksum *checksum,
    ...
    for (z = 0; z < LZ; z++) {
      for (y = 0; y < LY; y++) {
        for (x = 0; x < LX; x++) {
          ...
          if (prec == 32)
            be_to_cpu_assign_double2single((float *)(filebuffer + bufoffset), (double *)(s + i),
                                           sizeof(spinor) / 8);
          else
            be_to_cpu_assign((double *)(filebuffer + bufoffset), (double *)(s + i),
                             sizeof(spinor) / 8);
          DML_checksum_accum(checksum, rank, (char *)filebuffer + bufoffset, bytes);
          bufoffset += bytes;
        }
  ...
  if (status != LEMON_SUCCESS) {
    free(filebuffer);
    fprintf(stderr,
            "LEMON write error occurred with status = %d, while in write_binary_spinor_data_l "
            "(spinor_write_binary.c)!\n",
            status);
    return (-2);
  }
