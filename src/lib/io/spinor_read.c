...
paramsPropInfo PropInfo = {_default_propagator_splitted, _default_source_format_flag,
                           _default_prop_precision_flag, NULL};
paramsSourceInfo SourceInfo = {0,
                               _default_propagator_splitted,
                               _default_source_format_flag,
                               _default_prop_precision_flag,
                               0,
                               0,
                               0,
                               0,
...
int read_spinor(spinor* const s, spinor* const r, char* filename, const int position_) {
  ...
  if (status == LIME_EOF) {
    fprintf(stderr,
            ...
            "reached before record was found.\n",
            filename);
    return (-5);
  }
  ...
  if ((uint64_t)bytes ==
      LX * g_nproc_x * LY * g_nproc_y * LZ * g_nproc_z * T * g_nproc_t * sizeof(spinor)) {
    prec = 64;
  } else {
    if ((uint64_t)bytes ==
        LX * g_nproc_x * LY * g_nproc_y * LZ * g_nproc_z * T * g_nproc_t * sizeof(spinor) / 2) {
      prec = 32;
    } else {
      fprintf(stderr,
  ...
  if (g_disable_src_IO_checks != 1) {
    ...
    while ((status = ReaderNextRecord(reader)) != LIME_EOF) {
      ...
      if (strcmp("scidac-binary-data", header_type) == 0 ||
          strcmp("ildg-binary-data", header_type) == 0) {
        break;
      }
    }
