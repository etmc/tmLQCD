...
int read_binary_gauge_data(LemonReader* lemonreader, DML_Checksum* checksum,
  ...
  if (bytes != (n_uint64_t)g_nproc * (n_uint64_t)VOLUME * 4 * (n_uint64_t)sizeof(su3) /
    ...
    fprintf(
        stderr,
        "Lattice size and precision found in data file do not match those requested at input.\n");
    fprintf(stderr, "Expected LX = %d, LY = %d, LZ = %d, LT = %d, and %s precision.\n", input->lx,
            input->ly, input->lz, input->lt, (input->prec == 64 ? "double" : "single"));
    fprintf(stderr, "Expected %lu bytes, found %lu bytes in gauge file.\n",
            (unsigned long)(n_uint64_t)g_nproc * (n_uint64_t)VOLUME * 4 * (n_uint64_t)sizeof(su3) /
                (input->prec == 64 ? 1 : 2),
            (unsigned long)bytes);
  ...
  if ((void*)(filebuffer = malloc(VOLUME * bytes)) == NULL) {
    fprintf(stderr,
            "malloc errno %d in read_binary_gauge_data, returning without reading gauge file.\n",
            errno);
    errno = 0;
    return (-1);
  }
  ...
  if (status != LEMON_SUCCESS) {
    free(filebuffer);
    fprintf(stderr,
            "Lemon read error occurred with status = %d, while reading in gauge_read_binary.c!\n",
            status);
    return (-2);
  }

  if (g_debug_level > 0 && g_cart_id == 0) {
    engineering(measure, latticeSize[0] * latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes,
                "b");
    ...
    engineering(measure,
                latticeSize[0] * latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes /
                    (g_nproc * (tock - tick)),
                "b/s");
        ...
        for (x = 0; x < LX; x++) {
          rank = (DML_SiteRank)(g_proc_coords[1] * LX +
                                (((g_proc_coords[0] * T + t) * g_nproc_z * LZ +
                                  g_proc_coords[3] * LZ + z) *
                                     g_nproc_y * LY +
                                 g_proc_coords[2] * LY + y) *
                                    ((DML_SiteRank)LX * g_nproc_x) +
                                x);
          ...
          if (input->prec == 32) {
            ...
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][2], current + fbsu3,
                                           sizeof(su3) / 8);
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][3], current + 2 * fbsu3,
                                           sizeof(su3) / 8);
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][0], current + 3 * fbsu3,
                                           sizeof(su3) / 8);
          } else {
...
int read_binary_gauge_data(LimeReader *limereader, DML_Checksum *checksum, paramsIldgFormat *input,
    ...
    for (z = 0; z < LZ; z++) {
      for (y = 0; y < LY; y++) {
        ...
        limeReaderSeek(limereader,
                       (n_uint64_t)(((n_uint64_t)g_proc_coords[1] * LX) +
                                    ((n_uint64_t)(((g_proc_coords[0] * T + t) * g_nproc_z * LZ +
                                                   g_proc_coords[3] * LZ + z) *
                                                      g_nproc_y * LY +
                                                  g_proc_coords[2] * LY + y) *
                                     LX * g_nproc_x)) *
        ...
        for (x = 0; x < LX; x++) {
          ...
          if (input->prec == 32) {
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][0], &tmp2[3 * 18],
                                           sizeof(su3) / 8);
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][1], &tmp2[0 * 18],
                                           sizeof(su3) / 8);
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][2], &tmp2[1 * 18],
                                           sizeof(su3) / 8);
            be_to_cpu_assign_single2double(&gf[g_ipt[t][x][y][z]][3], &tmp2[2 * 18],
                                           sizeof(su3) / 8);
          } else {
  ...
  if (g_debug_level > 0) {
    ...
    if (g_cart_id == 0) {
      engineering(measure,
                  latticeSize[0] * latticeSize[1] * latticeSize[2] * latticeSize[3] * bytes, "b");
      fprintf(stdout, "# Time spent reading %s ", measure);
      engineering(measure, tock - tick, "s");
      fprintf(stdout, "was %s.\n", measure);
      engineering(
