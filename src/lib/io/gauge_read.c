...
int read_gauge_field(char *filename, su3 **const gf) {
  ...
  while ((status = ReaderNextRecord(reader)) != LIME_EOF) {
    ...
    if (strcmp("ildg-binary-data", header_type) == 0) {
      if (gauge_read_flag && !g_disable_IO_checks) { /* a previous ildg-binary-data record has
                                                        already been read from this file */
        fprintf(stderr,
                "In gauge file %s, multiple LIME records with name: \"ildg-binary-data\" found.\n",
                filename);
      ...
      if (checksum_string == (char *)NULL) {
        ...
        if (!g_disable_IO_checks) {
          fprintf(stderr,
                  "In gauge file %s, multiple LIME records with name: \"scidac-checksum\" found.\n",
                  filename);
          fprintf(stderr, "Unable to verify integrity of the gauge field data.\n");
          destruct_reader(reader);
          free(ildgformat_input);
      ...
      if (ildgformat_string == (char *)NULL) {
        ...
        if (!g_disable_IO_checks) {
          fprintf(stderr,
                  "In gauge file %s, multiple LIME records with name: \"ildg-format\" found.\n",
                  filename);
          fprintf(stderr, "Unable to verify integrity of the gauge field data.\n");
          destruct_reader(reader);
          free(ildgformat_input);
  ...
  if (!g_disable_IO_checks) {
    if (!ildgformat_read_flag) {
      fprintf(
          stderr,
          "LIME record with name: \"ildg-format\", in gauge file %s either missing or malformed.\n",
          filename);
    ...
    if (!gauge_read_flag) {
      fprintf(stderr,
              "LIME record with name: \"ildg-binary-data\", in gauge file %s either missing or "
              "malformed.\n",
              filename);
    ...
    if (!DML_read_flag) {
      fprintf(stderr,
              "LIME record with name: \"scidac-checksum\", in gauge file %s either missing or "
              "malformed.\n",
              filename);
    ...
    if (checksum_calc.suma != checksum_read.suma) {
      fprintf(
          stderr,
          "For gauge file %s, calculated and stored values for SciDAC checksum A do not match.\n",
          filename);
      destruct_reader(reader);
      free(ildgformat_input);
      return (-1);
    ...
    if (checksum_calc.sumb != checksum_read.sumb) {
      fprintf(
          stderr,
          "For gauge file %s, calculated and stored values for SciDAC checksum B do not match.\n",
          filename);
      destruct_reader(reader);
      free(ildgformat_input);
      return (-1);
    ...
    if (g_cart_id == 0 && g_debug_level > 0) {
      ...
      fprintf(stdout, "#   Lattice size: LX = %d, LY = %d, LZ = %d, LT = %d.\n", ildgformat_read.lx,
              ildgformat_read.ly, ildgformat_read.lz, ildgformat_read.lt);
      fprintf(stdout, "# Input parameters:\n");
      fprintf(stdout, "#   Precision = %d bits (%s).\n", ildgformat_input->prec,
              (ildgformat_input->prec == 64 ? "double" : "single"));
      fprintf(stdout, "#   Lattice size: LX = %d, LY = %d, LZ = %d, LT = %d.\n",
              ildgformat_input->lx, ildgformat_input->ly, ildgformat_input->lz,
              ildgformat_input->lt);
    }
  }
