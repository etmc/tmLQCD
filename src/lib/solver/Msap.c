...
void Mtm_plus_sym_block_psi(spinor* const l, spinor* const k, const int i) {
  ...
  Block_H_psi(blk, &g_spinor_field[DUM_MATRIX][i * vol], &g_spinor_field[DUM_MATRIX + 1][i * vol],
              OE);
...
void Msw_plus_sym_block_psi(spinor* l, spinor* k, const int i) {
  if (g_proc_id == g_stdio_proc) {
    ...
    printf(
        "Msw_plus_sym_block_psi doesn't work properly yet because we need the inverse of (1+T "
        "+img5) on odd sites for +mu which is not computed or stored ....\n");
  ...
  int vol = (*blk).volume / 2;
  Block_H_psi(blk, &g_spinor_field[DUM_MATRIX + 1][i * vol], k, EO);
  assign_mul_one_sw_pm_imu_inv_block(EE, &g_spinor_field[DUM_MATRIX][i * vol],
                                     &g_spinor_field[DUM_MATRIX + 1][i * vol], g_mu, blk);
  Block_H_psi(blk, &g_spinor_field[DUM_MATRIX + 1][i * vol], &g_spinor_field[DUM_MATRIX][i * vol],
              OE);
  // FIXME
  assign_mul_one_sw_pm_imu_inv_block(OO, &g_spinor_field[DUM_MATRIX][i * vol],
                                     &g_spinor_field[DUM_MATRIX + 1][i * vol], g_mu, blk);
  diff(l, k, &g_spinor_field[DUM_MATRIX][i * vol], vol);
  return;
}
...
void CGeoSmoother(spinor* const P, spinor* const Q, const int Ncy, const int dummy) {
  ...
  if (g_c_sw > 0) {
    cg_her(solver_field[3], solver_field[4], Ncy, 1.e-8, 1, VOLUME / 2, &Qsw_pm_psi);
    ...
    cg_her(solver_field[3], solver_field[4], Ncy, 1.e-8, 1, VOLUME / 2, &Qtm_pm_psi);
      ...
      for (blk = 0; blk < nb_blocks; blk++) {
        ...
        if (block_list[blk].evenodd == eo) {
          ...
          if (g_c_sw > 0) {
            ...
            mrblk(b_odd, a_odd, solver_field[3] + blk * 2 * 3 * vols, Niter, 1.e-31, 1, vol,
                  &Msw_plus_block_psi, blk);
...
#pragma omp parallel for
      ...
      for (int iblk = 0; iblk < nb_blocks / 2; iblk++) {
        ...
        if (g_c_sw > 0) {
          ...
          mrblk_32(b_odd, a_odd, c, Niter, 1.e-31, 1, vol, &Msw_plus_block_psi_32, blk);
          ...
          mrblk_32(b_odd, a_odd, c, Niter, 1.e-31, 1, vol, &Mtm_plus_block_psi_32, blk);
