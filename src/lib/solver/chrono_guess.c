...
void chrono_add_solution(spinor *const trial, spinor **const v, int index_array[], const int N,
                         int *_n, const int V) {
  double norm = 0.;
  int i;
...
int chrono_guess(spinor *const trial, spinor *const phi, spinor **const v, int index_array[],
  ...
  if (N > 0) {
    ...
    if (info == 0) {
      ...
      if (g_proc_id == 0 && g_debug_level > 2) {
        printf("# CSG: bn[%d] = %f %f\n", index_array[n - 1], creal(bn[index_array[n - 1]]),
               cimag(bn[index_array[n - 1]]));
      }
      for (i = n - 2; i > -1; i--) {
        assign_add_mul(trial, v[index_array[i]], bn[i], V);
        if (g_proc_id == 0 && g_debug_level > 2) {
          printf("# CSG: bn[%d] = %f %f\n", index_array[i], creal(bn[index_array[i]]),
                 cimag(bn[index_array[i]]));
        }
      }
    } else {
      assign(trial, phi, V);
