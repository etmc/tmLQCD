...
void Zrestart_X(_Complex double *X, int ldx, _Complex double *hVecs, int nLocal, int basisSize,
                int restartSize, _Complex double *rwork, int rworkSize) {
  char cN = 'N';
  int ONE = 1;
  ...
  while (i < nLocal) {
    ...
    _FT(zgemm)(&cN, &cN, &AvailRows, &restartSize, &basisSize, &tpone, &X[i], &ldx, hVecs,
               &basisSize, &tzero, rwork, &AvailRows, 1, 1);
