...
int incr_eigcg(const int N, const int nrhs, const int nrhs1, spinor *const x, spinor *const b,
               const int ldh, matrix_mult f, const double eps_sq1, const double eps_sq,
               double restart_eps_sq, const int rand_guess_opt, const int rel_prec, const int maxit,
               int nev, const int v_max) {
  ...
  if (nev ==
      0) { /*incremental eigcg is used as a cg solver. No need to restart forcing no-restart*/
    if (g_proc_id == g_stdio_proc && g_debug_level > 0) {
      fprintf(stdout,
              "CG won't be restarted in this mode since no deflation will take place (nev=0)\n");
      fflush(stdout);
    }
  ...
  if ((ncurRHS == 1) && (nev > 0)) /* If this is the first right-hand side and eigenvectors are
                                      needed, allocate needed memory*/
  {
  ...
  /*---------------------------------------------------------------*/
  wE = 0.0;
  wI = 0.0;             /* Start accumulator timers */
  ...
  while (flag == -1 || flag == 3) {
    ...
    if (flag == 3) { /* restart with the same rhs, set nev_used = 0 */
      ...
      if (ldh - ncurEvals < nev) nev = ldh - ncurEvals;
      nev_used = nev;
    }
    ...
    eigcg(N, LDN, x, b, &normb, eps_sq_used, restart_eps_sq, rel_prec, maxit_remain, &numIts,
          &cur_res, &flag, solver_field, f, nev_used, v_max, V, esize, ework);
  ...
  if (nev > 0) {
    ...
    for (i = 0; i < nev_used; i++) assign(evecs[i + ncurEvals], &V[i * LDN], N);
    ...
    if (g_proc_id == g_stdio_proc && g_debug_level > 0) {
      ...
      fprintf(stdout,
              "Note: Update Wall time doesn't include time for computing eigenvalues and their "
              "residuals.\n");
      fflush(stdout);
    }
  ...
  if ((ncurRHS == nrhs) && (nev > 0)) /*this was the last system to be solved and there were
