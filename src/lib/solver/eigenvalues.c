...
double eigenvalues(int *nr_of_eigenvalues, const int max_iterations, const double precision,
  ...
  jdher(N * sizeof(spinor) / sizeof(_Complex double), N2 * sizeof(spinor) / sizeof(_Complex double),
        50., 1.e-12, 1, 15, 8, max_iterations, 1, 0, 0, NULL, CG, solver_it_max, threshold_max,
        decay_max, verbosity, &converged, (_Complex double *)max_eigenvector,
        (double *)&max_eigenvalue, &returncode2, JD_MAXIMAL, 1, f);
  ...
  if (readwrite != 2) {
    ...
    if (maxmin)
      jdher(N * sizeof(spinor) / sizeof(_Complex double),
            N2 * sizeof(spinor) / sizeof(_Complex double), 50., prec, (*nr_of_eigenvalues), j_max,
            j_min, max_iterations, blocksize, blockwise, v0dim, (_Complex double *)eigenvectors, CG,
            solver_it_max, threshold_max, decay_max, verbosity, &converged,
            (_Complex double *)eigenvectors, eigenvls, &returncode, JD_MAXIMAL, 1, f);
    else
      jdher(N * sizeof(spinor) / sizeof(_Complex double),
            N2 * sizeof(spinor) / sizeof(_Complex double), 0., prec, (*nr_of_eigenvalues), j_max,
            j_min, max_iterations, blocksize, blockwise, v0dim, (_Complex double *)eigenvectors, CG,
            solver_it_max, threshold_min, decay_min, verbosity, &converged,
            (_Complex double *)eigenvectors, eigenvls, &returncode, JD_MINIMAL, 1, f);
  ...
  if (readwrite == 1) {
    if (even_odd_flag)
      for (v0dim = 0; v0dim < (*nr_of_eigenvalues); v0dim++) {
        sprintf(filename, eigenvector_prefix, maxmin ? "max" : "min", v0dim, nstore);
        if ((write_eospinor(&eigenvectors[v0dim * N2], filename, eigenvls[v0dim], prec, nstore)) !=
            0) {
          break;
        }
      }
