...
int solve_degenerate(spinor* const P, spinor* const Q, solver_params_t solver_params,
  ...
  if (g_debug_level > 2 || g_strict_residual_check ||
      solver_params.external_inverter == QPHIX_INVERTER ||
  ...
  if (solver_params.external_inverter == QUDA_INVERTER) {
    ...
    iteration_count = invert_eo_degenerate_quda(P, temp[0], eps_sq, max_iter, solver_type, rel_prec,
                                                1,  // we only support even-odd here
                                                solver_params, solver_params.sloppy_precision,
                                                solver_params.compression_type, QmQp);
    ...
    // using CG for the HMC, we always want to have the solution of (Q Q^dagger) x = b, which is
    // equivalent to gamma_5 (M M^dagger)^{-1} gamma_5 b
    // FIXME: this needs to be adjusted to also support BICGSTAB
    gamma5(temp[0], Q, VOLUME / 2);
    iteration_count = invert_eo_qphix_oneflavour(
        P, temp[0], max_iter, eps_sq, solver_type, rel_prec, solver_params,
        solver_params.sloppy_precision, solver_params.compression_type);
    mul_gamma5(P, VOLUME / 2);
  } else
#endif
      if (solver_type == MIXEDCG || solver_type == RGMIXEDCG) {
    // the default mixed solver is rg_mixed_cg_her
    int (*msolver_fp)(spinor* const, spinor* const, solver_params_t, const int, double, const int,
                      const int, matrix_mult, matrix_mult32) = rg_mixed_cg_her;
    ...
    if (f == Qtm_pm_psi) {
      iteration_count =
          msolver_fp(P, Q, solver_params, max_iter, eps_sq, rel_prec, N, f, &Qtm_pm_psi_32);
    } else if (f == Q_pm_psi) {
      iteration_count =
          msolver_fp(P, Q, solver_params, max_iter, eps_sq, rel_prec, N, f, &Q_pm_psi_32);
    } else if (f == Qsw_pm_psi) {
      copy_32_sw_fields();
      iteration_count =
          msolver_fp(P, Q, solver_params, max_iter, eps_sq, rel_prec, N, f, &Qsw_pm_psi_32);
    } else {
      if (g_proc_id == 0)
        printf("Warning: 32 bit matrix not available. Falling back to CG in 64 bit\n");
      solver_type = CG;
    }
  ...
  if (g_debug_level > 2 || g_strict_residual_check) {
    ...
    if (g_strict_residual_check) {
      if (diffnorm > 1.1 * (eps_sq * rel_nrm)) {
        fatal_error("Residual norm exceeds target by more than a factor of 1.1!",
                    "solve_degenerate");
      }
    }
  }
...
int solve_mms_tm(spinor** const P, spinor* const Q, solver_params_t* solver_params) {
  tm_stopwatch_push(&g_timers, __func__, "");
  int iteration_count = 0;
  ...
  if (solver_params->external_inverter == QPHIX_INVERTER && solver_params->type != MG) {
    gamma5(temp[0], Q, VOLUME / 2);
    iteration_count = invert_eo_qphix_oneflavour_mshift(
        P, temp[0], solver_params->max_iter, solver_params->squared_solver_prec,
        solver_params->type, solver_params->rel_prec, *solver_params,
        solver_params->sloppy_precision, solver_params->compression_type);
    ...
    gamma5(temp[0], Q, VOLUME / 2);
    tm_stopwatch_pop(&g_timers, 0, 1, "");
    iteration_count = invert_eo_quda_oneflavour_mshift(
        P, temp[0], solver_params->squared_solver_prec, solver_params->max_iter,
        solver_params->type, solver_params->rel_prec,
        1,  // we only support even-odd here
        *solver_params, solver_params->sloppy_precision, solver_params->compression_type);
    tm_stopwatch_push(&g_timers, "gamma5", "");
    for (int shift = 0; shift < solver_params->no_shifts; shift++) {
      mul_gamma5(P[shift], VOLUME / 2);
    }
    ...
    if (mg_no_shifts > 0 || mg_mms_mass >= solver_params->shifts[0]) {
      ...
      if (mg_mms_mass >= solver_params->shifts[0]) {
        ...
        while (mg_mms_mass < solver_params->shifts[mg_nshifts - 1]) {
          mg_nshifts--;
        }
      }
      ...
      for (int i = mg_nshifts - 1; i >= 0; i--) {
        ...
        if (i < mg_nshifts - no_cgmms_init_guess) init_guess_mms(P, Q, i, solver_params);
        g_mu3 = solver_params->shifts[i];
        iteration_count += MG_solver(P[i], Q, solver_params->squared_solver_prec,
                                     solver_params->max_iter, solver_params->rel_prec,
                                     solver_params->sdim, g_gauge_field, solver_params->M_psi);
    ...
    iteration_count = 0;
    // solver_params_t struct needs to be passed to all solvers except for cgmms, so we need to
    // construct it here and set the one relevant parameter
    ...
    for (int i = solver_params->no_shifts - 1; i >= 0; i--) {
      ...
      iter_local = rg_mixed_cg_her(P[i], Q, temp_params, solver_params->max_iter,
                                   solver_params->squared_solver_prec, solver_params->rel_prec,
                                   solver_params->sdim, solver_params->M_psi, f32);
      g_mu3 = _default_g_mu3;
      solver_params->use_initial_guess = 0;
      if (iter_local == -1) {
        return (-1);
  ...
  if (g_debug_level > 2 || g_strict_residual_check) {
    ...
    if (solver_params->M_psi == Qsw_pm_psi) f = Qsw_pm_psi_shift;

    int rel_nrm =
        solver_params->rel_prec && g_strict_residual_check ? square_norm(Q, VOLUME / 2, 1) : 1.0;
    int check_fail = 0;
    for (int shift = 0; shift < solver_params->no_shifts; shift++) {
    ...
    if (g_strict_residual_check && check_fail > 0) {
      fatal_error(
          "Residual norm for at least one shift exceeds target by more than a factor of 1.1!",
          "solve_mms_tm");
    }
  }
  if (g_debug_level > 2 || g_strict_residual_check ||
...
int solve_mms_nd(spinor** const Pup, spinor** const Pdn, spinor* const Qup, spinor* const Qdn,
                 solver_params_t* solver_params) {
  tm_stopwatch_push(&g_timers, __func__, "");
  int iteration_count = 0;
  solver_params->use_initial_guess = 0;
  ...
  if (solver_params->external_inverter == QUDA_INVERTER) {
    ...
    tm_stopwatch_pop(&g_timers, 0, 1, "");
    iteration_count = invert_eo_quda_twoflavour_mshift(
        Pup, Pdn, temp[0], temp[1], solver_params->squared_solver_prec, solver_params->max_iter,
        solver_params->type, solver_params->rel_prec,
        1,  // we only support even-odd here
        *solver_params, solver_params->sloppy_precision, solver_params->compression_type);
    ...
    gamma5(temp[0], Qup, VOLUME / 2);
    gamma5(temp[1], Qdn, VOLUME / 2);
    iteration_count = invert_eo_qphix_twoflavour_mshift(
        Pup, Pdn, temp[0], temp[1], solver_params->max_iter, solver_params->squared_solver_prec,
        solver_params->type, solver_params->rel_prec, *solver_params,
        solver_params->sloppy_precision, solver_params->compression_type);
    ...
    if (solver_params->type == MIXEDCGMMSND) {
      ...
      if (mg_no_shifts > 0 || mg_mms_mass >= solver_params->shifts[0]) {
        ...
        if (solver_params->external_inverter == QPHIX_INVERTER && mg_nshifts < nshifts) {
          ...
          gamma5(temp[1], Qdn, VOLUME / 2);
          iteration_count = invert_eo_qphix_twoflavour_mshift(
              Pup_cg, Pdn_cg, temp[0], temp[1], solver_params->max_iter,
              solver_params->squared_solver_prec, solver_params->type, solver_params->rel_prec,
              *solver_params, solver_params->sloppy_precision, solver_params->compression_type);

          // the tmLQCD ND operator used for HMC is normalised by the inverse of the maximum
          // eigenvalue so the inverse of Q^2 is normalised by the square of the maximum eigenvalue
          // or, equivalently, the square of the inverse of the inverse
          // note that in the QPhiX interface, we also correctly normalise the shifts
          const double maxev_sq = (1.0 / phmc_invmaxev) * (1.0 / phmc_invmaxev);
          for (int shift = 0; shift < solver_params->no_shifts; shift++) {
        ...
        if (solver_params->M_ndpsi == Qsw_pm_ndpsi) f = Qsw_pm_ndpsi_shift;
        ...
        for (int i = mg_nshifts - 1; i >= 0; i--) {
          ...
          iteration_count += MG_solver_nd(
              Pup[i], Pdn[i], Qup, Qdn, solver_params->squared_solver_prec, solver_params->max_iter,
      ...
      for (int i = solver_params->no_shifts - 1; i >= 0; i--) {
        ...
        // inverting
        g_shift = solver_params->shifts[i] * solver_params->shifts[i];
        iter_local = rg_mixed_cg_her_nd(Pup[i], Pdn[i], Qup, Qdn, temp_params,
                                        solver_params->max_iter, solver_params->squared_solver_prec,
                                        solver_params->rel_prec, solver_params->sdim, f, f32);
        g_shift = _default_g_shift;
        solver_params->use_initial_guess = 0;
        if (iter_local == -1) {
          return (-1);
  ...
  if (g_debug_level > 2 || g_strict_residual_check) {
    ...
    int rel_nrm = solver_params->rel_prec && g_strict_residual_check
                      ? square_norm(Qup, VOLUME / 2, 1) + square_norm(Qdn, VOLUME / 2, 1)
                      : 1.0;
    int check_fail = 0;
    for (int shift = 0; shift < solver_params->no_shifts; shift++) {
      g_shift = solver_params->shifts[shift] * solver_params->shifts[shift];
...
int solve_mms_nd_plus(spinor** const Pup, spinor** const Pdn, spinor* const Qup, spinor* const Qdn,
                      solver_params_t* solver_params) {
  tm_stopwatch_push(&g_timers, __func__, "");
  ...
  if (solver_params->type == MG) {
    ...
    if (solver_params->M_ndpsi == Qsw_pm_ndpsi) f = Qsw_tau1_ndpsi_add_Ishift;
    for (int i = solver_params->no_shifts - 1; i >= 0; i--) {
      // preparing initial guess
      ...
      iteration_count += MG_solver_nd(Pup[i], Pdn[i], Qup, Qdn, solver_params->squared_solver_prec,
                                      solver_params->max_iter, solver_params->rel_prec,
                                      solver_params->sdim, g_gauge_field, f);
      g_shift = _default_g_shift;
    }
  } else
#endif
  {
    ...
    if (solver_params->M_ndpsi == Qsw_pm_ndpsi) f = Qsw_tau1_ndpsi_sub_Ishift;
    spinor** temp;
    init_solver_field(&temp, VOLUMEPLUSRAND / 2, 2);
