...
void index_jd(int *nr_of_eigenvalues_ov, const int max_iterations, const double precision_ov,
  ...
  if (g_proc_id == g_stdio_proc) {
    printf(
        "Computing first the two lowest modes in the positive and negative chirality sector, "
        "respectively\n");
  ...
  for (intsign = 0; intsign < 2; intsign++) {
    ...
    jdher(VOLUME * sizeof(spinor) / sizeof(_Complex double),
          VOLUMEPLUSRAND * sizeof(spinor) / sizeof(_Complex double), shift, prec, blocksize, j_max,
          j_min, max_iter, blocksize, blockwise, v0dim,
          (_Complex double *)&lowvectors[first_blocksize * intsign * VOLUMEPLUSRAND], CG,
          solver_it_max, threshold_min, decay_min, verbosity, &converged,
          (_Complex double *)&lowvectors[first_blocksize * intsign * VOLUMEPLUSRAND],
          &lowestmodes[first_blocksize * intsign], &returncode, JD_MINIMAL, 1, Operator[intsign]);
    ...
    if (converged != blocksize && max_iter == 70) {
      ...
      for (i = 0; i < first_blocksize; i++) {
        sprintf(filename, "eigenvector_of_D%s.%.2d.%s.%.4d", ((intsign == 0) ? "plus" : "minus"), i,
                conf_filename, nstore);
  ...
  if (absdifference < 0.1 * max(lowestmodes[0], lowestmodes[first_blocksize])) {
    ...
    if (g_proc_id == g_stdio_proc) {
      ...
      fclose(ifs);
      for (k = 0; k < 2; k++) {
        sprintf(filename, "eigenvalues_of_D%s.%s.%.4d", k ? "minus" : "plus", conf_filename,
                nstore);
        ifs = fopen(filename, "w");
    ...
    for (i = 0; i < first_blocksize; i++) {
      assign(&eigenvectors_ov[i], &lowvectors[(first_blocksize * intsign + i) * VOLUMEPLUSRAND],
             N2);
    ...
    for (i = first_blocksize; i < (*nr_of_eigenvalues_ov); i += 3) {
...
#ifdef TM_USE_MPI
      /*      pjdher(VOLUME*sizeof(spinor)/sizeof(_Complex double),
         VOLUMEPLUSRAND*sizeof(spinor)/sizeof(_Complex double), shift, prec, omega, n_omega, ev_tr,
             i+blocksize, j_max, j_min,
             max_iterations, blocksize, blockwise, v0dim, (_Complex
         double*)(&eigenvectors_ov[i*VOLUMEPLUSRAND]), CG, solver_it_max, threshold_min, decay_min,
         verbosity, &converged, (_Complex double*) eigenvectors_ov, eigenvalues_ov, &returncode,
         JD_MINIMAL, 1, use_AV, Operator[intsign]);*/
#else
      jdher(VOLUME * sizeof(spinor) / sizeof(_Complex double),
            VOLUMEPLUSRAND * sizeof(spinor) / sizeof(_Complex double), shift, prec, blocksize,
            j_max, j_min, max_iter, blocksize, blockwise, v0dim,
            (_Complex double *)&eigenvectors_ov[i * VOLUMEPLUSRAND], CG, solver_it_max,
            threshold_min, decay_min, verbosity, &converged, (_Complex double *)eigenvectors_ov,
            eigenvalues_ov, &returncode, JD_MINIMAL, 1, Operator[intsign]);
#endif
      ...
      for (k = 0; k < converged; k++) {
        absdifference =
            fabs(lowestmodes[first_blocksize * ((intsign + 1) % 2)] - eigenvalues_ov[k]);
    ...
    if (g_proc_id == 0) {
      ...
      for (k = 0; k < 2; k++) {
        if (k == intsign) {
          ...
          for (i = 0; i < first_blocksize; i++) {
            lowestmodes[((intsign + 1) % 2) * first_blocksize + i] =
                2. * (1. + ov_s) * lowestmodes[((intsign + 1) % 2) * first_blocksize + i];
            fprintf(ifs, "%d %e %s\n", i, lowestmodes[((intsign + 1) % 2) * first_blocksize + i],
                    intsign ? "positive" : "negative");
            printf("%d %e %s\n", i, lowestmodes[((intsign + 1) % 2) * first_blocksize + i],
                   intsign ? "positive" : "negative");
          }
        }
