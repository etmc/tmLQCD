...
static inline unsigned int inner_loop_high(spinor *const x, spinor *const p, spinor *const q,
                                           spinor *const r, double *const rho1, const double delta,
                                           matrix_mult f, const double eps_sq, const unsigned int N,
...
static inline unsigned int inner_loop(spinor32 *const x, spinor32 *const p, spinor32 *const q,
                                      spinor32 *const r, float *const rho1, const float delta,
                                      matrix_mult32 f32, const float eps_sq, const unsigned int N,
                                      const unsigned int iter, const unsigned max_iter, float alpha,
                                      float beta, MCG_PIPELINED_TYPE pipelined, MCG_PR_TYPE pr) {
...
int rg_mixed_cg_her(spinor *const P, spinor *const Q, solver_params_t solver_params,
                    const int max_iter, const double eps_sq, const int rel_prec, const int N,
  ...
  if (g_debug_level > 0 && g_proc_id == 0) printf("#RG_Mixed CG: N_outer: %d \n", N_outer);
  ...
  iter_in_sp += inner_loop(x, p, q, r, &rho_sp, delta, f32, (float)target_eps_sq, N,
                           iter_out + iter_in_sp + iter_in_dp, max_iter, 0.0, 0.0, MCG_NO_PIPELINED,
                           MCG_NO_PR);
  ...
  for (iter_out = 1; iter_out < N_outer; ++iter_out) {
    ...
    if (high_control == 1) {
      ...
      beta_dp = 1 / rho_dp;
      iter_in_dp += inner_loop_high(xhigh, phigh, qhigh, rhigh, &rho_dp, delta, f, target_eps_sq, N,
    ...
    zero_spinor_field_32(x, N);
    iter_in_sp += inner_loop(x, p, q, r, &rho_sp, delta, f32, (float)target_eps_sq, N,
                             iter_out + iter_in_sp + iter_in_dp, max_iter, 0.0, 0.0,
                             MCG_NO_PIPELINED, MCG_NO_PR);
  }
...
void output_flops(const double seconds, const unsigned int N, const unsigned int iter_out,
  ...
  if (g_debug_level > 0 && g_proc_id == 0) {
    ...
    printf(
        "#RG_mixed CG: flopcount (for e/o tmWilson only): t/s: %1.4e mflops_local: %.1f mflops: "
        "%.1f\n",
