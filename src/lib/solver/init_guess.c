...
int init_guess_mms(spinor** const P, spinor* const Q, int shift,
                   solver_params_t* const solver_params) {
  ...
  if (g_debug_level > 2) {
    ...
    double res =
        sqrt(square_norm(temp[0], solver_params->sdim, 1) / square_norm(Q, solver_params->sdim, 1));
...
int init_guess_mms_nd(spinor** const Pup, spinor** const Pdn, spinor* const Qup, spinor* const Qdn,
  ...
  if (g_debug_level > 2) {
    ...
    double res =
        sqrt(square_norm(temp[0], solver_params->sdim, 1) +
             square_norm(temp[1], solver_params->sdim, 1)) /
...
int init_guess_mms_nd_plus(spinor** const Pup, spinor** const Pdn, spinor* const Qup,
                           spinor* const Qdn, int shift, solver_params_t* solver_params) {
  double* shifts = solver_params->shifts;
  int no_shifts = solver_params->no_shifts;
  if (shift == no_shifts - 1) {
    zero_spinor_field(Pup[shift], solver_params->sdim);
    ...
    for (int j = no_shifts - 1; j > shift; j--) {
      coeff = 1;
      for (int k = no_shifts - 1; k > shift; k--) {
        if (j != k) coeff *= (shifts[k] - shifts[shift]) / (shifts[k] - shifts[j]);
      }
      if (j == no_shifts - 1) {
        mul_r(Pup[shift], coeff, Pup[j], solver_params->sdim);
        mul_r(Pdn[shift], coeff, Pdn[j], solver_params->sdim);
