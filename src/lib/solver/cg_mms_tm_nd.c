...
int cg_mms_tm_nd(spinor** const Pup, spinor** const Pdn, spinor* const Qup, spinor* const Qdn,
        ...
        while (((err <= solver_params->squared_solver_prec) && (solver_params->rel_prec == 0)) ||
          ...
          if (g_debug_level > 3) {
            ...
            for (int is = shifts; is > 0; is--) {
              ...
              if (g_proc_id == 0)
                printf("#\t %d\t\t %e\t %e\n", is, solver_params->shifts[is],
                       solver_params->rel_prec ? err / squarenorm : err);
            }
            if (g_proc_id == 0)
              printf("#\t %d\t\t %e\t %e\n", 0, solver_params->shifts[0],
                     solver_params->rel_prec ? normsq / squarenorm : normsq);
          }
          if (g_debug_level > 2 && g_proc_id == 0) {
            printf(
                "# CGMMSND: at iteration %d removed one shift with residual %e. %d shifts "
                "remaining\n",
