...
int _PSWITCH(gcr4complex)(_C_TYPE *const P, _C_TYPE *const Q, const int m, const int max_restarts,
                          const double eps_sq, const int rel_prec, const int N, const int parallel,
...
#pragma omp for
#endif
    for (int i = 0; i < N; ++i) tmp += creal(conj(Q[i]) * Q[i]);
  ...
  for (int i = 0; i < omp_num_threads; ++i) nrm += g_omp_acc_re[i];
#else
  nrm = tmp;
#endif
...
_C_TYPE _PSWITCH(lscalar_prod)(_C_TYPE *const R, _C_TYPE *const S, const int N,
                               const int parallel) {
...
#pragma omp for
#endif
    for (int i = 0; i < N; ++i) tmp += conj(R[i]) * S[i];
  ...
  // having left the parallel section, we can now sum up
  for (int i = 0; i < omp_num_threads; ++i) res += g_omp_acc_cp[i];
#else
  res = tmp;
#endif
...
_F_TYPE _PSWITCH(lscalar_prod_r)(_C_TYPE *const R, _C_TYPE *const S, const int N,
                                 const int parallel) {
  double res = 0.;
#ifdef gcr4complexOMP
#pragma omp parallel
...
#pragma omp parallel for
#endif
  for (int i = 0; i < N; ++i) R[i] = c * R[i] + S[i];
}
