...
i=myid-1; if(myid==0) {i=numprocs-1;}
/* recieve the data from the neighbour on the right */
k=myid+1; if(myid==numprocs-1) {k=0;}
...
i=myid+1; if(myid==numprocs-1) {i=0;}
/* recieve the data from the neighbour on the left */
k=myid-1; if(myid==0) {k=numprocs-1;}
...
/* recieve the data from the neighbour on the right */
k=myid+1; if(myid==numprocs-1) {k=0;}
MPI_Sendrecv(&gauge_field[0][0].c11.re,       72*L*L*L, MPI_DOUBLE, i, 83,
             &gauge_field[T*L*L*L][0].c11.re, 72*L*L*L, MPI_DOUBLE, k, 83,
	     MPI_COMM_WORLD,&status);
...
/* recieve the data from the neighbour on the left */
k=myid-1; if(myid==0) {k=numprocs-1;}
MPI_Sendrecv(&gauge_field[(T-1)*L*L*L][0].c11.re,72*L*L*L, MPI_DOUBLE, i, 84,
             &gauge_field[(T+1)*L*L*L][0].c11.re,72*L*L*L, MPI_DOUBLE, k, 84,
	     MPI_COMM_WORLD,&status);
}
