...
\item {\ttfamily GMRESMParameter}:\\
  Krylov subspace size $m$ in GMRES($m$) and such like iterative solvers.

\item {\ttfamily GMRESDRNrEv}:\\
  Number of eigenvalues to be deflated in GMRES-DR iterative solver.

\item {\ttfamily SourceFormat}:\\
  Format of sources and propagators. Only needed when not standard
  format is used. Possible values then cmi.
...
\item {\ttfamily SourceInputFilename}:\\
  Filename of the source vector.

\item {\ttfamily SourceTimeSlice}:\\
  The time slice of the source to be read. At the moment used only for
  the automatic construction of filenames...
