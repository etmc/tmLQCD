...
int invert_doublet_eo(spinor* const Even_new_s, spinor* const Odd_new_s, spinor* const Even_new_c,
  ...
  if (external_inverter == QUDA_INVERTER) {
    return invert_doublet_eo_quda(Even_new_s, Odd_new_s, Even_new_c, Odd_new_c, Even_s, Odd_s,
                                  Even_c, Odd_c, precision, max_iter, solver_flag, rel_prec,
                                  even_odd_flag, sloppy, solver_params.refinement_precision,
                                  compression);
  }
#endif
  ...
  M_ee_inv_ndpsi(Even_new_s, Even_new_c, Even_s, Even_c, g_mubar, g_epsbar);
  ...
  if (external_inverter == NO_EXT_INV) {
    ...
    if (solver_flag == RGMIXEDCG) {
      iter = rg_mixed_cg_her_nd(Odd_new_s, Odd_new_c, g_spinor_field[DUM_DERI],
                                g_spinor_field[DUM_DERI + 1], solver_params, max_iter, precision,
                                rel_prec, VOLUME / 2, &Qtm_pm_ndpsi, &Qtm_pm_ndpsi_32);
    } else {
    ...
    Qtm_dagger_ndpsi(Odd_new_s, Odd_new_c, Odd_new_s, Odd_new_c);
  }  // if(NO_EXT_INV)
#ifdef TM_USE_QPHIX
  else if (external_inverter == QPHIX_INVERTER) {
    // using QPhiX, we invert M M^dagger y = b, so we don't need gamma_5 multiplications
    iter = invert_eo_qphix_twoflavour(Odd_new_s, Odd_new_c, g_spinor_field[DUM_DERI],
                                      g_spinor_field[DUM_DERI + 1], max_iter, precision,
  ...
  M_ee_inv_ndpsi(g_spinor_field[DUM_DERI + 2], g_spinor_field[DUM_DERI + 3],
                 g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1], g_mubar, g_epsbar);
...
int invert_cloverdoublet_eo(spinor* const Even_new_s, spinor* const Odd_new_s,
  ...
  Msw_ee_inv_ndpsi(Even_new_s, Even_new_c, Even_s, Even_c);
  Hopping_Matrix(OE, g_spinor_field[DUM_DERI], Even_new_s);
  Hopping_Matrix(OE, g_spinor_field[DUM_DERI + 1], Even_new_c);
  ...
  if (external_inverter == NO_EXT_INV) {
    ...
    if (solver_flag == RGMIXEDCG) {
      iter = rg_mixed_cg_her_nd(Odd_new_s, Odd_new_c, g_spinor_field[DUM_DERI],
                                g_spinor_field[DUM_DERI + 1], solver_params, max_iter, precision,
                                rel_prec, VOLUME / 2, &Qsw_pm_ndpsi, &Qsw_pm_ndpsi_32);
    } else {
      iter = cg_her_nd(Odd_new_s, Odd_new_c, g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1],
                       max_iter, precision, rel_prec, VOLUME / 2, &Qsw_pm_ndpsi);
    }

    Qsw_dagger_ndpsi(Odd_new_s, Odd_new_c, Odd_new_s, Odd_new_c);
  }  // if(NO_EXT_INV)
#ifdef TM_USE_QPHIX
  else if (external_inverter == QPHIX_INVERTER) {
    // using QPhiX, we invert M M^dagger y = b, so we don't need gamma_5 multiplications
