...
void X_psi(spinor *const R, spinor *const S, double const mstarsq) {
  ...
  /*cg_her(out spinor, in spinor, max iter, solver precision, flag relative precision default 0,
   * volume, operator to invert)*/
...
void X_psiSquare(spinor *const R, spinor *const S,
                 double const mstarsq) {  // inverts DD^+DD^+ instead of DD^+ but performs poorly
  spinor *aux_, *aux;
  {
#if (defined SSE || defined SSE2 || defined SSE3)
    aux_ = calloc(VOLUMEPLUSRAND + 1, sizeof(spinor));
