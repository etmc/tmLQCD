  ...
  if (even_odd_flag) {
    ...
    if (read_source_flag == 2) { /* save */
      ...
      if (write_cp_flag == 1) {
        ...
        read_spinorfield_cm_single(g_spinor_field[2], g_spinor_field[3], SourceInfo.basename, -1,
                                   0);
        ...
        if (n > 0) {
          ...
          for (i = 0; i < MIN(n, 1000); i++) {
            printf("%d,(%d,%d,%d,%d):%f vs. %f\n", nn[i], g_coord[nn[i]][0], g_coord[nn[i]][1],
                   g_coord[nn[i]][2], g_coord[nn[i]][3], creal((g_spinor_field[2][mm[i]].s0).c0),
                   creal((g_spinor_field[0][mm[i]].s0).c0));
            fflush(stdout);
          }
        }
        n = 0;
        ...
        if (n > 0) {
          ...
          for (i = 0; i < MIN(n, 1000); i++) {
            printf("%d,(%d,%d,%d,%d):%f vs. %f\n", nn[i], g_coord[nn[i]][0], g_coord[nn[i]][1],
                   g_coord[nn[i]][2], g_coord[nn[i]][3], creal(g_spinor_field[3][mm[i]].s0.c0),
                   creal(g_spinor_field[1][mm[i]].s0.c0));
            fflush(stdout);
          }
        }
        printf("max delta=%e", deltamax);
        fflush(stdout);
      }
#endif
    }
    ...
    if (read_source_flag > 0 &&
        write_cp_flag == 0) { /* read-source yes or nobutsave; checkpoint no */
