 ...
 *     (spinor * const R,spinor * const S,spinor * const U,const double c1,const double c2,const
 *double c3) Makes (*R) = c1*(*R) + c2*(*S) + c3*(*U) with c1, c2 and c3 real variables
 *
 *******************************************************************************/
...
void assign_mul_add_mul_add_mul_r(spinor *const R, spinor *const S, spinor *const U,
                                  const double c1, const double c2, const double c3, const int N) {
#ifdef TM_USE_OMP
#pragma omp parallel
  {
#endif
