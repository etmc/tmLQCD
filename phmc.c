...
void init_phmc() {
  ...
  if (phmc_compute_evs != -1) {
    ...
    if (g_epsbar != 0.0)
      phmc_cheb_evmin =
          eigenvalues_bi(&no_eigenvalues, max_iter_ev, eigenvalue_precision, 0, &Qsw_pm_ndbipsi);
    else {
      phmc_cheb_evmin = eigenvalues(&no_eigenvalues, max_iter_ev, eigenvalue_precision, 0, 0,
    ...
    if (g_epsbar != 0.0)
      phmc_cheb_evmax =
          eigenvalues_bi(&no_eigenvalues, max_iter_ev, eigenvalue_precision, 1, &Qsw_pm_ndbipsi);
    else
      phmc_cheb_evmax = eigenvalues(&no_eigenvalues, max_iter_ev, eigenvalue_precision, 1, 0,
                                    nstore, even_odd_flag);
  ...
  if ((g_proc_id == 0) && (g_debug_level > 1)) {
    printf("PHMC: interval of approximation [stilde_min, stilde_max] = [%e, %e]\n", stilde_min,
           stilde_max);
    printf("PHMC: degree for P = %d, epsilont = %e, normalisation = %e", phmc_dop_n_cheby - 1,
           phmc_cheb_evmin, phmc_invmaxev);
  }
  ...
  */
  ...
  if (mnl->MDPolyLocNormConst == -1.0) {
    if (!(g_epsbar != 0.0 || phmc_exact_poly == 0)) filename_const = filename_const_oox;
  ...
  if (g_epsbar == 0.0 && phmc_exact_poly == 1) filename_phmc_root = filename_phmc_root_oox;

  if (strlen(mnl->MDPolyRootsFile) != 0) filename_phmc_root = mnl->MDPolyRootsFile;
...
void phmc_compute_ev(const int trajectory_counter, const int id, matrix_mult_bi Qsq) {
  double atime, etime;
  ...
  if (mnl->external_eigsolver == QUDA_EIGSOLVER) {
    ...
    eigsolveQuda(&eval_min, no_eigenvalues, eigenvalue_precision, 1, 0, max_iter_ev, 0,
                 mnl->accprec, mnl->maxiter, mnl->eig_polydeg, mnl->eig_amin, mnl->eig_amax,
  ...
  if (g_proc_id == 0) {
    if (creal(eval_max) > mnl->EVMax) {
      fprintf(stderr,
              "\nWarning: largest eigenvalue for monomial %s: %.6f is larger than upper bound: "
              "%.6f\n\n",
              mnl->name, creal(eval_max), mnl->EVMax);
    }
    if (creal(eval_min) < mnl->EVMin) {
      fprintf(stderr,
              "\nWarning: smallest eigenvalue for monomial %s: %.6f is smaller than lower bound: "
              "%.6f\n\n",
              mnl->name, creal(eval_min), mnl->EVMin);
    }
    countfile = fopen(phmcfilename, "a");
    fprintf(countfile, "%.8d %1.5e %1.5e %1.5e %1.5e\n", trajectory_counter, creal(eval_min),
            creal(eval_max), mnl->EVMin, mnl->EVMax);
    fclose(countfile);
  }
  etime = gettime();
  if ((g_proc_id == 0) && g_debug_level > 1) {
...
void popPhmcVars() {
  if (phmc_var_stack != NULL) {
    ...
    if (phmc_var_stack != NULL) phmc_var_stack->next = NULL;

  } else {
    if (g_proc_id == 0) fprintf(stderr, "Error: there is no element on the stack\n");
  }
}
