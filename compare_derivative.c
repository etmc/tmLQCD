...
void compare_derivative(monomial *mnl, su3adj **ext_lib, su3adj **native, const double threshold,
    ...
    for (int mu = 0; mu < 4; mu++) {
      ...
      for (int j = 0; j < 8; ++j) {
        ...
        if (sqrt(diff * diff) > threshold || isnan(ext[j]) || isinf(ext[j])) {
          n_diff++;
          printf(
              "derivative at (t,x,y,z,mu,j) %d,%d,%d,%d,%d,%d,"
              " ext: %-14e, native: %-14e ratio: %-14g diff %-14g  on proc_id %d\n",
              g_coord[ix][0], g_coord[ix][1], g_coord[ix][2], g_coord[ix][3], mu, j, ext[j], nat[j],
              ext[j] / nat[j], ext[j] - nat[j], g_proc_id);
        }
      }
    }
  }
  if (n_diff > 0) {
    printf(
        "%s: the deviation between tmLQCD and the external library "
        "exceeds the threshold %.1e in %d case(s) for parameters: c0=%e c1=%e g_beta=%e on "
        "proc_id: %d\n",
        name, threshold, n_diff, mnl->c0, mnl->c1, mnl->beta, g_proc_id);

    if (g_strict_residual_check)
      fatal_error("Difference between external library and tmLQCD-native function!", name);
  }
