...
void setup_mregions_bats_counters(const int bufferSize) {
  ...
  int rc = Kernel_AllocateBaseAddressTable(batsubgroupID /*subgrpId*/, &batSubGrp, 2, /*nbatids*/
                                           batIds, 0 /* "User" use */);
  ...
  rc = Kernel_CreateMemoryRegion(&memRegion, SPIrecvBuffers, buffersSize);
  if (rc != 0) {
  ...
  uint64_t paAddr =
      (uint64_t)SPIrecvBuffers - (uint64_t)memRegion.BaseVa + (uint64_t)memRegion.BasePa;

  rc = MUSPI_SetBaseAddress(&batSubGrp, recvBufBatId, paAddr);
  ...
  rc = Kernel_CreateMemoryRegion(&memRegion, (void *)&recvCounter, sizeof(recvCounter));
  if (rc != 0) {
    printf("Kernel_CreateMemoryRegion failed with rc=%d\n", rc);
    exit(1);
  }

  paAddr = (uint64_t)&recvCounter - (uint64_t)memRegion.BaseVa + (uint64_t)memRegion.BasePa;

  uint64_t paAddrAtomic = MUSPI_GetAtomicAddress(paAddr, MUHWI_ATOMIC_OPCODE_STORE_ADD);

  rc = MUSPI_SetBaseAddress(&batSubGrp, recvCntrBatId, paAddrAtomic);
  ...
  rc = Kernel_CreateMemoryRegion(&memRegion, SPIsendBuffers, buffersSize);
  if (rc != 0) {
    printf("Kernel_CreateMemoryRegion failed with rc=%d\n", rc);
    exit(1);
  }

  sendBufPAddr = (uint64_t)SPIsendBuffers - (uint64_t)memRegion.BaseVa + (uint64_t)memRegion.BasePa;
  return;
}
...
void create_descriptors(MUHWI_Descriptor_t *descriptors, uint64_t *messageSizes, uint64_t *soffsets,
  ...
  uint64_t anyFifoMap = MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_AM | MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_AP |
                        MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_BM | MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_BP |
                        MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_CM | MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_CP |
                        MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_DM | MUHWI_DESCRIPTOR_TORUS_FIFO_MAP_DP |
  ...
  for (unsigned int i = 0; i < num_dirs; i++) {
    ...
    if (do_dynamic) {
      dinfo.Pt2Pt.Misc1 = nb2dest[i].hintsE | MUHWI_PACKET_USE_DYNAMIC_ROUTING |
      ...
      dinfo.Pt2Pt.Misc1 = nb2dest[i].hintsE | MUHWI_PACKET_USE_DETERMINISTIC_ROUTING |
                          MUHWI_PACKET_DO_NOT_ROUTE_TO_IO_NODE;

      dinfo.Pt2Pt.Misc2 =
          MUHWI_PACKET_VIRTUAL_CHANNEL_DETERMINISTIC | zoneRoutingMask | stayOnBubbleMask;
...
int get_destinations(int *mypers) {
  ...
  MPI_Status mstatus;
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_t_up, 0, (void *)tmp, 6, MPI_INT, g_nb_t_dn, 0,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[1].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_t_dn, 1, (void *)tmp, 6, MPI_INT, g_nb_t_up, 1,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[0].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
#endif
#if (defined PARALLELXT || defined PARALLELXYT || defined PARALLELXYZT)
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_x_up, 2, (void *)tmp, 6, MPI_INT, g_nb_x_dn, 2,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[3].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_x_dn, 3, (void *)tmp, 6, MPI_INT, g_nb_x_up, 3,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[2].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
#endif
#if (defined PARALLELXYT || defined PARALLELXYZT)
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_y_up, 4, (void *)tmp, 6, MPI_INT, g_nb_y_dn, 4,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[5].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_y_dn, 5, (void *)tmp, 6, MPI_INT, g_nb_y_up, 5,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[4].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
#endif
#if (defined PARALLELXYZT)
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_z_up, 6, (void *)tmp, 6, MPI_INT, g_nb_z_dn, 6,
               g_cart_grid, &mstatus);
  MUSPI_SetUpDestination(&nb2dest[7].dest, tmp[0], tmp[1], tmp[2], tmp[3], tmp[4]);
  MPI_Sendrecv((void *)mypers, 6, MPI_INT, g_nb_z_dn, 7, (void *)tmp, 6, MPI_INT, g_nb_z_up, 7,
               g_cart_grid, &mstatus);
...
typedef struct msg_InjFifoInfo {
  ...
  void *fifoMemoryPtr[BGQ_MU_NUM_INJ_FIFOS_PER_SUBGROUP * BGQ_MU_NUM_FIFO_SUBGROUPS_PER_NODE];
  void *fifoPtr[BGQ_MU_NUM_INJ_FIFOS_PER_SUBGROUP * BGQ_MU_NUM_FIFO_SUBGROUPS_PER_NODE];
...
uint64_t msg_InjFifoInject(msg_InjFifoHandle_t injFifoHandle, uint32_t relativeFifoId,
  ...
  uint64_t rc =
      MUSPI_InjFifoInject(MUSPI_IdToInjFifo(globalFifoId % BGQ_MU_NUM_INJ_FIFOS_PER_SUBGROUP,
                                            &info->subgroup[subgroupId]),
                          descPtr);
  return rc;
}

void msg_InjFifoTerm(msg_InjFifoHandle_t injFifoHandle) { return; /*Simple library do nothing! */ }
...
int msg_InjFifoInit(msg_InjFifoHandle_t *injFifoHandlePtr, uint32_t startingSubgroupId,
  ...
  while (numFifos > 0) {
    ...
    // Allocate the fifos
    rc = Kernel_AllocateInjFifos(subgroupId, &info->subgroup[subgroupId], numFifosInSubgroup,
                                 fifoIds, attrs);
    if (rc) {
    ...
    for (unsigned int i = 0; i < numFifosInSubgroup; i++) {
      Kernel_MemoryRegion_t memRegion;
      rc = Kernel_CreateMemoryRegion(&memRegion, info->fifoPtr[numFifos - i - 1], fifoSize);
      if (rc) {
      ...
      rc = Kernel_InjFifoInit(
          &info->subgroup[subgroupId], fifoIds[i], &memRegion,
          (uint64_t)info->fifoPtr[numFifos - i - 1] - (uint64_t)memRegion.BaseVa, fifoSize - 1);
      if (rc) {
    ...
    // Activate the fifos.
    rc = Kernel_InjFifoActivate(&info->subgroup[subgroupId], numFifosInSubgroup, fifoIds,
