...
/* FIXME temporary includes and declarations until IO and interface for invert and CGMMS are
 * generelized */
#include <io/params.h>
#include <io/spinor.h>
...
static void cgmms_write_props(spinor **const P, double const *const extra_masses,
                              const int no_extra_masses, const int id, const int iteration);
...
int invert_eo(spinor *const Even_new, spinor *const Odd_new, spinor *const Even, spinor *const Odd,
  ...
  if (external_inverter == QUDA_INVERTER) {
    return invert_eo_quda(Even_new, Odd_new, Even, Odd, precision, max_iter, solver_flag, rel_prec,
                          even_odd_flag, solver_params, sloppy, compression);
  }
#endif
  ...
  if (even_odd_flag) {
    ...
    if (external_inverter == QPHIX_INVERTER) {
      ...
      iter = invert_eo_qphix_oneflavour(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision,
                                        solver_flag, rel_prec, solver_params, sloppy, compression);
      // for solver_params.solution_type == TM_SOLUTION_M (the default)
      // QPhiX applies M(mu)^dag internally for normal equation solves, no call to tmLQCD operaor
      // required
    } else
#endif
      ...
      iter = bicgstab_complex(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec,
                              VOLUME / 2, &Mtm_plus_sym_psi);
      ...
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter = bicg_complex(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec,
                          VOLUME / 2, &Mtm_plus_sym_psi, &Mtm_plus_sym_dagg_psi);
    }

    else if (solver_flag == GMRES) {
      if (g_proc_id == 0) {
        printf("# Using GMRES! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter =
          gmres(Odd_new, g_spinor_field[DUM_DERI], gmres_m_parameter, max_iter / gmres_m_parameter,
                precision, rel_prec, VOLUME / 2, 1, &Mtm_plus_sym_psi);
    } else if (solver_flag == GCR) {
      if (g_proc_id == 0) {
        printf("# Using GCR! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter = gcr(Odd_new, g_spinor_field[DUM_DERI], gmres_m_parameter, max_iter / gmres_m_parameter,
                 precision, rel_prec, VOLUME / 2, 0, &Mtm_plus_sym_psi);
    } else if (solver_flag == MCR) {
      if (g_proc_id == 0) {
        printf("# Using MCR! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME / 2);
      iter = mcr(Odd_new, g_spinor_field[DUM_DERI], gmres_m_parameter, max_iter / gmres_m_parameter,
                 precision, rel_prec, VOLUME / 2, 0, &Qtm_pm_psi);
      Qtm_minus_psi(Odd_new, Odd_new);
    } else if (solver_flag == GMRESDR) {
      if (g_proc_id == 0) {
        printf("# Using GMRES-DR! m = %d, NrEv = %d\n", gmres_m_parameter, gmresdr_nr_ev);
        fflush(stdout);
      }
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter = gmres_dr(Odd_new, g_spinor_field[DUM_DERI], gmres_m_parameter, gmresdr_nr_ev,
                      max_iter / gmres_m_parameter, precision, rel_prec, VOLUME / 2,
                      &Mtm_plus_sym_psi);
      ...
      iter = fgmres(Odd_new, g_spinor_field[DUM_DERI], gmres_m_parameter,
                    max_iter / gmres_m_parameter, precision, rel_prec, VOLUME / 2, 0, &Qtm_pm_psi);
      gamma5(Odd_new, Odd_new, VOLUME / 2);
      Qtm_minus_psi(Odd_new, Odd_new);
    } else if (solver_flag == BICGSTABELL) {
      if (g_proc_id == 0) {
        printf("# Using BiCGstab2!\n");
        fflush(stdout);
      }
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter = bicgstabell(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, 3,
                         VOLUME / 2, &Mtm_plus_sym_psi);
      ...
      iter = pcg_her(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2,
                     &Qtm_pm_psi);
      Qtm_minus_psi(Odd_new, Odd_new);
      ...
      iter = incr_eigcg(VOLUME / 2, solver_params.eigcg_nrhs, solver_params.eigcg_nrhs1, Odd_new,
                        g_spinor_field[DUM_DERI], solver_params.eigcg_ldh, &Qtm_pm_psi,
                        solver_params.eigcg_tolsq1, solver_params.eigcg_tolsq,
                        solver_params.eigcg_restolsq, solver_params.eigcg_rand_guess_opt, rel_prec,
                        max_iter, solver_params.eigcg_nev, solver_params.eigcg_vmax);
      Qtm_minus_psi(Odd_new, Odd_new);
    } else if (solver_flag == MIXEDCG) {
      /* Here we invert the hermitean operator squared */
      gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME / 2);
      if (g_proc_id == 0) {
        printf("# Using Mixed Precision CG!\n");
        fflush(stdout);
      }
      iter = mixed_cg_her(Odd_new, g_spinor_field[DUM_DERI], solver_params, max_iter, precision,
                          rel_prec, VOLUME / 2, &Qtm_pm_psi, &Qtm_pm_psi_32);
      Qtm_minus_psi(Odd_new, Odd_new);
      ...
      iter = rg_mixed_cg_her(Odd_new, g_spinor_field[DUM_DERI], solver_params, max_iter, precision,
                             rel_prec, VOLUME / 2, &Qtm_pm_psi, &Qtm_pm_psi_32);
      Qtm_minus_psi(Odd_new, Odd_new);
      ...
      iter = cg_her(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2,
                    &Qtm_pm_psi);
      Qtm_minus_psi(Odd_new, Odd_new);
      ...
      iter = mr(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2, 1,
                &Mtm_plus_psi);
      ...
      mul_one_pm_imu_inv(g_spinor_field[DUM_DERI], +1., VOLUME / 2);
      iter = cgs_real(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2,
                      &Mtm_plus_sym_psi);
    } else {
      if (g_proc_id == 0) {
        printf("# Using CG as default solver!\n");
        fflush(stdout);
      }
      gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME / 2);
      iter = cg_her(Odd_new, g_spinor_field[DUM_DERI], max_iter, precision, rel_prec, VOLUME / 2,
    ...
    if (solver_flag == BICGSTAB) {
      ...
      if (use_preconditioning == 1 && g_precWS != NULL) {
        ...
        iter = bicgstab_complex(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], max_iter,
                                precision, rel_prec, VOLUME, &D_psi_prec);
      } else {
        if (g_proc_id == 0) {
          printf("# Not using preconditioning (which one?)!\n");
        }
        iter = bicgstab_complex(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], max_iter,
                                precision, rel_prec, VOLUME, &D_psi);
      }
    } else if (solver_flag == BICG) {
      if (g_proc_id == 0) {
        printf("# Using BiCG!\n");
      ...
      if (use_preconditioning == 1 && g_precWS != NULL) {
        ...
        // iter = bicg_complex(g_spinor_field[DUM_DERI+1], g_spinor_field[DUM_DERI], max_iter,
        // precision, rel_prec, VOLUME, &D_psi_prec, &D_dagg_psi_prec);

        if (g_proc_id == 0) {
          printf("# Not using preconditioning (which one?)!\n");
        }
        iter = bicg_complex(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], max_iter,
                            precision, rel_prec, VOLUME, &D_psi, &D_dagg_psi);
      } else {
        if (g_proc_id == 0) {
          printf("# Not using preconditioning (which one?)!\n");
        }
      ...
      if (use_preconditioning == 1 && g_precWS != NULL) {
        ...
        iter = cgs_real(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], max_iter, precision,
                        rel_prec, VOLUME, &D_psi_prec);
      } else {
        if (g_proc_id == 0) {
          printf("# Not using preconditioning (which one?)!\n");
        }
        iter = cgs_real(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], max_iter, precision,
                        rel_prec, VOLUME, &D_psi);
      }
      ...
      if (use_preconditioning == 1 && g_precWS != NULL) {
        ...
        iter = gmres(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                     max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, 1, &D_psi_prec);
      } else {
        if (g_proc_id == 0) {
          printf("# not using preconditioning (which one?)!\n");
        }
        iter = gmres(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                     max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, 1, &D_psi);
      }
    } else if (solver_flag == MIXEDCG) {
      if (g_proc_id == 0) {
        printf("# Using MIXEDCG!\n");
        fflush(stdout);
      }
      gamma5(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], VOLUME);
      iter = mixed_cg_her(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1], solver_params,
                          max_iter, precision, rel_prec, VOLUME, &Q_pm_psi, &Q_pm_psi_32);
      ...
      gamma5(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], VOLUME);
      iter = rg_mixed_cg_her(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI + 1], solver_params,
                             max_iter, precision, rel_prec, VOLUME, &Q_pm_psi, &Q_pm_psi_32);
      Q_minus_psi(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI]);
    } else if (solver_flag == FGMRES) {
      if (g_proc_id == 0) {
        printf("# Using FGMRES! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      iter = fgmres(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                    max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, Msap_precon, &D_psi);
    } else if (solver_flag == GCR) {
      if (g_proc_id == 0) {
        printf("# Using GCR! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      ...
      /* iter = mcr(g_spinor_field[DUM_DERI+1], g_spinor_field[DUM_DERI], gmres_m_parameter,
       * max_iter/gmres_m_parameter, precision, rel_prec, VOLUME, 0, &D_psi); */
      gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME);
      iter = mcr(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                 max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, 0, &Q_plus_psi);
    } else if (solver_flag == CR) {
      if (g_proc_id == 0) {
        printf("# Using CR and iQ!\n");
        fflush(stdout);
      }
      gamma5(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], VOLUME);
      iter = cr(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, 0, &Q_plus_psi);
      /* Solve DdaggD */
      /* gamma5(g_spinor_field[DUM_DERI+1], g_spinor_field[DUM_DERI], VOLUME);
         iter = cr(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI+1], gmres_m_parameter,
         max_iter/gmres_m_parameter, precision, rel_prec, VOLUME, 0, &Q_pm_psi);
         Q_minus_psi(g_spinor_field[DUM_DERI+1], g_spinor_field[DUM_DERI]);
      */
    } else if (solver_flag == DFLGCR) {
      if (g_proc_id == 0) {
        printf("# Using deflated GCR solver! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      iter = gcr(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
      ...
      if (g_proc_id == 0) {
        printf("# Using deflated FGMRES solver! m = %d\n", gmres_m_parameter);
        fflush(stdout);
      }
      iter = fgmres(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI], gmres_m_parameter,
                    max_iter / gmres_m_parameter, precision, rel_prec, VOLUME, 2, &D_psi);
      ...
      shifts[0] = g_mu;
      for (int i = 0; i < no_extra_masses; ++i) shifts[i + 1] = extra_masses[i];
      g_mu = 0;
      ...
      if (use_preconditioning == 1 && g_precWS != NULL) {
        ...
        if (g_prec_sequence_d_dagger_d[2] != 0.0) {
          ...
          spinorPrecondition(g_spinor_field[DUM_DERI + 1], g_spinor_field[DUM_DERI + 1], ws, T, L,
                             alpha, 0, 1);
        }
        ...
        if (g_prec_sequence_d_dagger_d[0] != 0.0) {
          ...
          spinorPrecondition(g_spinor_field[DUM_DERI], g_spinor_field[DUM_DERI], ws, T, L, alpha, 0,
                             1);
        }
...
/* FIXME temporary solution for the writing of CGMMS propagators until the input/output interface
   for invert_eo has been generalized NOTE that no_shifts = no_extra_masses+1 */
static void cgmms_write_props(spinor **const P, double const *const shifts, const int no_shifts,
                              const int id, const int iteration) {
  int append = 0;
  char filename[300];
  WRITER *writer = NULL;
  ...
  for (int im = 0; im < no_shifts; im++) {
    if (SourceInfo.type != SRC_TYPE_VOL) {
      if (PropInfo.splitted) {
        if (T_global > 99)
          sprintf(filename, "%s.%.2d.%.4d.%.3d.%.2d.cgmms.%.2d.inverted", PropInfo.basename, id,
                  SourceInfo.nstore, SourceInfo.t, SourceInfo.ix, im);
        else
          sprintf(filename, "%s.%.2d.%.4d.%.2d.%.2d.cgmms.%.2d.inverted", PropInfo.basename, id,
                  SourceInfo.nstore, SourceInfo.t, SourceInfo.ix, im);
      } else {
        sprintf(filename, "%s.%.2d.%.4d.%.2d.cgmms.%.2d.inverted", PropInfo.basename, id,
                SourceInfo.nstore, SourceInfo.t, im);
      }
    } else {
      sprintf(filename, "%s.%.2d.%.4d.%.5d.cgmms.%.2d.0", PropInfo.basename, id, SourceInfo.nstore,
              SourceInfo.sample, im);
    }
