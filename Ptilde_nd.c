...
void Ptilde_cheb_coefs(double aa, double bb, double dd[], int n, double exponent) {
  ...
  for (j = 0; j < n; j++) {
    double sum = 0.0;
    for (k = 0; k < n; k++) sum += f[k] * cos(PI * j * (k + 0.5) * inv_n);
    dd[j] = fac * sum;
  }
  free(f);
}
...
void Ptilde_ndpsi(spinor *R_s, spinor *R_c, double *dd, int n, spinor *S_s, spinor *S_c,
  ...
  for (int j = n - 1; j >= 1; j--) {
    ...
    double temp1 = -1.0;
    double temp2 = dd[j];
    assign_mul_add_mul_add_mul_add_mul_r(&ds[0], &R_s[0], &dds[0], &aux3s[0], fact2, fact1, temp1,
                                         temp2, VOLUME / 2);
    assign_mul_add_mul_add_mul_add_mul_r(&dc[0], &R_c[0], &ddc[0], &aux3c[0], fact2, fact1, temp1,
                                         temp2, VOLUME / 2);
  ...
  assign_mul_add_mul_add_mul_add_mul_r(&auxs[0], &ds[0], &dds[0], &aux3s[0], temp3, temp4, temp1,
                                       temp2, VOLUME / 2);
  assign_mul_add_mul_add_mul_add_mul_r(&auxc[0], &dc[0], &ddc[0], &aux3c[0], temp3, temp4, temp1,
                                       temp2, VOLUME / 2);
...
void degree_of_Ptilde(int *_degree, double **coefs, const double EVMin, const double EVMax,
                      const int sloppy_degree, const double acc, matrix_mult_nd Qsq,
                      const int repro) {
  tm_stopwatch_push(&g_timers, __func__, "");
  int i, j;
  double temp, temp2;
  ...
  *coefs = calloc(phmc_max_ptilde_degree, sizeof(double));
  ...
  if (g_debug_level > 2) {
    ...
    if (g_proc_id == g_stdio_proc) {
      printf(
          "# NDPOLY Acceptance Polynomial: Delta_IR at s=%f: | Ptilde P s_low P Ptilde - 1 |/2 = "
          "%e \n",
          EVMin, temp);
    }
  }
