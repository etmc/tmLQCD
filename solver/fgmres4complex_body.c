...
int _PSWITCH(fgmres4complex)(_Complex _F_TYPE *const P, _Complex _F_TYPE *const Q, const int m,
                             const int max_restarts, const double eps_sq, const int rel_prec,
                             const int N, const int parallel, const int lda, const int precon,
                             _PSWITCH(c_matrix_mult) f) {
  int restart, i, j, k;
  ...
  for (restart = 0; restart < max_restarts; restart++) {
    ...
    if (g_proc_id == g_stdio_proc && g_debug_level > 2) {
      printf("lFGMRES %d\t%g true residue\n", restart * m,
             creal(_PSWITCH(alpha)[0]) * creal(_PSWITCH(alpha)[0]));
      fflush(stdout);
    }
    ...
    for (j = 0; j < m; j++) {
      ...
      beta = sqrt(creal(_PSWITCH(H)[j][j] * conj(_PSWITCH(H)[j][j])) +
                  creal(_PSWITCH(H)[j + 1][j] * conj(_PSWITCH(H)[j + 1][j])));
      ...
      if (g_proc_id == g_stdio_proc && g_debug_level > 2) {
        printf("lFGMRES\t%d\t%g iterated residue\n", restart * m + j,
               creal(_PSWITCH(alpha)[j + 1]) * creal(_PSWITCH(alpha)[j + 1]));
        fflush(stdout);
      }
      ...
      if ((fltcntr > 20) || ((creal(_PSWITCH(alpha)[j + 1]) <= eps) && (rel_prec == 0)) ||
          ((creal(_PSWITCH(alpha)[j + 1]) <= eps * norm) && (rel_prec == 1))) {
      ...
      /* if not */
        ...
        if (j != m - 1) {
          _PSWITCH(lmul_r)(_PSWITCH(V)[(j + 1)], 1. / creal(_PSWITCH(H)[j + 1][j]), solver_field[1],
                           N);
        }
      }
    }
