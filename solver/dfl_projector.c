...
void project(spinor *const out, spinor *const in) {
  ...
  if (!dfl_sloppy_prec)
    prec = little_solver_high_prec;
  else
    prec = little_solver_low_prec;
  ...
  if (!usePL) {
    if (evenodd) {
      ...
      if (gcr32) {
        ...
        iter = gcr4complex(invvec, inprod, little_m, little_max_iter, prec, 1, nb_blocks * g_N_s, 1,
    ...
    if (evenodd) {
      ...
      little_P_L(v, inprod);
      iter = gcr4complex(w, v, little_m, little_max_iter, prec, 1, nb_blocks * g_N_s, 1,
                         nb_blocks * 9 * g_N_s, 0, &little_P_L_D);
      little_P_R(v, w);
      little_project(w, inprod, g_N_s);
      for (int i = 0; i < nb_blocks * g_N_s; ++i) invvec[i] = w[i] + v[i];
      if (g_proc_id == 0 && g_debug_level > 0) {
...
static void alloc_dfl_projector() {
  if (init_dfl_projector == 0) {
    ...
    inprod = calloc(nb_blocks * 9 * g_N_s,
                    sizeof(_Complex double)); /*inner product of spinors with bases */
    inprod32 = calloc(nb_blocks * 9 * g_N_s,
                      sizeof(_Complex float)); /*inner product of spinors with bases */
    inprod_eo = calloc(nb_blocks * 9 * g_N_s,
                       sizeof(_Complex double)); /*inner product of spinors with bases */
    inprod_o = calloc(nb_blocks * 9 * g_N_s,
                      sizeof(_Complex double)); /*inner product of spinors with bases */
    inprod_o32 = calloc(nb_blocks * 9 * g_N_s,
                        sizeof(_Complex float)); /*inner product of spinors with bases */
    inprod_e = calloc(nb_blocks * 9 * g_N_s,
                      sizeof(_Complex double)); /*inner product of spinors with bases */
    ctmp = calloc(nb_blocks * 9 * g_N_s,
                  sizeof(_Complex double)); /*inner product of spinors with bases */
    invvec = calloc(nb_blocks * 9 * g_N_s,
                    sizeof(_Complex double)); /*inner product of spinors with bases */
    invvec32 = calloc(nb_blocks * 9 * g_N_s,
                      sizeof(_Complex float)); /*inner product of spinors with bases */
    invvec_eo = calloc(nb_blocks * 9 * g_N_s,
                       sizeof(_Complex double)); /*inner product of spinors with bases */
    invvec_eo32 = calloc(nb_blocks * 9 * g_N_s,
                         sizeof(_Complex float)); /*inner product of spinors with bases */
...
void project2(spinor *const out, spinor *const in) {
  ...
  for (j = 1; j < g_N_s; j++) {
    for (i = 0; i < nb_blocks; i++)
      assign_add_mul(psi[i], block_list[i].basis[j], inprod[i * g_N_s + j], vol);
  }
...
int check_projectors(const int repro) {
  ...
  if (g_cart_id == 0) {
    ...
    printf("\n######################\n");
    printf(
        "# The following tests are only meaningful up to the precision little_D can be inverted "
        "for.\n");
    printf(
        "# They might, therefore, be only useful in a small volume and/or a small condition number "
        "of little_D\n");
...
void check_little_D_inversion(const int repro) {
  ...
  if (g_proc_id == 0) {
    ...
    printf(
        "# This test might be only meaningful for not too large condition number of little_D\n\n");
  }

  result = calloc(nb_blocks * 9 * g_N_s,
                  sizeof(_Complex double)); /*inner product of spinors with bases */
  ...
  gcr4complex(invvec, inprod, little_gmres_m_parameter, 1000, 1.e-24, 0, nb_blocks * g_N_s, 1,
              nb_blocks * 9 * g_N_s, 0, &little_D);
...
void check_local_D(const int repro) {
  ...
  for (j = 0; j < nb_blocks; j++) {
    ...
    if (sqrt(nrm) > 1.e-12) {
      printf(
          "# Check failed for local D against Hopping Matrix: ||delta|| = %1.5e block %d process "
          "%d\n",
