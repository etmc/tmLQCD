...
int mixed_cg_mms_tm_nd(spinor** const Pup, spinor** const Pdn, spinor* const Qup, spinor* const Qdn,
  ...
  if ((g_cart_id == 0 && g_debug_level > 2))
    printf("# CGMMSND_mixed: Initial mms residue: %.6e\n", rr);
  if (rr < 1.0e-4) {
    if ((g_cart_id == 0 && g_debug_level > 2))
      printf("# CGMMSND_mixed: norm of source too low: falling back to double mms solver %.6e\n",
             rr);
  ...
  // Pup/dn  can be used as auxiliary field to work on, as it is not later used (could be used as
  // initial guess at the very start) Q_up/dn  can be used as feedback, or if not, also as auxiliary
  // field
  ...
  */
  ...
  for (int im = 1; im < noshifts; im++) {
    ...
    if (g_cart_id == 0 && g_debug_level > 2)
      printf("# CGMMSND_mixed: shift %d is %e\n", im, sigma[im]);
  ...
  for (j = 0; j < max_iter; j++) {
    ...
    if ((g_cart_id == 0) && (g_debug_level > 2))
      printf("# CGMMSND_mixed: mms iteration j = %i: rr = %.6e\n", j, rr);
    ...
    if (((check_abs) && (rr <= eps_sq)) || ((check_rel) && (rr <= eps_sq * r0r0))) {
      if ((check_rel) && (rr <= eps_sq * r0r0)) {
        if ((g_cart_id == 0) && (g_debug_level > 3))
          printf("# CGMMSND_mixed: Reached relative solver precision of eps_rel = %.2e\n", eps_sq);
      }
      break;
    }
    ...
    for (int im = (noshifts - 1); im >= 0; im--) {
      ...
      if ((res[im] < rel_delta * res0[im]) && (res0[im] <= maxres[im]) && (use_reliable))
        rel_update = 1;
    ...
    if (!rel_update) {
      ...
      rr = rr_up + rr_dn;
      if ((g_cart_id == 0) && (g_debug_level > 3))
        printf("# CGMMSND_mixed: New residue after reliable update: %.6e\n", rr);
      ...
      if (res[trigger_shift] > res0[trigger_shift]) {
        if (g_cart_id == 0)
          printf("# CGMMSND_mixed: Warning: residue of shift no %d got larger after rel. update\n",
                 trigger_shift);
...
static void init_mms_tm_nd_32(const unsigned int _nr, const unsigned int N) {
  if (ini_mms_nd == 0 || _nr > nr_nd) {
    ...
    for (int i = 0; i < nr_nd; i++) {
      mms_x_up[i] =
          (spinor32*)(((unsigned long int)(x_up_qmms) + ALIGN_BASE32) & ~ALIGN_BASE32) + i * N;
      mms_x_dn[i] =
          (spinor32*)(((unsigned long int)(x_dn_qmms) + ALIGN_BASE32) & ~ALIGN_BASE32) + i * N;
      mms_d_up[i] =
          (spinor32*)(((unsigned long int)(d_up_qmms) + ALIGN_BASE32) & ~ALIGN_BASE32) + i * N;
      mms_d_dn[i] =
          (spinor32*)(((unsigned long int)(d_dn_qmms) + ALIGN_BASE32) & ~ALIGN_BASE32) + i * N;
    }
    ini_mms_nd = 1;
  }
}
