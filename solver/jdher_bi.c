 ...
 *                                                                          *
 ****************************************************************************/
static void print_status(int clvl, int it, int k, int j, int kmax, int blksize, int actblksize,
                         double *s, double *resnrm, int *actcorrits);
static void sorteig(int j, double S[], _Complex double U[], int ldu, double tau, double dtemp[],
                    int idx1[], int idx2[], int strategy);
...
static char *fupl_u = "U", *fupl_n = "N", *fupl_a = "A", *fupl_v = "V", *filaenv = "zhetrd",
            *fvu = "VU";
...
void jdher_bi(int n, int lda, double tau, double tol, int kmax, int jmax, int jmin, int itmax,
              ...
              _Complex double *Q, double *lambda, int *it, int maxmin, const int shift_mode,
  ...
  int i, k, j, actblksize, eigworklen, found, conv, keep, n2,
      N = n * sizeof(_Complex double) / sizeof(bispinor);
  ...
  while ((*it) < itmax) {
    ...
    for (act = 0; act < actblksize; act++) {
      ...
      if (solver_flag == BICGSTAB) {
        info = bicgstab_complex_bi((bispinor *)v, (bispinor *)r, linitmax, it_tol * it_tol,
                                   g_relative_precision_flag, VOLUME / 2, &Proj_A_psi_bi);
      } else if (solver_flag == CG) {
        info = cg_her_bi((bispinor *)v, (bispinor *)r, linitmax, it_tol * it_tol,
                         g_relative_precision_flag, VOLUME / 2, &Proj_A_psi_bi);
      } else {
        info = bicgstab_complex_bi((bispinor *)v, (bispinor *)r, linitmax, it_tol * it_tol,
                                   g_relative_precision_flag, VOLUME / 2, &Proj_A_psi_bi);
    ...
    if (g_proc_id == 0) {
      print_status(verbosity, *it, k, j - blksize, kmax, blksize, actblksize, s, resnrm,
                   actcorrits);
    }
 ...
 */
static void print_status(int verbosity, int it, int k, int j, int kmax, int blksize, int actblksize,
                         double *s, double *resnrm, int *actcorrits) {
...
void Proj_A_psi_bi(bispinor *const y, bispinor *const x) {
  ...
  /* y = y - Q*p_work_bi */
  _FT(zgemv)(fupl_n, &p_n, &p_k, &CMONE, p_Q_bi, &p_lda, (_Complex double *)p_work_bi, &ONE, &CONE,
