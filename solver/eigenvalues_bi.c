...
double eigenvalues_bi(int *nr_of_eigenvalues, const int max_iterations, const double precision,
  ...
  jdher_bi((VOLUME) / 2 * sizeof(bispinor) / sizeof(_Complex double),
           (VOLUME) / 2 * sizeof(bispinor) / sizeof(_Complex double), startvalue, prec,
           (*nr_of_eigenvalues), j_max, j_min, max_iterations, blocksize, blockwise, v0dim,
           (_Complex double *)eigenvectors_bi, BICGSTAB, solver_it_max, threshold, decay, verbosity,
           &converged, (_Complex double *)eigenvectors_bi, eigenvls_bi, &returncode, maxmin, 1,
           Qsq);
