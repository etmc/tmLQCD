...
int update_dfl_subspace(const int Ns, const int N, const int Nsmooth) {
  ...
  for (int j = 0; j < Nsmooth; j++) {
    ...
    for (int i = 0; i < Ns; i++) {
      ...
      ModifiedGS((_Complex double *)g_spinor_field[0], vol, i, (_Complex double *)dfl_fields[0],
                 vpr);
  ...
  if (usePL) {
    ...
    if (!little_evenodd) {
      ...
      for (int i = 0; i < Ns; i++) {
        ...
        for (int j = 0; j < Ns; j++) {
          for (int blk = 0; blk < nb_blocks; blk++) {
            little_dfl_fields[i][j + blk * Ns] =
                scalar_prod(block_list[blk].basis[j], psi[blk], block_list[0].volume, 0);
          }
        }
      }
      ...
      for (int i = 0; i < Ns; i++) {
        ...
        for (int blk = 0; blk < nb_blocks; blk++) {
          if (block_list[blk].evenodd == 1) {
            for (int j = 0; j < Ns; j++) {
              little_dfl_fields_eo[i][(nb_blocks / 2 + i_o) * Ns + j] =
                  scalar_prod(block_list[blk].basis[j], psi[blk], block_list[0].volume, 0);
            }
            i_o++;
          }
      ...
      for (int i = 0; i < Ns; i++) {
        ...
        lmul_r(little_dfl_fields_eo[i], 1. / sqrt(creal(s)), little_dfl_fields_eo[i],
               nb_blocks * Ns);
      }
      ...
      for (int i = 0; i < Ns; i++) {
        ...
        for (int j = 0; j < Ns; j++) {
          ...
          if (g_proc_id == 0 && g_debug_level > 4) {
            printf("%1.3e %1.3ei, ", creal(little_A_eo[i * Ns + j]),
                   cimag(little_A_eo[i * Ns + j]));
          }
        }
        if (g_proc_id == 0 && g_debug_level > 4) printf("\n");
      }
...
int init_little_dfl_subspace(const int N_s) {
  if (init_little_subspace == 0) {
    // why nb_blocks*9*N_s?
    if ((void *)(_little_dfl_fields = (_Complex double *)calloc((N_s)*nb_blocks * 9 * N_s + 4,
                                                                sizeof(_Complex double))) == NULL) {
      return (1);
    }
    if ((void *)(little_dfl_fields = (_Complex double **)calloc(N_s, sizeof(_Complex double *))) ==
        NULL) {
      return (1);
    }
    // why do we need this factor of 9??
    if ((void *)(_little_dfl_fields_eo = (_Complex double *)calloc(
                     (N_s)*nb_blocks * 9 * N_s + 4, sizeof(_Complex double))) == NULL) {
      return (1);
    }
    if ((void *)(little_dfl_fields_eo =
                     (_Complex double **)calloc(N_s, sizeof(_Complex double *))) == NULL) {
      return (1);
    }
    // why do we need this factor of 9??
    if ((void *)(_little_dfl_fields_eo_32 = (_Complex float *)calloc(
                     (N_s)*nb_blocks * 9 * N_s + 4, sizeof(_Complex float))) == NULL) {
      return (1);
    }
    if ((void *)(little_dfl_fields_eo_32 =
                     (_Complex float **)calloc(N_s, sizeof(_Complex float *))) == NULL) {
      return (1);
    }

    little_dfl_fields[0] =
        (_Complex double *)(((unsigned long int)(_little_dfl_fields) + ALIGN_BASE) & ~ALIGN_BASE);
    little_dfl_fields_eo[0] =
        (_Complex double *)(((unsigned long int)(_little_dfl_fields_eo) + ALIGN_BASE) &
                            ~ALIGN_BASE);
    little_dfl_fields_eo_32[0] =
        (_Complex float *)(((unsigned long int)(_little_dfl_fields_eo_32) + ALIGN_BASE) &
                           ~ALIGN_BASE);
    for (int i = 1; i < N_s; i++) {
    ...
    if ((void *)(little_A = (_Complex double *)calloc(N_s * N_s, sizeof(_Complex double))) ==
        NULL) {
      return (1);
    }
    if ((void *)(little_A_eo = (_Complex double *)calloc(N_s * N_s, sizeof(_Complex double))) ==
        NULL) {
      return (1);
    }
    if ((void *)(little_A_eo_32 = (_Complex float *)calloc(N_s * N_s, sizeof(_Complex float))) ==
        NULL) {
      return (1);
    }
