...
int cgs_real(spinor* const P, spinor* const Q, const int max_iter, double eps_sq,
  ...
  for (i = 0; i <= max_iter; i++) {
    ...
    if (((res_sq < eps_sq) && (rel_prec == 0)) ||
        ((res_sq < eps_sq * squarenorm) && (rel_prec == 1))) {
    ...
    alpha = rjr0 / denom;
    /* calc q */
    mul_diff_mul_r(solver_field[4], solver_field[2], solver_field[3], one, alpha, N);
    /* calc P and residual */
    ...
    beta = nom / rjr0;
    /* calc new u */
    mul_diff_mul_r(solver_field[2], solver_field[0], solver_field[4], one, -beta, N);
    /* calc new p */
    /* _mult_assign_complex(nom,beta,beta); */
    nom = beta * beta;
    mul_r(solver_field[1], nom, solver_field[1], N);
    assign_add_mul_add_mul_r(solver_field[1], solver_field[4], solver_field[2], beta, one, N);
  }
  finalize_solver(solver_field, nr_sf);
  return -1;
}
