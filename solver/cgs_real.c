...
/*   init_solver_field(6); */
  ...
  for(i=0;i<=max_iter;i++) {
    ...
    rjr0.re=scalar_prod_r(g_spinor_field[DUM_SOLVER],g_spinor_field[DUM_SOLVER+5], N);
    ...
    /* calc alpha */
    denom.re=scalar_prod_r(g_spinor_field[DUM_SOLVER+3],g_spinor_field[DUM_SOLVER+5], N);
    /* _div_complex(alpha,rjr0,denom);*/
    alpha.re=rjr0.re/denom.re;
    /* calc q */
    mul_diff_mul(g_spinor_field[DUM_SOLVER+4],g_spinor_field[DUM_SOLVER+2],g_spinor_field[DUM_SOLVER+3],one,alpha, N); 
    /* calc P and residual */
    /* calc alpha(u+q) into sf2 */
    assign_add_mul_r(g_spinor_field[DUM_SOLVER+2],g_spinor_field[DUM_SOLVER+4],1., N);
    assign_add_mul(P,g_spinor_field[DUM_SOLVER+2],alpha, N); /* new P */
    /* calc new residual */
    f(g_spinor_field[DUM_SOLVER+3],g_spinor_field[DUM_SOLVER+2]);
    assign_diff_mul(g_spinor_field[DUM_SOLVER],g_spinor_field[DUM_SOLVER+3],alpha, N);
    /* calc beta */
    nom.re=scalar_prod_r(g_spinor_field[DUM_SOLVER],g_spinor_field[DUM_SOLVER+5], N);
    /* _div_complex(beta,nom,rjr0); */
    beta.re=nom.re/rjr0.re;
    /* calc new u */
    mul_add_mul(g_spinor_field[DUM_SOLVER+2],g_spinor_field[DUM_SOLVER],g_spinor_field[DUM_SOLVER+4],one,beta, N);
    /* calc new p */
    /* _mult_assign_complex(nom,beta,beta); */
    nom.re=beta.re*beta.re;
    mul(g_spinor_field[DUM_SOLVER+1],nom,g_spinor_field[DUM_SOLVER+1], N);	
    assign_add_mul_add_mul(g_spinor_field[DUM_SOLVER+1],g_spinor_field[DUM_SOLVER+4],g_spinor_field[DUM_SOLVER+2],beta,one, N);
  }
  return -1;
}
