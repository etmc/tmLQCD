...
/*-------------------------------------------------------------------------
  ...
  Parameters:
  ----------------------------------
  n        active problem size as a number of spinor components, The active part of A is an n-by-n
  spinors lde	   physical problem size (as spinors). A and vectors are stored in lde size spinors
...
static void displayInfo(float tol, int maxit, int flag, int iter, float resnorm) {
  if (flag != 0) {
  ...
  switch (flag) {
    case 0:
      if (iter == 0)
        fprintf(stdout,
                "The initial guess has relative residual %0.2g which is within\nthe desired "
                "tolerance %0.2g\n",
                resnorm, tol);
      else
        fprintf(stdout, "eigCG converged at iteration %d to a solution with residual norm %0.2g",
                iter, resnorm);
      break;
    case 1:
  ...
  if (flag != 0)
    fprintf(stdout, "\nThe iterate returned at iteration %d has residual norm %0.2g", iter,
            resnorm);
  ...
  *flag = 1;
  ...
  for (it = 0; it < maxit; it++) {
    ...
    *reshist = rho;
    ...
    /* Convergence test */
    if (((*reshist < eps_sq) && (rel_prec == 0)) ||
        ((*reshist < eps_sq * (*normb) * (*normb)) && (rel_prec == 1))) {
      *flag = 0;
    ...
    if (nev > 0 && v_size == v_max) assign(Ap_prev, Ap, n);
    /*---------------------------------------------------------------------*/
    ...
    if (nev > 0) {
      ...
      if (v_size == v_max) {
        ...
        _FT(zheev)(&cV, &cU, &tempi, Hevecsold, &v_max, Hevalsold, zwork, &lwork, rwork, &info, 1,
                   1);
        ...
        _FT(zcopy)(&allelems, H, &ONE, Hevecsold, &ONE);
        _FT(zunmqr)(&cR, &cN, &v_max, &v_max, &v_size, Hevecs, &v_max, TAU, Hevecsold, &v_max,
                    zwork, &lwork, &info);
        ...
        _FT(zunmqr)(&cL, &cC, &v_max, &v_size, &v_size, Hevecs, &v_max, TAU, Hevecsold, &v_max,
                    zwork, &lwork, &info);
        ...
        _FT(zunmqr)(&cL, &cN, &v_max, &v_size, &v_size, Hevecs, &v_max, TAU, Hevecsold, &v_max,
                    zwork, &lwork, &info);
