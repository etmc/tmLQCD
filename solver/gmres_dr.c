  ...
  for (restart = 1; restart < max_restarts; restart++) {
    ...
    if (g_proc_id == 0 && g_debug_level > 3) {
      printf("zgesv returned info = %d, c[m-1]= %e, %e , idx[m-1]=%d\n", info,
             creal(alpha[idx[m - 1] - 1]), cimag(alpha[idx[m - 1] - 1]), idx[m - 1]);
    }
    /* c - \bar H * d -> c */
    ...
    _FT(zgemm)("N", "N", &mp1, &ne, &_m, &cpone, work2[0], &mp1, work[0], &mp1, &czero, G[0], &mp1,
               1, 1);
    _FT(zgemm)("C", "N", &np1, &ne, &mp1, &cpone, work[0], &mp1, G[0], &mp1, &czero, H[0], &mp1, 1,
               1);
    ...
    /* V_k+1 = V_m+1 P_k+1 */
    /*     _FT(zgemm)("N", "N", &_N, &np1, &mp1, &cpone, (_Complex double*)V[0], &V2, work[0], &mp1,
     * &czero, (_Complex double*)Z[0], &V2, 1, 1);  */
    for (l = 0; l < np1; l++) {
        ...
        for (l = 0; l < np1; l++) {
          ...
          if (g_proc_id == 0) {
            printf("(V[%d], V[%d]) = %e %e %d %d %d %d %d %d %e %e\n", l, i, creal(tmp1),
                   cimag(tmp1), np1, mp1, ne, _m, _N, V2, creal(H[l][i]), cimag(H[l][i]));
          }
        }
      }
    }
...
_Complex double short_scalar_prod(_Complex double *const y, _Complex double *const x, const int N) {
  ...
  for (int ix = 0; ix < N; ++ix) res += conj(y[ix]) * x[ix];
  return (res);
}
