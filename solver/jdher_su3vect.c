 ...
 ***********************************************************************/
/*
 * Routines for the computation of eigensystems of the Laplacian operator, with Jacobi-Davidson
 *algo. Authors Luigi Scorzato, Marco Cristoforetti
 *
 *
 *******************************************************************************/
#ifdef HAVE_CONFIG_H
 ...
 *                                                                          *
 ****************************************************************************/
static void print_status_su3vect(int clvl, int it, int k, int j, int kmax, int blksize,
                                 int actblksize, double *s, double *resnrm, int *actcorrits);
...
static char *fupl_u = "U", *fupl_c = "C", *fupl_n = "N", *fupl_a = "A", *fupl_v = "V",
            *filaenv = "zhetrd", *fvu = "VU";

void jdher_su3vect(int n, int lda, double tau, double tol, int kmax, int jmax, int jmin, int itmax,
                   int blksize, int blkwise, int V0dim, _Complex double *V0, int solver_flag,
                   int linitmax, double eps_tr, double toldecay, int verbosity, int *k_conv,
                   _Complex double *Q, double *lambda, int *it, int maxmin, int shift_mode,
                   int tslice, matrix_mult_su3vect A_psi) {
  /*******************
   * Local variables *
   *******************/
  ...
  _Complex double *V;
  _Complex double *Vtmp = NULL, *U = NULL, *M = NULL, *Z = NULL, *Res_ = NULL, *Res,
  ...
  for (cnt = 0; cnt < j; cnt++) {
    ...
    for (i = 0; i < idummy; i++) {
      M[cnt * jmax + i] =
          scalar_prod_su3vect((su3_vector *)(V + i * lda), (su3_vector *)temp1, N, 1);
    }
  }
  ...
  while ((*it) < itmax) {
    ...
    while (found) {
      ...
      if (found) {
        /* Store Eigenvalues */
        for (act = 0; act < conv; act++) lambda[k + act] = s[convind[act]];
        /* Re-use non approximated Ritz-Values */
        for (act = 0; act < keep; act++) s[act] = s[keepind[act]];
        /* Shift the others in the right position */
        for (act = 0; act < (j - actblksize); act++) s[act + keep] = s[act + actblksize];
        ...
        for (act = 0; act < n; act = act + jmax) {
          ...
          _FT(zgemm)(fupl_n, fupl_n, &cnt, &idummy, &j, &CONE, Vtmp, &jmax, U + actblksize * jmax,
                     &jmax, &CZERO, V + act + keep * lda, &lda, 1, 1);
        }
        ...
        for (act = 0; act < conv; act++) {
          _FT(zlacpy)(fupl_a, &n, &ONE, Q + (k + convind[act]) * lda, &lda, Q + (k + act) * lda,
                      &lda, 1);
        }
      ...
      if (j + actblksize > jmax) {
        ...
        for (act = 0; act < n; act = act + jmax) { /* V = V * U(:,1:j) */
          ...
          _FT(zgemm)(fupl_n, fupl_n, &cnt, &j, &idummy, &CONE, Vtmp, &jmax, U, &jmax, &CZERO,
                     V + act, &lda, 1, 1);
        }
        _FT(zlaset)(fupl_a, &j, &j, &CZERO, &CONE, U, &jmax, 1);
        _FT(zlaset)(fupl_u, &j, &j, &CZERO, &CZERO, M, &jmax, 1);
        for (act = 0; act < j; act++) M[act * jmax + act] = s[act];
      }
    } /* while(found) */
    ...
    for (act = 0; act < actblksize; act++) {
      ...
      if (solver_flag == CG) {
        info = cg_her_su3vect((su3_vector *)v, (su3_vector *)r, linitmax, it_tol * it_tol, 0,
                              n * sizeof(_Complex double) / sizeof(su3_vector), tslice,
                              &Proj_A_psi_su3vect);
      }
    ...
    if (g_proc_id == 0) {
      print_status_su3vect(verbosity, *it, k, j - blksize, kmax, blksize, actblksize, s, resnrm,
                           actcorrits);
    }
 ...
 */
static void print_status_su3vect(int verbosity, int it, int k, int j, int kmax, int blksize,
                                 int actblksize, double *s, double *resnrm, int *actcorrits) {
...
static void sorteig_su3vect(int j, double S[], _Complex double U[], int ldu, double tau,
  ...
  switch (strategy) {
    case 0:
      for (i = 0; i < j; i++) dtemp[i] = fabs(S[i] - tau);
      break;
    case 1:
      for (i = 0; i < j; i++)
        if (S[i] < tau)
...
void Proj_A_psi_su3vect(su3_vector *const y, su3_vector *const x, int tslice) {
  ...
  /* p_work = Q^dagger*y */
  for (i = 0; i < p_k; i++) {
    p_work[i] = scalar_prod_su3vect((su3_vector *)(p_Q + i * p_lda), (su3_vector *)y,
                                    p_n * sizeof(_Complex double) / sizeof(su3_vector), 1);
  }
  /* y = y - Q*p_work */
  _FT(zgemv)(fupl_n, &p_n, &p_k, &CMONE, p_Q, &p_lda, (_Complex double *)p_work, &ONE, &CONE,
             (_Complex double *)y, &ONE, 1);
}
