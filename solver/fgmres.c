...
int fgmres(spinor *const P, spinor *const Q, const int m, const int max_restarts,
           const double eps_sq, const int rel_prec, const int N, const int precon, matrix_mult f) {
  int restart;
  int j = 0;
    ...
    for (j = 0; j < m; j++) {
      ...
      if (g_proc_id == g_stdio_proc && g_debug_level > 0) {
        printf("FGMRES\t%d\t%g iterated residue\n", restart * m + j,
               creal(alpha[j + 1]) * creal(alpha[j + 1]));
        fflush(stdout);
      }
      if (((creal(alpha[j + 1]) <= eps) && (rel_prec == 0)) ||
        ...
        if (g_proc_id == g_stdio_proc && g_debug_level > 0) {
          ...
          printf("FGMRES cumulative little solver iterations %d, average %e\n", cumiter_lgcr,
                 (double)cumiter_lgcr / (double)(restart * m + j));
