...
static void output_flops(const double seconds, const unsigned int N, const unsigned int iter_out,
                         const unsigned int iter_in_sp, const unsigned int iter_in_dp,
                         const double eps_sq);
...
static inline unsigned int inner_loop_high(spinor *const x_up, spinor *const x_dn,
                                           ...
                                           double *const rho1, const double delta, matrix_mult_nd f,
                                           const double eps_sq, const unsigned int N,
                                           const unsigned int iter, const unsigned int max_iter) {
  ...
  while (rho > delta * rhomax && j + iter <= max_iter) {
    ...
    assign_add_mul_r(x_up, p_up, alpha, N);
    assign_add_mul_r(x_dn, p_dn, alpha, N);
    assign_add_mul_r(r_up, q_up, -alpha, N);
    assign_add_mul_r(r_dn, q_dn, -alpha, N);
    rho = (square_norm(r_up, N, 1) + square_norm(r_dn, N, 1));
    beta = rho / *rho1;
    *rho1 = rho;
    assign_mul_add_r(p_up, beta, r_up, N);
    assign_mul_add_r(p_dn, beta, r_dn, N);
...
static inline unsigned int inner_loop(spinor32 *const x_up, spinor32 *const x_dn,
                                      ...
                                      spinor32 *const r_up, spinor32 *const r_dn, float *const rho1,
                                      const float delta, matrix_mult_nd32 f32, const float eps_sq,
                                      const unsigned int N, const unsigned int iter,
                                      const unsigned int max_iter, float alpha, float beta,
                                      MCG_PIPELINED_TYPE pipelined, MCG_PR_TYPE pr) {
  ...
  if (pipelined == MCG_NO_PIPELINED) {
    ...
    while (rho > delta * rhomax && j + iter <= max_iter) {
      ...
      alpha = rho / pro;
      assign_add_mul_r_32(x_up, p_up, alpha, N);
      assign_add_mul_r_32(x_dn, p_dn, alpha, N);
      assign_add_mul_r_32(r_up, q_up, -alpha, N);
      assign_add_mul_r_32(r_dn, q_dn, -alpha, N);
      rho = (square_norm_32(r_up, N, 1) + square_norm_32(r_dn, N, 1));
      // Polak-Ribiere computation of beta, claimed to be self-stabilising, positive effect so far
      // not observed or required
      if (pr == MCG_PR) {
        beta = alpha * (alpha * (square_norm_32(q_up, N, 1) + square_norm_32(q_dn, N, 1)) - pro) /
               *rho1;
      } else {
        beta = rho / *rho1;
      }
      *rho1 = rho;
      assign_mul_add_r_32(p_up, beta, r_up, N);
      assign_mul_add_r_32(p_dn, beta, r_dn, N);
      if (g_debug_level > 2 && g_proc_id == 0) {
    ...
    // e.g.: one could do the collective communication for sqrnrm(r) while other stuff is being
    // computed it is also self-initialising (alpha=0, beta=0 will work)
    while (rho > delta * rhomax && j + iter <= max_iter) {
      ++j;
      assign_add_mul_r_32(x_up, p_up, alpha, N);
      assign_add_mul_r_32(x_dn, p_dn, alpha, N);
      ...
      if (pr == MCG_PR) {
        beta =
            alpha * (alpha * (square_norm_32(q_up, N, 1) + square_norm_32(q_dn, N, 1)) - pro) / rho;
      } else {
        beta = rho / *rho1;
      }
      *rho1 = rho;
...
int rg_mixed_cg_her_nd(spinor *const P_up, spinor *const P_dn, spinor *const Q_up,
                       spinor *const Q_dn, solver_params_t solver_params, const int max_iter,
                       const double eps_sq, const int rel_prec, const int N, matrix_mult_nd f,
                       matrix_mult_nd32 f32) {
  ...
  phigh_up = solver_field[7];
  phigh_dn = solver_field[6];
  xhigh_up = solver_field[5];
  xhigh_dn = solver_field[4];
  rhigh_up = solver_field[3];
  rhigh_dn = solver_field[2];
  qhigh_up = solver_field[1];
  qhigh_dn = solver_field[0];

  x_up = solver_field32[7];
  x_dn = solver_field32[6];
  r_up = solver_field32[5];
  r_dn = solver_field32[4];
  p_up = solver_field32[3];
  p_dn = solver_field32[2];
  q_up = solver_field32[1];
  q_dn = solver_field32[0];
  ...
  if (g_debug_level > 0 && g_proc_id == 0) printf("#RG_Mixed CG_ND: N_outer: %d \n", N_outer);

  zero_spinor_field_32(x_up, N);
  zero_spinor_field_32(x_dn, N);

  if (solver_params.use_initial_guess == 0) {
    assign(phigh_up, Q_up, N);
    assign(phigh_dn, Q_dn, N);
    assign(rhigh_up, Q_up, N);
    assign(rhigh_dn, Q_dn, N);
    rho_dp = sourcesquarenorm;
  } else {
    // computing initial guess
    f(rhigh_up, rhigh_dn, P_up, P_dn);
    diff(rhigh_up, Q_up, rhigh_up, N);
    diff(rhigh_dn, Q_dn, rhigh_dn, N);
    assign(phigh_up, rhigh_up, N);
    assign(phigh_dn, rhigh_dn, N);
    rho_dp = (square_norm(rhigh_up, N, 1) + square_norm(rhigh_dn, N, 1));
  }

  assign_to_32(r_up, rhigh_up, N);
  assign_to_32(r_dn, rhigh_dn, N);
  rho_sp = rho_dp;
  assign_32(p_up, r_up, N);
  assign_32(p_dn, r_dn, N);

  iter_in_sp += inner_loop(x_up, x_dn, p_up, p_dn, q_up, q_dn, r_up, r_dn, &rho_sp, delta, f32,
                           (float)target_eps_sq, N, iter_out + iter_in_sp + iter_in_dp, max_iter,
                           0.0, 0.0, MCG_NO_PIPELINED, MCG_NO_PR);
  ...
  for (iter_out = 1; iter_out < N_outer; ++iter_out) {
    ...
    if (high_control == 0) {
      ...
      addto_32(P_up, x_up, N);
      addto_32(P_dn, x_dn, N);
      // compute real residual
      f(qhigh_up, qhigh_dn, P_up, P_dn);
      diff(rhigh_up, Q_up, qhigh_up, N);
      diff(rhigh_dn, Q_dn, qhigh_dn, N);
      beta_dp = 1 / rho_dp;
      rho_dp = (square_norm(rhigh_up, N, 1) + square_norm(rhigh_dn, N, 1));
      beta_dp *= rho_dp;
    }

    // the iteration limit was reached in the previous iteration, let's try to save the day using
    // double precision
    if (high_control == 1) {
      assign(phigh_up, rhigh_up, N);
      assign(phigh_dn, rhigh_dn, N);
      zero_spinor_field(xhigh_up, N);
      zero_spinor_field(xhigh_dn, N);
      beta_dp = 1 / rho_dp;
      iter_in_dp += inner_loop_high(xhigh_up, xhigh_dn, phigh_up, phigh_dn, qhigh_up, qhigh_dn,
                                    rhigh_up, rhigh_dn, &rho_dp, delta, f, target_eps_sq, N,
                                    iter_out + iter_in_sp + iter_in_dp, max_iter);
      rho_sp = rho_dp;
      // accumulate solution
      add(P_up, P_up, xhigh_up, N);
      add(P_dn, P_dn, xhigh_dn, N);
      // compute real residual
      f(qhigh_up, qhigh_dn, P_up, P_dn);
      diff(rhigh_up, Q_up, qhigh_up, N);
      diff(rhigh_dn, Q_dn, qhigh_dn, N);
    ...
    // if it seems like we're stuck and reaching the iteration limit, we skip this correction and
    // proceed in full precision above
    if (iter_out >= (N_outer - 2)) {
      ...
      assign_to_32(r_up, rhigh_up, N);
      assign_to_32(r_dn, rhigh_dn, N);
      rho_sp = rho_dp;  // not sure if it's fine to truncate this or whether one should calculate it
                        // in SP directly, it seems to work fine though
      assign_32(p_up, r_up, N);
      assign_32(p_dn, r_dn, N);
    }
...
void output_flops(const double seconds, const unsigned int N, const unsigned int iter_out,
                  const unsigned int iter_in_sp, const unsigned int iter_in_dp,
                  const double eps_sq) {
  double flops;
  ...
  if (g_debug_level > 0 && g_proc_id == 0) {
    ...
    if (N != VOLUME) {
      ...
      flops = 2 *
              (2 * (2 * 1608.0 + 2 * 3 * 4) + 2 * 3 * 4 +
               total_it * (2. * (2 * 1608.0 + 2 * 3 * 4) + 10 * 3 * 4)) *
              N / 1.0e6f;
    } else {
      /* 2 A + 2 Nc Ns + N_Count ( 2 A + 10 Nc Ns ) */
      flops = 2 *
              (2 * (1608.0 + 2 * 3 * 4) + 2 * 3 * 4 +
               total_it * (2. * (1608.0 + 2 * 3 * 4) + 10 * 3 * 4)) *
              N / 1.0e6f;
    }
    ...
    printf(
        "#RG_mixed CG_ND: flopcount (for e/o tmWilson only): t/s: %1.4e mflops_local: %.1f mflops: "
        "%.1f\n",
