...
void get_c(const double aa, const double bb, double c[], const int n) {
  const double inv_n = 1. / (double)n;
  double *f = calloc(n, sizeof(double)); /*vector(0,n-1);*/
  fflush(stdout);
  const double bma = 0.5 * (bb - aa);
  const double bpa = 0.5 * (bb + aa);
  for (int k = 0; k < n; k++) {
    double y = cos(PI * (k + 0.5) * inv_n);
    f[k] = f_pre(y * bma + bpa);
  }
  const double fac = 2.0 * inv_n;
  for (int j = 0; j < n; j++) {
    double sum = 0.0;
    for (int k = 0; k < n; k++) sum += f[k] * cos(PI * j * (k + 0.5) * inv_n);
    c[j] = fac * sum;
  }
  free(f);
}
...
void poly_precon(spinor *const R, spinor *const S, const double prec, const int n) {
  double maxev = 4., tnorm, minev = g_mu * g_mu, auxnorm;
  static spinor *d_, *d, *dd_, *dd, *aux_, *aux, *aux3_, *aux3;
  static int initp = 0;
  static double *c;
  const int N = VOLUME;
  ...
  double fact1 = 4. / (maxev - minev);
  double fact2 = -2 * (maxev + minev) / (maxev - minev);
...
void poly_nonherm_precon(spinor *const R, spinor *const S, const double e, const double d,
