...
extern void _FT(zgeev)(char *jobvl, char *jobvr, int const *n, _Complex double *a, int const *lda,
                       _Complex double *w, _Complex double *vl, int *ldvl, _Complex double *vr,
                       int *ldvr, _Complex double *work, int *lwork, double *rwork, int *info);

extern void _FT(dposv)(char *jobvl, int const *n, int const *nrhs, double *mat, int const *lda,
                       double *rhs, int const *ldrhs, int const *lapackINfo);
 ...
 *@param rawp raw lattice momentum (how it goes to the fft), will be converted to the correct
 *lattice momentum internally
 ...
 * this function applies the desired treelevel Dirac operator with an arbitrary (_Complex double)
 * exponent to the given spinor
 */
 ...
 */
void planeWave(spinor *spinor, int k, int rawp[4], int tt, int ll,
               unsigned int momspace /* =false */);
...
#define _exp_complex(/*_Complex double*/ x, /*_Complex double*/ z, /*double*/ dum) x = cexp(z);

/* res = z^x = exp ( x * ln(z)) */
#define _pow_complex(/*_Complex double*/ res, /*_Complex double*/ z, /*_Complex double*/ x, \
                     /*_Complex double*/ dum)                                               \
  res = cpow(z, x);
...
#define _complex_muleq_complex(z1, z2, dum) (z1) *= (z2);
...
#define _spinor_scalar_prod(proj, r, s)                                                         \
  (proj) =                                                                                      \
      conj((r).s0.c0) * (s).s0.c0 + conj((r).s0.c1) * (s).s0.c1 + conj((r).s0.c2) * (s).s0.c2 + \
      conj((r).s1.c0) * (s).s1.c0 + conj((r).s1.c1) * (s).s1.c1 + conj((r).s1.c2) * (s).s1.c2 + \
      conj((r).s2.c0) * (s).s2.c0 + conj((r).s2.c1) * (s).s2.c1 + conj((r).s2.c2) * (s).s2.c2 + \
      conj((r).s3.c0) * (s).s3.c0 + conj((r).s3.c1) * (s).s3.c1 + conj((r).s3.c2) * (s).s3.c2;
