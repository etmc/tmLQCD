...
int mixed_cg_her(spinor *const P, spinor *const Q, solver_params_t solver_params,
                 const int max_iter, double eps_sq, const int rel_prec, const int N, matrix_mult f,
                 matrix_mult32 f32) {
  ...
  for (int i = 0; i < N_outer; i++) {
    ...
    for (j = 0; j <= max_inner_it; j++) {
      ...
      // if (((err <= eps_sq) && (rel_prec == 0)) || ((err <= eps_sq*squarenorm) && (rel_prec ==
      // 1))){
      if ((err <= mixcg_innereps * sqnrm) || (j == max_inner_it) ||
          ((1.3 * err <= eps_sq) && (rel_prec == 0)) ||
          ((1.3 * err <= eps_sq * sourcesquarenorm) && (rel_prec == 1))) {
        break;
      }
      beta_cg = err / sqnrm2;
    ...
    if (((sqnrm_d <= eps_sq) && (rel_prec == 0)) ||
        ((sqnrm_d <= eps_sq * sourcesquarenorm) && (rel_prec == 1))) {
      etime = gettime();
      ...
      if (g_debug_level > 0 && g_proc_id == 0) {
        if (N != VOLUME) {
          ...
          flops = (2 * (2 * 1608.0 + 2 * 3 * 4) + 2 * 3 * 4 +
                   iter * (2. * (2 * 1608.0 + 2 * 3 * 4) + 10 * 3 * 4)) *
                  N / 1.0e6f;
          printf("# mixed CG: iter: %d eps_sq: %1.4e t/s: %1.4e\n", iter, eps_sq, etime - atime);
          printf(
              "# mixed CG: flopcount (for e/o tmWilson only): t/s: %1.4e mflops_local: %.1f "
              "mflops: %.1f\n",
              etime - atime, flops / (etime - atime), g_nproc * flops / (etime - atime));
        } else {
          /* 2 A + 2 Nc Ns + N_Count ( 2 A + 10 Nc Ns ) */
          flops = (2 * (1608.0 + 2 * 3 * 4) + 2 * 3 * 4 +
                   iter * (2. * (1608.0 + 2 * 3 * 4) + 10 * 3 * 4)) *
                  N / 1.0e6f;
          printf("# mixed CG: iter: %d eps_sq: %1.4e t/s: %1.4e\n", iter, eps_sq, etime - atime);
          printf(
              "# mixed CG: flopcount (for non-e/o tmWilson only): t/s: %1.4e mflops_local: %.1f "
              "mflops: %.1f\n",
              etime - atime, flops / (etime - atime), g_nproc * flops / (etime - atime));
        }
      }
