...
int sumr(spinor *const P, spinor *const Q, const int max_iter, double eps_sq) {
...
#if DEBUG_SUMR == 1
  printf(
      "delta=%g;\t phihat=%g;\t tauhat=%g;\t w=%g;\t p=%g;\t phi=%g;\t s=%g;\t lambda=%g;\t "
      "r_off=%g;\t r_off_old=%g;\t r_diag=%g;\t r_diag_old=%g;\t gamm=%g;\t sigma=%g;\t c=%g;\t "
      "v=%g;\t v_til=%g;\t ",
      delta, cabs(phi_hat), tau_hat, square_norm(w), square_norm(p), cabs(phi), s, cabs(lambda),
      cabs(r_off), cabs(r_off_old), cabs(r_diag), cabs(r_diag_old), cabs(gamm), sigma, cabs(c),
