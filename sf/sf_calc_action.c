...
/* this function sets all the links in the lattice to one, except the temporal links at t wich are
 * set to zero */
...
/* it calculates an su3 matrix "u" which is gonna be the (continuum) spatially constant abelian
 * field */
...
void print_su3_matrix(su3 u) {
  printf(" %e i %e  %e i %e  %e i %e \n", creal(u.c00), cimag(u.c00), creal(u.c01), cimag(u.c01),
         creal(u.c02), cimag(u.c02));
  printf(" %e i %e  %e i %e  %e i %e \n", creal(u.c10), cimag(u.c10), creal(u.c11), cimag(u.c11),
         creal(u.c12), cimag(u.c12));
  printf(" %e i %e  %e i %e  %e i %e \n", creal(u.c20), cimag(u.c20), creal(u.c21), cimag(u.c21),
...
/* differently as in the previous cases, now bulk and boundaries are calculated in different
 * functions */
        ...
        if (mu1 != mu2) {
          ...
          if ((g_t[ix] == 0 || g_t[ix] == t) && (mu1 != 0 && mu2 != 0)) {
            ac *= c1_ss;
                   ...
                   mu1 == 0) { /* 1 movement in t <=> 2 links on a (time) boundary */
            ...
            ac *= c1_tss;
                   ...
                   mu2 == 0) { /* 2 movement in t <=> 1 links on a (time) boundary */
            ...
            ac *= c1_tts;
                     ...
                     (mu1 == 0 || mu2 == 0)) { /* out of the lattice on the right side */
...
double measure_wilson_action_sf_separate_boundary(int t, double beta) {
  ...
  plaquette = measure_plaquette_sf_weights_bulk(t) + measure_plaquette_sf_weights_boundary_0() +
              measure_plaquette_sf_weights_boundary_t(t);
...
but "not hard-coded" */
double measure_wilson_action_sf_weights_improvement_separate_boundary(int t, double beta, double cs,
                                                                      double ct) {
  double plaquette;
  double wilson;
...
double measure_iwasaki_action_sf(int t, double beta, double cs, double ct, double c0, double c1,
                                 double c1_ss, double c1_tss, double c1_tts) {
  double plaquette;
  double rectangle;
  double iwasaki;
...
/* For other choice of the weight factors (not option B), this will NOT be the minimal action
   configuration. Actually, besides in this case B, the analytical expression will not be know so,
...
/* the next function gives us the classical lattice background field (V) "plaquette action S[V]"
 * with SF b.c. */
...
double lattice_background_plaquette_action_sf(int t, double beta, double ct, double eta) {
  ...
  plaquette_back = factor3 * (sin(factor_a) * sin(factor_a) + sin(factor_b) * sin(factor_b) +
                              sin(factor_c) * sin(factor_c));
...
/* implementation of the derivative of the analytical expression taken from Rainer's notes in
 * Schladming (eq.71)*/
/* NOTE: there was a sign mistake in Rainer's notes!!! */
...
double partial_lattice_lo_effective_plaquette_action_sf_k(int t, double beta, double ct,
                                                          double eta) {
  double pi;
...
double partial_lattice_lo_effective_iwasaki_action_sf_k(int t, double beta, double c0, double c1,
                                                        double eta) {
  double pi;
  double factor1, factor;
  double partial_eff_lo;
  ...
  partial_eff_lo = factor1 * (c0 * (sin(factor) + sin(2. * factor)) +
                              4. * c1 * (sin(2. * factor) + sin(4. * factor)));
...
/* it calculates an su3 matrix "u" which is gonna be the eighth-generator of SU(3) "lambda_8" times
 * "i" */
...
/* it has been taken from Rainer's notes in Schladming (eq.73) (we've checked and gotten the same
 * formula) */
...
double partial_iwasaki_action_sf_respect_to_eta(int t, double beta, double cs, double ct, double c0,
                                                double c1, double c1_ss, double c1_tss,
                                                double c1_tts) {
  double partial_plaquette;
