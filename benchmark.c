...
int main(int argc, char *argv[]) {
  ...
  if (even_odd_flag) {
    ...
    if (g_proc_id == 0) {
      printf(
          "# The following result is just to make sure that the calculation is not optimized away: "
          "%e\n",
          antioptaway);
...
#endif
      printf("\n%12d Mflops(total) %8d Mflops(process)", (int)(g_nproc * 1608.0f / sdt),
             (int)(1608.0f / sdt));
#ifdef TM_USE_OMP
    ...
    if (g_proc_id == 0) {
      printf(
          "# The following result is printed just to make sure that the calculation is not "
          "optimized away: %e\n",
          antioptaway);
      printf("# Communication switched off: \n\n%12d Mflops(total) %8d Mflops(process)",
             (int)(g_nproc * 1608.0f / dt), (int)(1608.0f / dt));
#ifdef TM_USE_OMP
    ...
    if (g_proc_id == 0) {
      ...
      printf("# Total compute time %e sec, variance of the time %e sec. (%d iterations).\n\n", sdt,
             sqdt, j_max);
      printf(" %12d Mflops(total) %8d Mflops(process)", (int)(1680.0f * g_nproc / sdt),
             (int)(1680.0f / sdt));
#ifdef TM_USE_OMP
