...
int init_integrator() {
  ...
  for (i = 0; i < no_monomials; i++) {
    ...
    if (ts < Integrator.no_timescales && ts > -1) {
      ...
      if (g_proc_id == 0) {
        fprintf(stderr,
                "Warning: monomial %d is not on a valid timescale and will not be integrated\n", i);
      }
    }
  }
  for (i = 0; i < Integrator.no_timescales; i++) {
...
void integrate_omf4(const double tau, const int S, const int halfstep, const double tau2) {
  ...
  if (S == 0) {
    for (j = 1; j < itgr->n_int[0]; j++) {
      ...
      update_momenta(itgr->mnls_per_ts[0], 0.5 * (1 - 2. * (omf4_lamb + omf4_vartheta)) * eps,
                     itgr->no_mnls_per_ts[0], &itgr->hf);
      ...
      update_momenta(itgr->mnls_per_ts[0], omf4_lamb * eps, itgr->no_mnls_per_ts[0], &itgr->hf);
      update_gauge(omf4_rho * eps, &itgr->hf);
      update_momenta(itgr->mnls_per_ts[0], 2 * omf4_vartheta * eps, itgr->no_mnls_per_ts[0],
                     &itgr->hf);
    }
    update_gauge(omf4_rho * eps, &itgr->hf);
    update_momenta(itgr->mnls_per_ts[0], omf4_lamb * eps, itgr->no_mnls_per_ts[0], &itgr->hf);
    ...
    update_gauge(omf4_rho * eps, &itgr->hf);
    if (halfstep != 1) {
      update_momenta(itgr->mnls_per_ts[0], omf4_vartheta * (eps + eps2), itgr->no_mnls_per_ts[0],
                     &itgr->hf);
    }
  } else {
    for (i = 1; i < itgr->n_int[S]; i++) {
      ...
      update_momenta(itgr->mnls_per_ts[S], 0.5 * (1 - 2. * (omf4_lamb + omf4_vartheta)) * eps,
                     itgr->no_mnls_per_ts[S], &itgr->hf);
      ...
      update_momenta(itgr->mnls_per_ts[S], 2 * omf4_vartheta * eps, itgr->no_mnls_per_ts[S],
                     &itgr->hf);
    }
    itgr->integrate[S - 1](omf4_rho * eps, S - 1, 0, omf4_theta * eps);
    update_momenta(itgr->mnls_per_ts[S], omf4_lamb * eps, itgr->no_mnls_per_ts[S], &itgr->hf);
    ...
    if (S == itgr->no_timescales - 1) {
      ...
      itgr->integrate[S - 1](omf4_rho * eps, S - 1, halfstep, omf4_rho * eps2);
    if (halfstep != 1 && S != itgr->no_timescales - 1) {
      update_momenta(itgr->mnls_per_ts[S], omf4_vartheta * (eps + eps2), itgr->no_mnls_per_ts[S],
                     &itgr->hf);
    }
  }
...
void integrate_2mn(const double tau, const int S, const int halfstep, const double tau2) {
  ...
  if (S == 0) {
    ...
    if (S == itgr->no_timescales - 1) {
      ...
      itgr->integrate[S - 1](eps / 2., S - 1, halfstep, eps2 / 2.);
    if (halfstep != 1 && S != itgr->no_timescales - 1) {
      update_momenta(itgr->mnls_per_ts[S], itgr->lambda[S] * (eps + eps2), itgr->no_mnls_per_ts[S],
                     &itgr->hf);
    }
  }
...
void integrate_leap_frog(const double tau, const int S, const int halfstep, const double tau2) {
  ...
  if (S == 0) {
    ...
    if (S == itgr->no_timescales - 1) {
      ...
      itgr->integrate[S - 1](eps, S - 1, halfstep, eps2);
    if (halfstep != 1 && S != itgr->no_timescales - 1) {
